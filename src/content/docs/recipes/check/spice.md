---
title: "spice"
description: "The `spice:` SPICE-wire check verb for VMs — handshake, native-SPICE display screenshots, input injection — served out-of-process by candy/plugin-spice (the vendored Shells-com/spice library)."
---
...
RPC fd-passing. See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) "Connecting from a remote workstation"
...
- **`libvirt:`** — every call goes through libvirtd RPC. Use when the thing
  ...
  verb served out-of-process — by `candy/plugin-vm` (see [`/charly-check:libvirt`](/recipes/check/libvirt/)).
...
- [`/charly-check:libvirt`](/recipes/check/libvirt/) — the sibling declarative `libvirt:` check verb,
  served out-of-process by `candy/plugin-vm` (libvirt RPC: framebuffer
  screenshot, send-key, QMP, snapshots, guest agent).
- [`/charly-check:check`](/recipes/check/check/) — the unified check system and the Op (a plan step)
  that holds every verb discriminator + modifier.
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — the arch VM that ships SPICE by default; "Connecting from
  a remote workstation".
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — the external-charly-verb plugin model the spice
  verb follows.
