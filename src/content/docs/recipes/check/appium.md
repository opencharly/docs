---
title: "appium"
description: "The `appium:` check verb (declarative, dispatched to the out-of-process plugin candy/plugin-appium) — Android UI automation, WebDriver session management, APK install via Appium, element find/click/send-keys, mobile WebDriver caps."
---
...
`github.com/zach-klippenstein/goadb` (used by the sibling [`/charly-check:adb`](/recipes/check/adb/))
has the same maintenance posture — pinned to a 2020 release. Same
mitigation if it stops working.

## Related skills

- [`/charly-check:adb`](/recipes/check/adb/) — sibling verb for low-level Android Debug Bridge
...
- [`/charly-check:check`](/recipes/check/check/) — the unified check system and the Op struct that
