---
title: "check — beds-and-r10"
description: "Detail page of the check recipe card."
---

<!-- DO-NOT-EDIT — generated by `charly docs generate`; run `task docs:sync` to regenerate. -->

*Detail page of the [check](/recipes/check/check/) recipe card.*

# Beds and R10 — disposable acceptance gate reference

Full detail for [`/charly-check:check`](/recipes/check/check/). Sole-owner content: the R10 gate-by-change-class matrix and the flag-discipline catalog (every other skill points here).
...
See [`/charly-internals:disposable`](/recipes/internals/disposable/) "The resource-arbitration axis" + [`/charly-core:deploy`](/recipes/core/deploy/).
...
never host `nvidia-smi`. See [`/charly-vm:vm`](/recipes/vm/vm/) "GPU passthrough (VFIO)".
...
token until then. Full model + RCA: [`/charly-vm:vm`](/recipes/vm/vm/) "GPU driver-mode switch" +
[`/charly-internals:disposable`](/recipes/internals/disposable/) "resource-arbitration axis".
...
no authorization — the lever catalog is [`/charly-internals:agents`](/recipes/internals/agents/) "Speed
levers".
...
[`/charly-core:clean`](/recipes/core/clean/).
...
for the unattended destroy+rebuild (see [`/charly-internals:disposable`](/recipes/internals/disposable/)). Two
...
each long bed, which no sub-agent can own (see [`/charly-internals:agents`](/recipes/internals/agents/)
...
identity with `charly vm list`; see [`/charly-vm:vm`](/recipes/vm/vm/) "`charly vm destroy` — the DEPLOY name keys the
domain" and [`/charly-internals:agents`](/recipes/internals/agents/) "The binding rule".
...
a missing daemon. See [`/charly-vm:vm`](/recipes/vm/vm/) "Prereq", [`/charly-check:libvirt`](/recipes/check/libvirt/).
...
- **`charly check run <bed>`** — runs a `disposable: true` R10 deploy (full
  ...
  [`references/authoring-gotchas.md`](/recipes/check/check/authoring-gotchas/) (Agent Driven Evaluation) and
...
3. **Verify critical OCI / capability labels post-build** (R8) — `charly box labels <ref> --format init` (or any `ai.opencharly.<key>` shorthand) prints the built ref's label and exits non-zero when absent; `charly box labels <ref>` lists the whole capability contract ([`/charly-internals:capabilities`](/recipes/internals/capabilities/)). Empty / missing label → the detection path silently returned nil → regression.
...
multi-hop chain above. See [`references/cross-deployment-probing.md`](/recipes/check/check/cross-deployment-probing/) ("Cross-deployment
