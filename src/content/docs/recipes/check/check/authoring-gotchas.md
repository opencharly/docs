...
Full detail for [`/charly-check:check`](/recipes/check/check/): Agent Driven Evaluation (the agent-graded steps + the six-stage loop), the numbered non-obvious authoring gotchas, and the AI-iteration harness's pgrep-deadlock defense.
...
Worked example: [`/charly-coder:sshd`](/recipes/coder/sshd/) ships exactly the `-u user --` pattern. Alternative: give the step `context: [deploy]` so it runs against a live container where you control the user-switch externally.
