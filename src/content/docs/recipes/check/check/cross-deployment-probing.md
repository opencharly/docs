---
title: "check — cross-deployment-probing"
...
Full detail for [`/charly-check:check`](/recipes/check/check/): the host-target image-preflight fetch surface, and cross-deployment probing (one deployment testing another via venue-from-position members and `${HOST:...}` addressing).
...
[`/charly-local:local-spec`](/recipes/local/local-spec/) "What the deploy does NOT do" and the project rulebook
...
- **venue-from-position members** — bring the driver up alongside the subject on
  the shared `charly` network (see [`/charly-core:deploy`](/recipes/core/deploy/) "Sibling members"). A deploy
...
The `chrome-headless` box sets it; see [`/charly-check:cdp`](/recipes/check/cdp/) "Requirements".
