...
Full detail for [`/charly-check:check`](/recipes/check/check/). Covers the deploy-context-only probe verbs that dispatch out-of-process to their serving plugin candy. The `kube`/`adb`/`appium`/`spice`/`libvirt` verbs are each served by their own plugin and documented in their own skill — see this skill's "Related skills".
...
port-publishing gotcha, validator rules) lives in [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/).
