...
backend (see [`/charly-vm:vm`](/recipes/vm/vm/) "Prereq"). The `charly check run <bed>` path for
...
- [`/charly-check:check`](/recipes/check/check/) — parent router; the `libvirt:` verb catalog entry, the artifact-validation modifiers, and `charly check live <vm> --filter libvirt`.
- [`/charly-check:spice`](/recipes/check/spice/) — the sibling `spice:` SPICE-wire check verb, also served out-of-process by `candy/plugin-vm`/`candy/plugin-spice`.
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — the out-of-process provider model that serves `libvirt` (`candy/plugin-vm`).
- [`/charly-vm:vm`](/recipes/vm/vm/) — the `charly vm` VM lifecycle command family (create/start/stop/destroy/ssh/console/gpu) — separate from this introspection verb.
