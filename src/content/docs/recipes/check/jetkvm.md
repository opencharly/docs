---
title: "jetkvm"
description: "The `jetkvm:` check/control verb (declarative, dispatched to the out-of-process plugin candy/plugin-jetkvm) — drive a JetKVM IP-KVM without a browser over its own WebRTC control plane: screenshots, keyboard/mouse input, ATX/DC power, virtual media, USB gadget, device configuration."
---
...
- [`/charly-check:check`](/recipes/check/check/) — parent router; the `jetkvm:` verb dispatches
  out-of-process via `candy/plugin-jetkvm`.
- [`/charly-check:vnc`](/recipes/check/vnc/) — the sibling desktop-automation verb (RFB for a
  container/VM display rather than a physical KVM).
- [`/charly-check:adb`](/recipes/check/adb/) — the sibling out-of-process device verb.
- [`/charly-image:layer`](/recipes/image/layer/) — layer authoring; the ordered `plan:` step list is part
  of every `charly.yml`.
