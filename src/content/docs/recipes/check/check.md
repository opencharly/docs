---
title: "check"
description: "MUST be invoked for every `charly check` mode, candy or box plan authoring, disposable test beds, R10 runs, agent grading, baked plan labels, or check operation."
---
...
- [authoring-gotchas](/recipes/check/check/authoring-gotchas/)
- [beds-and-r10](/recipes/check/check/beds-and-r10/)
- [cross-deployment-probing](/recipes/check/check/cross-deployment-probing/)
- [live-probe-verbs](/recipes/check/check/live-probe-verbs/)
- [verb-catalog](/recipes/check/check/verb-catalog/)
...
autodetect. Full detail: [`references/beds-and-r10.md`](/recipes/check/check/beds-and-r10/).
...
grader stays opt-in. Full detail: [`references/authoring-gotchas.md`](/recipes/check/check/authoring-gotchas/).
...
|---|---|---|
| Disposable beds, R10 acceptance gate | [`references/beds-and-r10.md`](/recipes/check/check/beds-and-r10/) | The bed roster, the "R10 gate by change class" matrix (sole owner), the 11 Testing Standards, "Flag discipline" (sole owner), exit codes, the three primary modes, wall-clock/timing data |
| Deterministic verb catalog | [`references/verb-catalog.md`](/recipes/check/check/verb-catalog/) | The `plan:` authoring model, the file/package/service/port/process/command/http/... verb catalog, shared modifiers, matcher forms, verb routing (executor selection), runtime variables, `charly.yml` overlay rules |
| Live-container probe verbs | [`references/live-probe-verbs.md`](/recipes/check/check/live-probe-verbs/) | The out-of-process `cdp`/`wl`/`dbus`/`vnc`/`mcp` verbs — method allowlists, worked examples, artifact-validation modifiers |
| Authoring gotchas, ADE, AI harness | [`references/authoring-gotchas.md`](/recipes/check/check/authoring-gotchas/) | Agent Driven Evaluation (the six-stage loop, the agent-grader contract), the 12 numbered authoring gotchas, the AI-iteration harness pgrep-deadlock defense |
| Cross-deployment probing | [`references/cross-deployment-probing.md`](/recipes/check/check/cross-deployment-probing/) | Image preflight (host-target runs), venue-from-position members, `${HOST:...}` addressing, the pod→pod and host→pod/VM worked examples |

## Related skills

- **Live-container probe verbs — ALL out-of-process** — the `wl:` Wayland verb ([`/charly-check:wl`](/recipes/check/wl/)), the `cdp:` Chrome-DevTools verb ([`/charly-check:cdp`](/recipes/check/cdp/)), the `vnc:` VNC-framebuffer verb ([`/charly-check:vnc`](/recipes/check/vnc/)), the `dbus:` D-Bus verb ([`/charly-check:dbus`](/recipes/check/dbus/)), the `mcp:` MCP-protocol verb ([`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/)), the `record:` recording verb ([`/charly-check:record`](/recipes/check/record/)), the `kube:` cluster-probe verb ([`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/)), the `adb:` ADB verb ([`/charly-check:adb`](/recipes/check/adb/)), the `appium:` Android-UI verb ([`/charly-check:appium`](/recipes/check/appium/)), the `spice:` SPICE-wire verb ([`/charly-check:spice`](/recipes/check/spice/)), and the `libvirt:` VM-probe verb ([`/charly-check:libvirt`](/recipes/check/libvirt/)) are NOT host `charly check` subcommands — each is a declarative check verb served out-of-process by its plugin (`candy/plugin-wl`, `candy/plugin-cdp`, `candy/plugin-vnc`, `candy/plugin-dbus`, `candy/plugin-mcp`, `candy/plugin-record`, `candy/plugin-kube`, `candy/plugin-adb`, `candy/plugin-appium`, `candy/plugin-spice`, `candy/plugin-vm`). See [`references/live-probe-verbs.md`](/recipes/check/check/live-probe-verbs/).
- [`/charly-image:layer`](/recipes/image/layer/) — layer authoring; the ordered `plan:` step list is part of every `charly.yml`.
- [`/charly-image:image`](/recipes/image/image/) — image-level plan steps at composition time.
- [`/charly-core:deploy`](/recipes/core/deploy/) — local `charly.yml` overlay rules and the plan-step merge.
- [`/charly-build:validate`](/recipes/build/validate/) — static schema + cross-scope variable checks; the first
  gate before `charly box build`.
- [`/charly-build:build`](/recipes/build/build/) — how check entries are embedded into the OCI label at build
  time; LABELs-at-end cache behavior.
- [`/charly-build:inspect`](/recipes/build/inspect/) — view the merged plan / description structure as JSON.
- [`/charly-build:migrate`](/recipes/build/migrate/) — `charly migrate` brings legacy configs up to the
  current schema (one flat ordered `plan:` list per entity).
- [`/charly-internals:go`](/recipes/internals/go/) — implementation map. Core keeps the check-harness
...
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — how `LabelDescriptionSet` is written into the Containerfile
  via `writeJSONLabel`, and why the LABEL block lives at the end of the
  final stage.
- [`/charly-internals:capabilities`](/recipes/internals/capabilities/) — the `ai.opencharly.description` label (`LabelDescriptionSet`) carrying the baked plan is
  part of the same capability contract as `LabelService`.
- [`/charly-internals:agents`](/recipes/internals/agents/) — the sub-agents (`check-bed-runner`,
...
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — a `check:` step's `<word>: <input>` verb sugar dispatches
