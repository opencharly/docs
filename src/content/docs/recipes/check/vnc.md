---
title: "vnc"
description: "VNC/RFB desktop automation via the declarative `vnc:` check verb, served out-of-process by `candy/plugin-vnc` (covers pod AND vm targets)."
---
...
author them with `context: [deploy]`. See [`/charly-check:check`](/recipes/check/check/) for the full YAML
shape. Example:
...
See [`/charly-check:cdp`](/recipes/check/cdp/) for the full Google sign-in recipe.
...
- [`/charly-check:check`](/recipes/check/check/) — parent router; the `vnc:` verb dispatches out-of-process via `candy/plugin-vnc` (the host pre-resolves the pod/vm RFB endpoint).
- [`/charly-check:wl`](/recipes/check/wl/) — Wayland-native desktop automation (sibling verb; works on NVIDIA headless).
- [`/charly-check:cdp`](/recipes/check/cdp/) — Chrome DevTools Protocol automation (sibling verb; same container, different protocol).
...
- [`/charly-check:wl`](/recipes/check/wl/) — the `wl:` verb's sway-* methods for Sway compositor control (window management, workspaces)
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — VNC password storage, `secret_backend` setting, `migrate-secrets` command
- [`/charly-core:service`](/recipes/core/service/) — Managing wayvnc supervisord service
- [`/charly-core:deploy`](/recipes/core/deploy/) — VNC password setup in deployment workflows
- [`/charly-core:shell`](/recipes/core/shell/) — Executing commands inside containers
- [`/charly-image:layer`](/recipes/image/layer/) — wayvnc layer configuration (port tcp:5900)
...
**Workflow position:** Desktop automation. Use for pixel-level interaction when CDP can't reach the element. See also [`/charly-check:cdp`](/recipes/check/cdp/) (DOM, preferred), [`/charly-check:wl`](/recipes/check/wl/) (sway subgroup) (window).
