---
title: "fedora-coder"
description: "Kitchen-sink development box: coding + AI-coding CLIs + DevOps tooling in one container."
---
...
`container-nesting` kernel RCA — see [`/charly-distros:container-nesting`](/recipes/distros/container-nesting/) for the
...
alongside [`/charly-coder:arch-coder`](/recipes/coder/arch-coder/) on the same host (both want sshd on 2222 and
...
`strip_components: 1` modifier. See [`/charly-coder:uv`](/recipes/coder/uv/) and [`/charly-image:layer`](/recipes/image/layer/).

**`gh` owns all git tooling** — the `gh` candy installs `gh` + `git` +
`git-lfs` (with the noscripts + post-install trigger). `dev-tools`
intentionally does NOT install any of these. See [`/charly-coder:gh`](/recipes/coder/gh/).
...
# (see [/charly-check:check](/recipes/check/check/) "Live vs disposable executor selection")
charly check box ghcr.io/opencharly/fedora-coder:latest
...
mount path is `/workspace`. See [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) for the three
...
Conflicts with [`/charly-coder:charly-arch`](/recipes/coder/charly-arch/) (identical ports) and several
...
google-cloud-npm) by forking charly.yml. See [`/charly-image:image`](/recipes/image/image/) for authoring.

## Key Candies

- [`/charly-tools:charly`](/recipes/tools/charly/) — the full toolchain: charly binary + virtualization + gocryptfs + socat (+ podman-machine, gvisor-tap-vsock for nested VMs)
- [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) — MCP gateway; auto-falls back to `opencharly/charly` when no bind-mount present
- [`/charly-distros:container-nesting`](/recipes/distros/container-nesting/) — rootless nested podman recipe (authoritative RCA for `mount_too_revealing()` + `unmask=/proc/*`)
- [`/charly-coder:sshd`](/recipes/coder/sshd/) — SSH daemon + passwordless sudo for the `user` account
- [`/charly-coder:forgecode`](/recipes/coder/forgecode/) — the 5th AI coding CLI
- [`/charly-coder:language-runtimes`](/recipes/coder/language-runtimes/) — Go + PHP + .NET + nodejs-devel + python3-devel (system-python stack, no pixi)
- [`/charly-coder:uv`](/recipes/coder/uv/) — direct-download binary at /usr/local/bin/uv
- [`/charly-coder:gh`](/recipes/coder/gh/) — GitHub CLI + git + git-lfs (single-responsibility)
...
|---|---|---|---|
| [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) | `fedora-nonfree` (Fedora 43) | rpm | `user:user` (create) |
| [`/charly-coder:arch-coder`](/recipes/coder/arch-coder/) | `arch` | pac + aur | `user:user` (create) |
...
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — adds the Hermes AI agent daemon on top of the AI CLIs; prefer when you want an agent, not just CLI tools.

## Related Commands

- [`/charly-core:shell`](/recipes/core/shell/) — open an interactive shell inside the container (as user, with sudo)
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — deploy setup (--bind project=…, tunnel, --update-all)
- [`/charly-core:start`](/recipes/core/start/), [`/charly-core:stop`](/recipes/core/stop/) — lifecycle
- [`/charly-check:check`](/recipes/check/check/) — live-service tests (`charly check live <name>`) and build-scope tests (`charly check box <ref>`)
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — MCP gateway documentation; `--no-default-repo` to disable auto-fallback
- [`/charly-vm:vm`](/recipes/vm/vm/) — nested libvirt VMs (via virtqemud inside the container)
- [`/charly-image:layer`](/recipes/image/layer/) — authoring reference (covers the new `strip_components:` modifier)
