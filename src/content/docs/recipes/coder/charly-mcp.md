---
title: "charly-mcp"
description: "MCP server exposing the full charly CLI as tools (Streamable HTTP on port 18765)."
---
...
See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) Part 2 for the full server architecture: Kong
...
`CHARLY_PROJECT_DIR` is always populated. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/)
"Project-dir wiring".
...
(e.g. [`/charly-coder:charly-arch`](/recipes/coder/charly-arch/)) and host-networked ones (e.g.
[`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/), [`/charly-distros:charly-fedora`](/recipes/distros/charly-fedora/)).
...
- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) — kitchen-sink dev box; uses pattern 3 (auto-fallback) by default, pattern 1 when the developer wants local edits visible.
- [`/charly-coder:charly-arch`](/recipes/coder/charly-arch/) — Arch-based charly toolchain box (bridge network, ports 2222/18765).
...
- [`/charly-tools:charly`](/recipes/tools/charly/) — The underlying binary candy this wraps.
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) — Init system for the `charly-mcp` program.
- [`/charly-jupyter:jupyter-mcp`](/recipes/jupyter/jupyter-mcp/) — Sibling MCP server (notebook manipulation, FastMCP-based).
- [`/charly-selkies:chrome-devtools-mcp`](/recipes/selkies/chrome-devtools-mcp/) — Sibling MCP server (browser automation, mcp-proxy wrapper).

## Cross-References

- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — **Part 2: Server** is the authoritative reference for `charly mcp serve` architecture (externalized to `candy/plugin-mcp`), destructive-hint policy, `--read-only` filter, and the managed project-prefix logic.
- [`/charly-image:image`](/recipes/image/image/) — "Project directory resolution" covers the `-C` / `--dir` / `CHARLY_PROJECT_DIR` global flag and `--repo` / `CHARLY_PROJECT_REPO`.
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — `--bind project=<path>` is the deployer's handshake with this candy's `volume:` declaration.
- [`/charly-check:check`](/recipes/check/check/) — Deploy-scope `mcp:` test verb methods used here.
- [`/charly-internals:go`](/recipes/internals/go/) — the `charly __cli-model` host seam (`charly/cli_model_cmd.go`) the externalized server consumes; `computeProjectPrefix`/`childCharlyEnv` in `candy/plugin-mcp/serve.go` own the charly.yml-presence check.
