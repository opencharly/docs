---
title: "uv"
description: "uv + uvx — Astral's fast Python package/project manager."
---
...
- HOME-dependency — the binary path would vary per-image's `$HOME`, and
  ...
  [`/charly-internals:generate-source`](/recipes/internals/generate-source/) "UID-keyed sibling grouping").
- Confusion — "uv is installed but `which uv` says not found" for
  anyone who isn't in a pixi shell.

So uv installs the way [`/charly-coder:typst`](/recipes/coder/typst/) and [`/charly-languages:pixi`](/recipes/languages/pixi/) itself do
...
`strip_components: 1` modifier (see [`/charly-image:layer`](/recipes/image/layer/) "Download verb")
...
- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) — the canonical kitchen-sink dev box.
- Any box composing `hermes-full` or directly including `uv`.

## Related Candies

- [`/charly-languages:pixi`](/recipes/languages/pixi/) — direct-download pattern this candy now mirrors. Pixi remains a legitimate multi-binary Python env manager for boxes that genuinely need one (jupyter, whisper, openwebui); uv is too small to justify the overhead.
- [`/charly-coder:typst`](/recipes/coder/typst/) — sibling direct-download binary pattern.
- [`/charly-languages:python`](/recipes/languages/python/) — the pixi-python meta-layer this candy does **not** depend on.
- [`/charly-coder:rust`](/recipes/coder/rust/) — system Rust toolchain, if you want to build uv from source instead (`cargo install uv`). Usually not worth it — the Astral binary is pre-optimized.

## Related Commands

- [`/charly-image:layer`](/recipes/image/layer/) — authoring reference (covers `download:` verb + `strip_components:` modifier)
- [`/charly-core:shell`](/recipes/core/shell/) — run `uv` inside a container
