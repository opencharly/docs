---
title: "ubuntu-coder"
description: "Kitchen-sink development box on Ubuntu 24.04 noble: coding + AI-coding CLIs + DevOps tooling in one container."
---
...
**Why adopt over rename?** Ubuntu's cloud-init tooling, documentation, and `/etc/passwd` metadata all expect the user to be named `ubuntu`. Renaming fights the upstream contract. Adopting honors it. See [`/charly-image:image`](/recipes/image/image/) "user_policy" for the full rationale and the 3-value policy table.
...
Identical to [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/) — the only diff is `User` field.
...
| sudo | passwordless via `/etc/sudoers.d/charly-user` (discovered via `getent passwd 1000` → `ubuntu`) | [`/charly-coder:sshd`](/recipes/coder/sshd/) |

## Ubuntu-specific layer quirks (differences vs debian-coder)

- **fastfetch test is skipped** via `exclude_distros: [ubuntu:24.04]` ([`/charly-coder:dev-tools`](/recipes/coder/dev-tools/)). Ubuntu 24.04 noble main does not ship `fastfetch` (Debian 13 trixie does, and so do Fedora/Arch). Skipping is cleaner than failing — see [`/charly-check:check`](/recipes/check/check/) for the `exclude_distros:` field of the declarative-test schema.
- **dotnet-sdk-9.0 source**: uses Microsoft's `dotnet-install.sh` (same as debian-coder). Note: Canonical's noble main/universe ships `dotnet-sdk-8.0` and `dotnet-sdk-10.0` but NOT 9.0; Microsoft's noble apt repo ships 10.0 only. `dotnet-install.sh --channel 9.0` is the only cross-distro-consistent path to .NET 9. See [`/charly-coder:language-runtimes`](/recipes/coder/language-runtimes/).
...
|---|---|---|
| 2222 | sshd-wrapper (SSH as `ubuntu` with sudo) | [`/charly-coder:sshd`](/recipes/coder/sshd/) |
| 18765 | charly-mcp | [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) |

Conflicts with the other three coder-family boxes on these ports.

## Image size

~10.2 GB uncompressed, nearly identical to [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/).

## Cross-distro siblings

- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) — RPM-family, `user:user` create.
- [`/charly-coder:arch-coder`](/recipes/coder/arch-coder/) — pacman-family + AUR, `user:user` create.
- [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/) — deb on Debian 13, `user:user` create.
- [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) — this box; deb on Ubuntu 24.04, `user:ubuntu` **adopt**.

## Related boxes

- [`/charly-distros:ubuntu`](/recipes/distros/ubuntu/) — parent base; declares `base_user:` in the embedded build vocabulary.
- [`/charly-distros:ubuntu-builder`](/recipes/distros/ubuntu-builder/) — multi-stage builder, also runs as `ubuntu:1000`.

## Related layers

- [`/charly-coder:sshd`](/recipes/coder/sshd/) — getent-based sudoers (works regardless of whether the uid-1000 account is `user` or `ubuntu`).
- [`/charly-coder:dev-tools`](/recipes/coder/dev-tools/) — bat→batcat + fastfetch `exclude_distros`.
- [`/charly-coder:language-runtimes`](/recipes/coder/language-runtimes/) — Microsoft `dotnet-install.sh` for .NET 9.
- [`/charly-infrastructure:virtualization`](/recipes/infrastructure/virtualization/) — `package:` + `package_map:` tests.
- [`/charly-distros:container-nesting`](/recipes/distros/container-nesting/), [`/charly-tools:charly`](/recipes/tools/charly/), [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) — shared rootless baseline.

## Related commands

- [`/charly-image:image`](/recipes/image/image/) — **`user_policy:` field** (the pivot for this box's behavior).
- [`/charly-build:build`](/recipes/build/build/) — **`base_user:` declaration** (where `ubuntu:1000` is declared).
- [`/charly-check:check`](/recipes/check/check/) — `exclude_distros:` field reference.
- [`/charly-build:generate`](/recipes/build/generate/) — adopt-vs-create writeBootstrap emission.
- [`/charly-core:shell`](/recipes/core/shell/), [`/charly-core:charly-config`](/recipes/core/charly-config/), [`/charly-core:start`](/recipes/core/start/), [`/charly-core:stop`](/recipes/core/stop/).
