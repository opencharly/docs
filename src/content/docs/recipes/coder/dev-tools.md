---
title: "dev-tools"
description: "Developer tools including bat, ripgrep, neovim, direnv, fd-find, htop, podman-compose, and many more CLI utilities."
---
...
On images whose `ai.opencharly.platform.distro` OCI label includes `ubuntu`, the test runner skips this check with a reason — see [`/charly-check:check`](/recipes/check/check/) "`exclude_distro:` field". fastfetch is omitted from the Ubuntu package set, so the baked test probe would otherwise false-fail on Ubuntu.

### Git tooling lives in [`/charly-coder:gh`](/recipes/coder/gh/)

dev-tools does NOT install `gh`, `git`, or `git-lfs` — those belong
exclusively to [`/charly-coder:gh`](/recipes/coder/gh/), the dedicated GitHub / git tooling candy.
That keeps ownership unambiguous (one candy installs and tests them). If
you want the git tooling, compose [`/charly-coder:gh`](/recipes/coder/gh/) alongside
[`/charly-coder:dev-tools`](/recipes/coder/dev-tools/).
...
## Related Candies
- [`/charly-coder:gh`](/recipes/coder/gh/) — Sibling GitHub CLI commonly paired with dev-tools
- [`/charly-coder:devops-tools`](/recipes/coder/devops-tools/) — Sibling DevOps cloud CLI collection in bootc images
- [`/charly-coder:build-toolchain`](/recipes/coder/build-toolchain/) — Sibling C/C++ toolchain in bootc image stacks

## Related Boxes
- [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/), [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) — canonical consumers of the bat→batcat symlink
- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/), [`/charly-coder:arch-coder`](/recipes/coder/arch-coder/) — consumers where the symlink plan step is a harmless no-op

## Related Commands
- [`/charly-build:build`](/recipes/build/build/) — Build boxes that ship the dev-tools package set
- [`/charly-core:shell`](/recipes/core/shell/) — Interactive shell to use bat/ripgrep/neovim/etc.
- [`/charly-check:check`](/recipes/check/check/) — `exclude_distro:` field reference for the fastfetch-binary test
- [`/charly-image:layer`](/recipes/image/layer/) — authoring reference for distro-tolerant `command:` plan steps and `exclude_distro:`
