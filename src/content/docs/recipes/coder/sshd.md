---
title: "sshd"
description: "OpenSSH server and client on port 22 for remote access."
---
...
[`/charly-check:check`](/recipes/check/check/) "Cross-distro package names (`package_map:`)" for the
...
|---|---|---|
| fedora-coder, arch-coder, debian-coder | `user` (create mode — [`/charly-image:image`](/recipes/image/image/) "user_policy") | `user ALL=(ALL) NOPASSWD: ALL` |
| ubuntu-coder | `ubuntu` (adopt mode — [`/charly-distros:ubuntu`](/recipes/distros/ubuntu/) `base_user:`) | `ubuntu ALL=(ALL) NOPASSWD: ALL` |

Why not `${USER}` substitution? The generator substitutes `${USER}` in plan-step fields (paths, URLs, etc.) but **not** inside `command:` command text — `command:` is passed verbatim to bash, and bash at RUN time doesn't have `$USER` exported. `getent` is the robust, fully-generic alternative. See [`/charly-image:layer`](/recipes/image/layer/) "`${VAR}` substitution scope" and [`/charly-image:image`](/recipes/image/image/) "user_policy" for the full architectural context.
...
- Composed into coder/dev and headless-desktop boxes that need an in-container SSH server (e.g. [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/)), and applied to VM guests at deploy time
...
- `/etc/sudoers.d/charly-user` (the NOPASSWD rule written by this candy) is
  ...
  semantic instead. See [`/charly-check:check`](/recipes/check/check/) Authoring Gotcha #10.
- Host-side port reachability uses `127.0.0.1:${HOST_PORT:2222}`, not
  `${CONTAINER_IP}:${HOST_PORT:2222}`. See [`/charly-check:check`](/recipes/check/check/) Gotcha #1.
...
caught during `charly-arch` bring-up. See [`/charly-check:check`](/recipes/check/check/) Authoring Gotcha #11.

## Related Skills

- [`/charly-distros:cloud-init`](/recipes/distros/cloud-init/) -- depends on sshd for VM provisioning
- [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) -- canonical adopt-mode example; sudoers correctly targets `ubuntu` via getent
- [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/) -- canonical create-mode deb-family example; sudoers targets `user`
- [`/charly-distros:ubuntu`](/recipes/distros/ubuntu/) -- declares the `base_user:` block that makes ubuntu-coder run as `ubuntu`
- [`/charly-check:check`](/recipes/check/check/) -- declarative testing framework (gotchas #10 and #11, `package_map:`, `exclude_distro:`)
- [`/charly-image:image`](/recipes/image/image/) -- `user_policy:` field (create / adopt / auto) that drives which account this candy's sudoers targets
- [`/charly-image:layer`](/recipes/image/layer/) -- candy authoring (`${VAR}` substitution scope, command: vs write:)
