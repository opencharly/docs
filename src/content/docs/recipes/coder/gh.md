---
title: "gh"
description: "GitHub CLI, git, and git-lfs — the single-responsibility home for all git/GitHub tooling."
---
...
candy (including [`/charly-coder:dev-tools`](/recipes/coder/dev-tools/)) installs them. That keeps ownership
...
- [`/charly-coder:dev-tools`](/recipes/coder/dev-tools/) — does not install git/gh/git-lfs (this candy owns them)
- [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) — pairs with gh for SSH/GPG agent access (you usually want both when driving gh from inside a container with the host's GPG keys forwarded)
- [`/charly-distros:github-runner`](/recipes/distros/github-runner/) — self-hosted Actions runner; different candy, different purpose
- [`/charly-coder:github-actions`](/recipes/coder/github-actions/) — installs `act` + `actionlint` for local Actions testing; also different from this candy

## Related Commands

- [`/charly-build:secrets`](/recipes/build/secrets/) — provision `GITHUB_TOKEN` for `gh auth login`
- [`/charly-core:shell`](/recipes/core/shell/) — run gh interactively inside a container
...
- Understanding why [`/charly-coder:dev-tools`](/recipes/coder/dev-tools/) does not install gh
