...
description: "Multi-language runtime meta-layer — Go, PHP, .NET 9 SDK, nodejs-devel, python3-devel, ramalama."
---
...
See [`/charly-image:layer`](/recipes/image/layer/) for general cross-distro plan-step authoring patterns.
...
Consequence for [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) (the biggest consumer): the whole
`python` / `pixi` charly-layer chain stays out of the resolved candy set (because
[`/charly-coder:uv`](/recipes/coder/uv/) and [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) likewise carry no python
...
Go and Node.js testing is delegated to the [`/charly-coder:golang`](/recipes/coder/golang/) and
[`/charly-coder:nodejs`](/recipes/coder/nodejs/) candy skills — those are
...
- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) — kitchen-sink dev box, canonical RPM consumer.
- [`/charly-coder:arch-coder`](/recipes/coder/arch-coder/) — pacman-based sibling.
- [`/charly-coder:debian-coder`](/recipes/coder/debian-coder/), [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) — deb-based siblings, consumers of the dotnet-install.sh plan step.

## Related Candies

- [`/charly-coder:nodejs`](/recipes/coder/nodejs/) — Node.js runtime (direct dependency)
- [`/charly-coder:rust`](/recipes/coder/rust/) — Rust toolchain (direct dependency)
- [`/charly-languages:python`](/recipes/languages/python/) — Pixi-python env. **Not a dep of this candy.**
- [`/charly-coder:golang`](/recipes/coder/golang/) — Go toolchain (may be added separately for clarity even though `golang-bin` is already in this candy's RPM list)
- [`/charly-coder:uv`](/recipes/coder/uv/) — direct-download Rust binary (also carries no pixi-python dep)

## Related Commands

- [`/charly-image:layer`](/recipes/image/layer/) — authoring reference (tag-section cascade, `command:` vs declarative repos)
- [`/charly-build:build`](/recipes/build/build/) — `base_user:` and bootstrap packages
- [`/charly-core:shell`](/recipes/core/shell/) — verify runtimes inside a container
