---
title: "charly-cachyos"
description: "Operator CachyOS workstation profile — a kind:local template plus a host:local deploy that installs the full dev stack (a large candy list) onto a CachyOS host via ShellExecutor."
---
...
**not** `disposable`; see [`/charly-vm:cachyos-bootstrap-vm`](/recipes/vm/cachyos-bootstrap-vm/)) — so `charly-cachyos` is no longer
...
the root's own locals are. See [`/charly-internals:go`](/recipes/internals/go/) "Remote-layer resolver".)
