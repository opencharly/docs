...
`box/arch/charly.yml`. See [`/charly-core:deploy`](/recipes/core/deploy/) "Deploy-into nesting".
...
- [`/charly-local:local-spec`](/recipes/local/local-spec/) — author-facing reference for `kind: local` templates.
- [`/charly-internals:local-infra`](/recipes/internals/local-infra/) — Go file map for the executor/ledger surface.
- [`/charly-core:deploy`](/recipes/core/deploy/) — parent command family.
- [`/charly-image:layer`](/recipes/image/layer/) — `service:` schema rendered as systemd units on local-target deploys.
- [`/charly-vm:vm`](/recipes/vm/vm/) — managed ssh-config fragment writen on `charly vm create`.
- [`/charly-check:check`](/recipes/check/check/) — `--verify` re-runs candy `check:` against the deploy post-install.
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — shared IR consumed by the external `deploy:local` plugin over the executor reverse channel.
