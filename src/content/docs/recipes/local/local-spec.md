---
title: "local-spec"
description: "MUST be invoked before any work involving: authoring `kind: local` templates, `local.yml` files, inline `kind: local` template nodes in `charly.yml`, or the merge semantics between a `kind: local` template and the deploy that deploys it."
---
...
- [`/charly-internals:local-infra`](/recipes/internals/local-infra/) — Go file map (`candy/plugin-fleet/node_resolve.go`'s `lookupLocalTemplate`, the plugin-side template resolver).
