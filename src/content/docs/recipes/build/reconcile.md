---
title: "reconcile"
description: "Use when @github layer/namespace pins drift across repos and the resolver emits \"referenced at multiple versions\" warnings — `charly box reconcile` aligns every pin of a repo to one version (clearing the warnings)."
---
...
Invoked as `charly box reconcile`. See [`/charly-image:image`](/recipes/image/image/) for the family overview.
...
the newest** (see [`/charly-internals:go`](/recipes/internals/go/) "Remote-layer resolver", [`/charly-build:validate`](/recipes/build/validate/)).
...
- Run `--remote` deliberately, at the ONE point in a cutover where adopting the
  producer's newest tag is the intent — [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) B6: after
...
[`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) B6: land + tag the producer FIRST, then
...
- [`/charly-internals:go`](/recipes/internals/go/) "Remote-layer resolver" — the warn-and-newest-wins resolver this aligns to.
- [`/charly-build:validate`](/recipes/build/validate/) — surfaces the multi-version warning reconcile clears.
- [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) — cross-repo (B6) producer→consumer landing that calls reconcile.
- [`/charly-build:migrate`](/recipes/build/migrate/) — per-merge CalVer tags that reconcile pins point at.
- [`/charly-image:image`](/recipes/image/image/) — `import:` / namespace authoring + the family overview.
