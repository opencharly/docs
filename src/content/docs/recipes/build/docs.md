...
The order is forced by the chains above — it is [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) B6's
...
4. **Re-pin the new `docs` merge** in `candy/docs-site/charly.yml`, whose clone is
   ...
   one occurrence — see [`/charly-tools:docs-site`](/recipes/tools/docs-site/), which owns the pin contract and the
...
targets in the **skill files** the generator reads, [`/charly-check:check`](/recipes/check/check/) alone appearing many
...
[`/charly-tools:docs-site`](/recipes/tools/docs-site/) used a made-up two-word reference as its example of an unresolvable
...
- [`/charly-tools:docs-site`](/recipes/tools/docs-site/) — the candy + `check-docs` bed that builds and proves the site.
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — placement, the provider model, the CUE-schema contract.
- [`/charly-internals:skills`](/recipes/internals/skills/) — the skill corpus this site publishes.
