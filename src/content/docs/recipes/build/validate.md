...
- [`/charly-internals:egress`](/recipes/internals/egress/) — the **egress** counterpart: `charly box validate` proves the INPUT config; [`/charly-internals:egress`](/recipes/internals/egress/) validates the config files charly WRITES to a system (cloud-init, kubernetes, units, ssh_config, libvirt XML) before the bytes hit disk.
...
The loader still raises hard load-time errors on obsolete keys — the obsolete `qc` / `cachyos-dx` deployment names, and any obsolete `kind: deployment` doc or root-key `deployment:` (the deploy kind is now the name-first substrate kind: `pod:` / `vm:` / `kubernetes:` / `local:` / `android:` / `group:`). Every such error points at `charly migrate`. See the project rulebook (`AGENTS.md` / `CLAUDE.md`) and [`/charly-build:migrate`](/recipes/build/migrate/).
