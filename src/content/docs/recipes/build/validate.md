---
title: "validate"
description: "MUST be invoked before any work involving: charly box validate command, validation rules, common validation errors, or checking charly.yml and layer definitions."
---
...
Invoked as `charly box validate`. See [`/charly-image:image`](/recipes/image/image/) for the family overview.
...
catalog the legacy `task:` map used). See [`/charly-image:layer`](/recipes/image/layer/) for the full
...
- Collection is reachability-scoped: only layers reachable from the enabled
  images' `base:`/`builder:` chains are fetched. See [`/charly-internals:go`](/recipes/internals/go/)
  "Remote-layer resolver" and [`/charly-build:reconcile`](/recipes/build/reconcile/).
...
`charly box validate` resolves `charly.yml` via `os.Getwd()`. Override with `-C <dir>` / `--dir <dir>` / `CHARLY_PROJECT_DIR=<dir>`. See [`/charly-image:image`](/recipes/image/image/) "Project directory resolution".
...
- [`/charly-build:build`](/recipes/build/build/) -- Building validated images
- [`/charly-build:generate`](/recipes/build/generate/) -- Containerfile generation after validation
- [`/charly-build:inspect`](/recipes/build/inspect/) -- Inspect a specific image after validation
- [`/charly-build:list`](/recipes/build/list/) -- Enumerate images/layers to validate
- [`/charly-build:merge`](/recipes/build/merge/) -- Post-build layer consolidation
- [`/charly-build:new`](/recipes/build/new/) -- Scaffold new candies before validation
- [`/charly-build:pull`](/recipes/build/pull/) -- Pull prebuilt images (orthogonal to validation)

### Related skills

- [`/charly-image:layer`](/recipes/image/layer/) — **Canonical reference** for the `run:`/`check:` step verb catalog, `var:` substitution, YAML anchors, execution order. The validator rules above enforce what's documented there.
- [`/charly-build:generate`](/recipes/build/generate/) — What the generator emits from validated input (per-verb emitters, cache-mount inheritance, inline-content staging).
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — Internal architecture of the run-step emission pipeline.
- [`/charly-internals:egress`](/recipes/internals/egress/) — the **egress** counterpart: `charly box validate` proves the INPUT config; [`/charly-internals:egress`](/recipes/internals/egress/) validates the config files charly WRITES to a system (cloud-init, k8s, units, ssh_config, libvirt XML) before the bytes hit disk.
...
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — the standalone reference for the `mcp:` verb: required modifiers (`tool:` for `call`, `uri:` for `read`), the 7-method allowlist, and the URL-rewrite / port-publishing behavior that authors occasionally hit.
- [`/charly-check:cdp`](/recipes/check/cdp/), [`/charly-check:wl`](/recipes/check/wl/), [`/charly-check:dbus`](/recipes/check/dbus/), [`/charly-check:vnc`](/recipes/check/vnc/) — per-verb references for the other four live-container verbs.
...
The loader still raises hard load-time errors on obsolete keys — the obsolete `qc` / `cachyos-dx` deployment names, and any obsolete `kind: deployment` doc or root-key `deployment:` (the deploy kind is now the name-first substrate kind: `pod:` / `vm:` / `k8s:` / `local:` / `android:` / `group:`). Every such error points at `charly migrate`. See the project rulebook (`AGENTS.md` / `CLAUDE.md`) and [`/charly-build:migrate`](/recipes/build/migrate/).
