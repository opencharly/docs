...
- every project `charly.yml` (splitting into per-kind `vm.yml` / `local.yml` / … siblings is an optional `import:` convenience a project MAY use — never the default; see [`/charly-image:image`](/recipes/image/image/))
...
The payoff of the declarative engine: the common case needs **zero new Go** — but since the schema lives in the sdk module, a schema-version bump is **CROSS-REPO**: the sdk repo lands + tags FIRST, then the superproject consumes it.
...
3. **Land + tag the sdk repo** (its Go-module tag scheme `v0.<YYYYDDD>.<HHMM leading-zeros-stripped>` — see [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/)).
...
- [`/charly-image:layer`](/recipes/image/layer/) — the compact-node-form candy schema migrations produce
- [`/charly-image:image`](/recipes/image/image/) — `image:` entries + `charly box build/validate/inspect`
- [`/charly-core:deploy`](/recipes/core/deploy/) — the deploy entries a migration may rewrite
- [`/charly-local:local-spec`](/recipes/local/local-spec/) — `kind: local` templates
- [`/charly-build:secrets`](/recipes/build/secrets/), [`/charly-build:settings`](/recipes/build/settings/) — the credential schema
- [`/charly-internals:go`](/recipes/internals/go/) — loader internals (`LoadUnified`, `ParseCalVer`, the `gateSchemaVersion` load gate, the `candy/plugin-migrate/engine.go` op-walker)
- [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) — why hard-cutover + a single idempotent `charly migrate` is the required shape
