---
title: "generate"
description: "Containerfile generation from charly.yml and layers."
---
...
Invoked as `charly box generate`. See [`/charly-image:image`](/recipes/image/image/) for the family overview.
...
The pivot is `ResolvedBox.UserAdopted`, set by the `user_policy:` reconciliation in `sdk/buildkit/config_resolve.go:ResolveBox`. See [`/charly-image:image`](/recipes/image/image/) "user_policy" for the policy semantics, [`/charly-build:build`](/recipes/build/build/) "base_user:" for the declaration, and [`/charly-distros:ubuntu`](/recipes/distros/ubuntu/) for the canonical adopt-mode worked example.
...
- Layer dependencies resolve transitively and topologically; circular `require:` is a validation error (surfaced by [`/charly-build:validate`](/recipes/build/validate/)).
...
iteration. See [`/charly-internals:generate-source`](/recipes/internals/generate-source/) "LABEL Placement" for the
rationale and [`/charly-check:check`](/recipes/check/check/) for author-facing workflow implications.
...
- [`/charly-build:build`](/recipes/build/build/) -- Building images (calls generate internally)
- [`/charly-build:inspect`](/recipes/build/inspect/) -- Inspect generated output for a specific image
- [`/charly-build:list`](/recipes/build/list/) -- Enumerate targets before generation
- [`/charly-build:merge`](/recipes/build/merge/) -- Post-build layer consolidation
- [`/charly-build:new`](/recipes/build/new/) -- Scaffold a new candy to generate into
- [`/charly-build:pull`](/recipes/build/pull/) -- Pull prebuilt images (orthogonal to generate)
- [`/charly-build:validate`](/recipes/build/validate/) -- Validation rules for images and layers (including per-verb task rules)

### Related skills

- [`/charly-image:layer`](/recipes/image/layer/) — **Canonical task verb catalog, `var:` substitution, YAML anchors, execution order.** Read this first for authoring questions.
- [`/charly-check:check`](/recipes/check/check/) — test-authoring workflow; `check:` blocks are embedded via `writeJSONLabel` and benefit directly from LABELs-at-end cache efficiency.
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — Deep dive on Containerfile emission internals, `Task` struct, per-verb emitters, `StageInlineContent`, the `spec.ShellQuote` quoting helper and the `BuildStepShellDashC`/`BuildStepShellHeredoc` shell-prefix pair, LABEL-placement rationale.
- [`/charly-internals:go`](/recipes/internals/go/) — Source-code map: `charly/tasks.go` (64 lines, one function: `invokeOpEmitFragmentOpt`), `sdk/deploykit/tasks_emit.go` (the per-verb emitters), `sdk/deploykit` (`WriteCandySteps` + `WriteLabels`, relocated in #67), `charly/layers.go` struct definitions.
- [`/charly-selkies:ffmpeg`](/recipes/selkies/ffmpeg/) — canonical URL-repo consumer (triggers the `dnf5-plugins` prepend).
