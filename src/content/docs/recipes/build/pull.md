---
title: "pull"
description: "Fetch an image from its registry into local container storage so deploy-mode commands can read its OCI labels — `charly box pull`, the ErrImageNotLocal error, fetching by short name / fully-qualified ref / @github.com remote ref."
---
...
service. That's `charly update`'s job (see [`/charly-core:charly-update`](/recipes/core/charly-update/)).
...
`charly box pull` resolves `charly.yml` via `os.Getwd()` when given a short name (to resolve registry + tag). Override with `-C <dir>` / `--dir <dir>` / `CHARLY_PROJECT_DIR=<dir>`. Fully-qualified refs and `@github.com/...` remote refs don't need a project dir. See [`/charly-image:image`](/recipes/image/image/) "Project directory resolution".

## Cross-References

- [`/charly-image:image`](/recipes/image/image/) — family overview; `charly box pull` is one of 8 subcommands.
- [`/charly-build:build`](/recipes/build/build/) — pulls and builds are orthogonal; build creates images,
  pull fetches existing ones.
- [`/charly-core:charly-update`](/recipes/core/charly-update/) — rolls deployed services to a new image version
  (pulls + data-seeds + restarts).
- [`/charly-build:inspect`](/recipes/build/inspect/) — print resolved ref from `charly.yml` without pulling.
- [`/charly-core:shell`](/recipes/core/shell/), [`/charly-core:start`](/recipes/core/start/), [`/charly-core:charly-config`](/recipes/core/charly-config/), [`/charly-automation:alias`](/recipes/automation/alias/), [`/charly-vm:vm`](/recipes/vm/vm/) —
  deploy-mode commands that require a pulled image.
- [`/charly-core:deploy`](/recipes/core/deploy/) — charly.yml overlay semantics applied on top of the
  labels `pull` materializes.
- [`/charly-internals:go`](/recipes/internals/go/) — `ErrImageNotLocal` / `dispatchBuildEnsure` / `ExtractMetadata`
