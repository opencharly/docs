...
All six steps are also callable as MCP tools (`box.new.project`, `box.new.candy`, `candy.add-rpm`, …), so an agent driving `charly mcp serve` can run this entire flow over RPC. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) "Authoring tools" for the worked MCP-only example.
