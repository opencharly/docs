---
title: "new"
description: "Scaffold new candies, boxes, and whole projects with template files — `charly box new {project, box, candy}`, creating projects/boxes/candies, scaffolding directories."
---
...
The `charly box new` family groups three scaffolding verbs. See [`/charly-image:image`](/recipes/image/image/) for the full family overview and [`/charly-image:layer`](/recipes/image/layer/) for the layer-authoring verb catalog it bootstraps.
...
Each verb also auto-becomes an MCP tool (`box.new.project`, `box.new.box`, `box.new.candy`) via the `charly __cli-model` reflection seam (`charly/cli_model_cmd.go`) consumed by the externalized MCP server (`candy/plugin-mcp`) — so an LLM agent driving `charly mcp serve` can scaffold a project from scratch over RPC. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) "Authoring tools".
...
4. `charly candy add-rpm my-svc openssh-server` — populate packages (see [`/charly-image:layer`](/recipes/image/layer/))
...
All the steps are also callable as MCP tools (`box.new.project`, `box.new.candy`, `candy.add-rpm`, …), so an agent driving `charly mcp serve` can run this entire flow over RPC. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) "Authoring tools" for the worked MCP-only example.
...
`charly box new candy <name>` writes into `candy/<name>/` relative to `os.Getwd()`. Override with `-C <dir>` / `--dir <dir>` / `CHARLY_PROJECT_DIR=<dir>`. See [`/charly-image:image`](/recipes/image/image/) "Project directory resolution".
...
- [`/charly-build:build`](/recipes/build/build/) -- Build images containing the new candy
- [`/charly-build:generate`](/recipes/build/generate/) -- Containerfile generation
- [`/charly-build:inspect`](/recipes/build/inspect/) -- Inspect built image including the new candy
- [`/charly-build:list`](/recipes/build/list/) -- Enumerate layers (the new one shows up here)
- [`/charly-build:merge`](/recipes/build/merge/) -- Post-build layer consolidation
- [`/charly-build:pull`](/recipes/build/pull/) -- Pull prebuilt images (unrelated; for consumers of your new candy)
- [`/charly-build:validate`](/recipes/build/validate/) -- Validate candy and box definitions

### Related skills

- [`/charly-image:layer`](/recipes/image/layer/) -- Layer authoring guide, charly.yml format, install files + the `charly candy set / add-rpm / add-deb / add-pac / add-aur` editing surface
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) -- "Authoring tools" table + the MCP-only build-from-scratch worked example
- [`/charly-internals:go`](/recipes/internals/go/) -- Implementation notes: the `yaml.v3` Node API is the reason edits preserve comments; `sdk/kit/scaffold.go` + `sdk/kit/yaml.go` house the create-side logic (`kit.ScaffoldProject` / `kit.AddBox`), `candy/plugin-authoring/authoring_edit.go` the candy-list edits
