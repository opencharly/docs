---
title: "merge"
description: "Post-build layer optimization via merging consecutive small layers."
---
...
Invoked as `charly box merge [<image>]`. See [`/charly-image:image`](/recipes/image/image/) for the family overview.
...
`charly box merge` resolves `charly.yml` via `os.Getwd()`. Override with `-C <dir>` / `--dir <dir>` / `CHARLY_PROJECT_DIR=<dir>`. See [`/charly-image:image`](/recipes/image/image/) "Project directory resolution".
...
- [`/charly-build:build`](/recipes/build/build/) -- Building images (merge runs inline after each build level)
- [`/charly-build:generate`](/recipes/build/generate/) -- Containerfile generation
- [`/charly-build:inspect`](/recipes/build/inspect/) -- Inspect merged images
- [`/charly-build:list`](/recipes/build/list/) -- Enumerate images before merging
...
- [`/charly-build:validate`](/recipes/build/validate/) -- Validate before merging

### Related skills

- [`/charly-image:layer`](/recipes/image/layer/) -- Layer authoring (layer size affects merge behavior)
