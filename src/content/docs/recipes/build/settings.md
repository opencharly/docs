---
title: "settings"
description: "Runtime configuration management for the charly CLI — `charly settings`, engine selection, bind address, storage paths, secret backend configuration."
---
...
| `hosts.<alias>` | *(none)* | — | SSH target for `charly --host <alias>` remote execution. Free-form: `host`, `user@host`, `user@host:port`. Consulted by the top-level `--host` flag to re-exec `charly` commands on another machine over SSH. See [`/charly-core:ssh`](/recipes/core/ssh/). |
...
- [`/charly-core:charly-config`](/recipes/core/charly-config/) -- deployment configuration (uses settings)
- [`/charly-build:secrets`](/recipes/build/secrets/) -- credential management
- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) -- diagnose settings and secret storage health
- [`/charly-automation:enc`](/recipes/automation/enc/) -- encrypted volume paths
