---
title: "secrets"
description: "MUST be invoked before any work involving: charly secrets commands, Secret Service / config-file credential management, GPG-encrypted .secrets files, credential import/export, or secret administration."
---
...
findable. Set `keyring_collection_label` via [`/charly-build:settings`](/recipes/build/settings/) to pin charly to a
...
subscription (zero CPU) until the user unlocks the keyring. See [`/charly-automation:enc`](/recipes/automation/enc/)
...
`charly.yml` or the generated quadlet as plaintext**. See [`/charly-image:layer`](/recipes/image/layer/)
...
rotation semantics differ. See [`/charly-image:layer`](/recipes/image/layer/) (secret_accept / secret_require).
...
For GPG agent forwarding into containers (so `gpg --decrypt` works inside), use the `agent-forwarding` layer. See [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) for details. The container's GPG uses the host's agent via a forwarded socket — no GPG agent runs inside the container.

## Cross-References

- [`/charly-core:charly-config`](/recipes/core/charly-config/) — `secret_backend` and other settings keys
- [`/charly-automation:enc`](/recipes/automation/enc/) — encrypted volume credential lookup, iteration-capable ssClient, broken-collection troubleshooting, source classification (`env`/`keyring`/`config`/`locked`/`unavailable`/`default`)
- [`/charly-build:settings`](/recipes/build/settings/) — `keyring_collection_label`, `secret_backend`, and other runtime config keys
- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) — Secret Service collection health + shadow index consistency checks
- [`/charly-core:service`](/recipes/core/service/) — container secrets (`secrets` field in charly.yml, provisioned at `charly config`)
- [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) — SSH/GPG agent forwarding into containers
- [`/charly-infrastructure:gnupg`](/recipes/infrastructure/gnupg/) — GnuPG package layer
- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) — two-tier secrets pattern: podman secrets (`WEBUI_SECRET_KEY` auto-generated) + GPG `.secrets` (API keys)
- [`/charly-coder:direnv`](/recipes/coder/direnv/) — direnv environment loader
