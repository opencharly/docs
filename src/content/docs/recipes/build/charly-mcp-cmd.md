---
title: "charly-mcp-cmd"
description: "MUST be invoked for either Model Context Protocol direction: the declarative out-of-process `mcp:` check verb that probes advertised servers, or `charly mcp serve`, which exposes the reflected Charly CLI over Streamable HTTP or stdio."
---
...
Source: the endpoint resolution lives in `candy/plugin-mcp` (`resolve.go` — the `cc.ResolveImageLabel` + `cc.ResolveEndpoint` reverse-legs), alongside the MCP client (the 7 methods + the dial: `provider.go` dispatch + `methods.go` client layer). See [`/charly-internals:plugin`](/recipes/internals/plugin/) for the out-of-process provider model and [`/charly-internals:go`](/recipes/internals/go/) for the host-side map.
...
See [`/charly-image:image`](/recipes/image/image/) "Project directory resolution" for the flag/env semantics; the implementation is `computeProjectPrefix` + `childCharlyEnv` in `candy/plugin-mcp/serve.go`.
...
- [`/charly-image:layer`](/recipes/image/layer/) — `mcp_provide` / `mcp_accept` / `mcp_require` field reference for layer authoring.
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — how `mcp_provide` gets injected into `charly.yml` `provides.mcp:` and synthesized into `CHARLY_MCP_SERVERS` for consumers at `charly config` time; pod-aware resolution to `localhost`; instance-aware MCP server naming with `-<instance>` suffix.
- [`/charly-build:validate`](/recipes/build/validate/) — authoring-time validation rules (method allowlist, required modifiers, scope enforcement).
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly.yml` `port:` override semantics (the port-publishing gotcha lives here operationally).
- [`/charly-check:cdp`](/recipes/check/cdp/) — sibling live-container verb (Chrome DevTools Protocol).
- [`/charly-check:wl`](/recipes/check/wl/) — sibling (Wayland desktop control).
- [`/charly-check:dbus`](/recipes/check/dbus/) — sibling (D-Bus calls/notifications).
- [`/charly-check:vnc`](/recipes/check/vnc/) — sibling (VNC framebuffer / input).
- [`/charly-jupyter:jupyter-mcp`](/recipes/jupyter/jupyter-mcp/) — the FastMCP server implementation layer (11 tools for notebook manipulation over CRDT: notebook_*/cell_* + notebook_list_users + room_list; clients do not manage CRDT rooms — the server auto-attaches).
- [`/charly-selkies:chrome-devtools-mcp`](/recipes/selkies/chrome-devtools-mcp/) — the mcp-proxy wrapper around chrome-devtools-mcp (29 tools for browser automation).
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — a consumer (`mcp_accept: jupyter, chrome-devtools`); use the `mcp:` check verb (`charly check live <consumer> --filter mcp`) to verify the services hermes discovers are actually alive.
- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) — another consumer (`mcp_accept: jupyter, chrome-devtools`).
- [`/charly-jupyter:jupyter`](/recipes/jupyter/jupyter/), [`/charly-jupyter:jupyter-ml`](/recipes/jupyter/jupyter-ml/), [`/charly-jupyter:jupyter-ml-notebook`](/recipes/jupyter/jupyter-ml-notebook/) — images bundling `jupyter-mcp`; `charly check live <image> --filter mcp` exercises the verb end-to-end.
- [`/charly-selkies:sway-browser-vnc`](/recipes/selkies/sway-browser-vnc/), [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/), [`/charly-selkies:selkies-labwc-nvidia`](/recipes/selkies/selkies-labwc-nvidia/) — images bundling `chrome-devtools-mcp` (transitively via the chrome metalayer).
- [`/charly-internals:go`](/recipes/internals/go/) — host-side implementation map: `check_endpoint_resolve.go` (the host-endpoint reverse-legs — `resolveImageLabel` + `resolveVerbEndpoint` the mcp plugin pulls), `cli_model_cmd.go` (the `charly __cli-model` seam the server consumes; the server itself is `candy/plugin-mcp/serve.go`), `candy/plugin-box/validate_check.go` (op-level deploy-scope enforcement; the `mcp` method-name + required-modifier checks live in `candy/plugin-mcp` + the CUE `#Op` enum). The MCP CLIENT (the 7 methods + the go-sdk dial) lives out-of-process in `candy/plugin-mcp` — see [`/charly-internals:plugin`](/recipes/internals/plugin/).
- [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) — the deployment layer that wires `charly mcp serve` into an image via supervisord. Includes the `/workspace` bind-mount (volume NAME `project`) + `CHARLY_PROJECT_DIR` env var pattern for build-mode tools.
- [`/charly-tools:charly`](/recipes/tools/charly/) — the underlying binary layer; required by `charly-mcp`.
- [`/charly-image:image`](/recipes/image/image/) — "Project directory resolution" subsection documents the `-C` / `--dir` / `CHARLY_PROJECT_DIR` global flag that makes the server's project-dir bind-mount work.
- [`/charly-coder:charly-arch`](/recipes/coder/charly-arch/), [`/charly-distros:charly-fedora`](/recipes/distros/charly-fedora/) — canonical images composing `charly-mcp` for remote charly-via-MCP deployments.
