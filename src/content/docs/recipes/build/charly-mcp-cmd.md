...
Source: the endpoint resolution lives in `candy/plugin-mcp` (`resolve.go` — the `cc.ResolveImageLabel` + `cc.ResolveEndpoint` reverse-legs), alongside the MCP client (the MCP methods + the dial: `provider.go` dispatch + `methods.go` client layer). See [`/charly-internals:plugin`](/recipes/internals/plugin/) for the out-of-process provider model and [`/charly-internals:go`](/recipes/internals/go/) for the host-side map.
