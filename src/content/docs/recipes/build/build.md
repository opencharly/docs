...
**Mode purity**: `charly box build` reads `charly.yml` only. `charly.yml` is never read during build — this is enforced by `LoadConfig` in `charly/config.go`, which calls `LoadConfigRaw` (no `MergeDeployOverlay`) to guarantee OCI labels are baked strictly from authored configuration, never from local deploy-time overrides. See [`/charly-internals:go`](/recipes/internals/go/) "Mode purity" for the architectural invariant this protects and the bug it prevents.
...
Consumed by the `user_policy:` reconciliation in `charly/config.go:ResolveBox` — see [`/charly-image:image`](/recipes/image/image/) "user_policy" for the three-value policy (`auto` / `adopt` / `create`) and the decision matrix.
