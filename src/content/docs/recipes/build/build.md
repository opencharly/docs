---
title: "build"
description: "MUST be invoked before any work involving: building container images, charly box build command, pushing to registries, merging layers, build caches, or Containerfile generation."
---
...
If you find yourself rebuilding `charly` to chase a bug and the symptom persists, the binary path on $PATH may not be the one you just refreshed — confirm with `which charly`, and remember a worktree's own `./bin/charly` is a THIRD candidate the freshness guard tracks separately (see [`/charly-internals:agents`](/recipes/internals/agents/) "The charly binary in a multi-teammate / multi-worktree setup").
