---
title: "selkies"
description: "Browser-accessible desktop streaming via WebSocket using pixelflux and pcmflux."
---
...
This fix was rolled out via `charly update -i INSTANCE` across all live selkies-desktop instances — see [`/charly-core:charly-update`](/recipes/core/charly-update/) for the per-instance update pattern. The rollback recipe there also applies if a pixelflux patch regresses.

### Rebuild cadence

Pixelflux is compiled **from source** in the selkies build stage (`candy/selkies/build.sh`), not installed from PyPI, because these fixes live on a fork not yet merged upstream. The build stage clones from a pinned commit, applies four inline source patches, and runs `pip install .` against the box's pixi builder (`arch-builder` on the cachyos base, `cuda-arch-builder` on the GPU build). See [`/charly-distros:arch-builder`](/recipes/distros/arch-builder/) and [`/charly-coder:build-toolchain`](/recipes/coder/build-toolchain/) (5 package categories — smithay backend headers, codec devel, bindgen runtime, rust, generic C/C++) for the builder-stage dependency story.

### Related fixes

- [`/charly-selkies:wl-screenshot-pixelflux`](/recipes/selkies/wl-screenshot-pixelflux/) — Reuses the singleton capture path for screenshots
- [`/charly-selkies:wl-record-pixelflux`](/recipes/selkies/wl-record-pixelflux/) — Reuses the singleton capture path for recording
- [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) — Build Pipeline Note explaining the patched pixelflux compilation
...
| `DRINODE` | Auto-detected | GPU render node — injected at runtime by `charly config` from first `/dev/dri/renderD*` device. See [`/charly-core:charly-config`](/recipes/core/charly-config/) |
...
See [`/charly-selkies:labwc`](/recipes/selkies/labwc/) for the full list of XKB variables (LAYOUT, VARIANT, MODEL, OPTIONS).
...
- [`/charly-selkies:labwc`](/recipes/selkies/labwc/) — Nested compositor that selkies hosts inside `wayland-1` (autostart Chrome-duplication race + keyboard layout)
- [`/charly-selkies:chrome`](/recipes/selkies/chrome/) — Chrome browser (supervised by the `[program:chrome]` service in `selkies-core`) + cgroup resource caps
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) — Service ordering, event listeners, crash-loop escalation
- [`/charly-selkies:wl-screenshot-pixelflux`](/recipes/selkies/wl-screenshot-pixelflux/) — Screenshot path via the shared ScreenCapture singleton
- [`/charly-selkies:wl-record-pixelflux`](/recipes/selkies/wl-record-pixelflux/) — Recording path via the shared ScreenCapture singleton
- [`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) — Metalayer composing selkies with labwc, Chrome, waybar, desktop tools
- [`/charly-distros:nvidia`](/recipes/distros/nvidia/), [`/charly-distros:rocm`](/recipes/distros/rocm/) — GPU runtime candies feeding DRINODE into the selkies VAAPI encoder
- [`/charly-selkies:ffmpeg`](/recipes/selkies/ffmpeg/) — Codec dep used by the capture bridge for H.264→PNG decode
- [`/charly-coder:build-toolchain`](/recipes/coder/build-toolchain/) — Builder-stage packages (smithay, bindgen, codec devel) that compile the patched pixelflux
- [`/charly-distros:rpmfusion`](/recipes/distros/rpmfusion/) — Applied before build-toolchain so codec devel libs (libva-devel, x264-devel, ffmpeg-devel) install

## Related Boxes

- [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/), [`/charly-selkies:selkies-labwc-nvidia`](/recipes/selkies/selkies-labwc-nvidia/) — Boxes that bundle this candy
- [`/charly-distros:arch-builder`](/recipes/distros/arch-builder/) — Builder image for the pixelflux from-source compilation (`cuda-arch-builder` on the GPU build)

## Related Commands

- [`/charly-check:wl`](/recipes/check/wl/) — Wayland automation (screenshot via capture bridge, input, windows)
- [`/charly-check:cdp`](/recipes/check/cdp/) — Chrome DevTools and SPA bridge (click, type, key-combo through remote desktop)
- [`/charly-check:record`](/recipes/check/record/) — Desktop video recording via capture bridge
- [`/charly-core:charly-update`](/recipes/core/charly-update/) — Per-instance update pattern used to roll out pixelflux memory fixes
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — DRINODE auto-injection, resource caps, NO_PROXY auto-enrichment, keyboard layout XKB env
- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) — Host GPU probe feeding `appendAutoDetectedEnv()` (DRINODE, HSA_OVERRIDE_GFX_VERSION)
