...
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — the auto-included `chrome-devtools-mcp` sub-candy exposes 29 tools via Streamable HTTP on port 9224; run the declarative 2-check suite via `charly check live <image> --filter mcp`
