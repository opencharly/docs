---
title: "selkies-desktop-layer"
description: "Metalayer composing a full Selkies Wayland streaming desktop with Chrome, Waybar, desktop automation tools, and accessibility introspection."
---
...
- **Configurable keyboard layout** via `XKB_DEFAULT_LAYOUT` — German (de), French (fr), Nordic (no), etc. AltGr characters (@, €, \\, ~) work via direct scancode injection. See [`/charly-selkies:labwc`](/recipes/selkies/labwc/)
...
See [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) for full multi-instance deployment examples.

## Related Skills

- [`/charly-selkies:selkies`](/recipes/selkies/selkies/) — Streaming engine, Pixelflux Memory Management, ScreenCapture singleton, DRINODE auto-detection, keyboard layout support
- [`/charly-selkies:labwc`](/recipes/selkies/labwc/) — Nested Wayland compositor (its autostart no longer launches Chrome — selkies-core supervises it)
- [`/charly-selkies:selkies-core`](/recipes/selkies/selkies-core/) — owns the supervised `[program:chrome]` service shared by both selkies flavors
- [`/charly-selkies:chrome`](/recipes/selkies/chrome/) — Chrome browser with CDP proxy, HTTP proxy support, cgroup resource caps
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) — supervisord process model (the selkies `[program:chrome]` uses `restart: always`)
- [`/charly-selkies:wl-record-pixelflux`](/recipes/selkies/wl-record-pixelflux/) — Desktop video recording via the shared capture singleton
- [`/charly-selkies:wl-screenshot-pixelflux`](/recipes/selkies/wl-screenshot-pixelflux/) — Screenshots via the shared capture singleton
- [`/charly-distros:arch-builder`](/recipes/distros/arch-builder/) — Builder image that compiles patched pixelflux from source on the cachyos base (`cuda-arch-builder` on the GPU build)
- [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) — CPU labwc box that bundles this metalayer (with [`/charly-selkies:selkies-labwc-nvidia`](/recipes/selkies/selkies-labwc-nvidia/) for the GPU build)
- [`/charly-check:wl`](/recipes/check/wl/) — Wayland automation (screenshots, input, windows)
...
- [`/charly-core:charly-update`](/recipes/core/charly-update/) — Per-instance update pattern used to roll out pixelflux/Chrome fixes
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — Multi-instance deployment, resource caps, tunnel, proxy env vars, NO_PROXY auto-enrichment
- [`/charly-core:deploy`](/recipes/core/deploy/) — Tunnel configuration (charly.yml-only, instance inheritance gap)
