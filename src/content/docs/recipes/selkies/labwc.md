---
title: "labwc"
description: "Lightweight Wayland compositor (wlroots-based) for nested desktop inside pixelflux."
---
...
The compositor and selkies input handler both read `XKB_DEFAULT_LAYOUT` from the environment, ensuring the scancode map matches the compositor's layout. See [`/charly-selkies:selkies`](/recipes/selkies/selkies/) for the keyboard input pipeline details.
...
[`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) and [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) for the
supervised-service pattern, and [`/charly-selkies:chrome`](/recipes/selkies/chrome/) for the resource caps.
...
- [`/charly-selkies:selkies`](/recipes/selkies/selkies/) — pixelflux streaming engine (provides `wayland-1` that labwc connects to)
- [`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) — desktop metalayer that composes labwc + chrome + waybar + selkies
- [`/charly-selkies:waybar-labwc`](/recipes/selkies/waybar-labwc/) — status bar configured for labwc
- [`/charly-selkies:chrome`](/recipes/selkies/chrome/) — Chrome browser (supervised by the selkies-core `[program:chrome]` service)
