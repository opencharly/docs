---
title: "a11y-tools"
description: "AT-SPI2 accessibility introspection — Python AT-SPI2 bindings to query the accessibility tree of GTK, Qt, and Chrome apps for element-based automation (find by name/role, not pixel coordinates), backing the `wl: atspi` verb (tree/find/click actions)."
---
...
- `python3-pyatspi` installs under the **system** `/usr/lib/python3.X/
  ...
  See [`/charly-check:check`](/recipes/check/check/) Authoring Gotchas #7 (no bash defaults) and #8
  (system vs pixi Python).

## Related Skills

- [`/charly-check:wl`](/recipes/check/wl/) — the `wl: atspi` verb (tree/find/click)
- [`/charly-infrastructure:dbus-layer`](/recipes/infrastructure/dbus-layer/) — Required dependency (D-Bus session bus)
