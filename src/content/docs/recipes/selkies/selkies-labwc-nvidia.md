...
description: "GPU-accelerated Selkies streaming desktop (labwc flavor) on the CachyOS NVIDIA base, with real NVENC."
---
...
The NVIDIA-GPU build of the **labwc flavor** ([`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/)) — the
...
CPU sibling is [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) (now in this same `opencharly/distro-cachyos`
...
[`/charly-distros:cachyos`](/recipes/distros/cachyos/), [`/charly-distros:nvidia`](/recipes/distros/nvidia/), [`/charly-distros:cuda`](/recipes/distros/cuda/).
...
## Key Candies
- [`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) — full Selkies labwc streaming desktop metalayer
- [`/charly-distros:nvidia`](/recipes/distros/nvidia/) — GPU runtime and CDI device auto-detection
- [`/charly-distros:cuda`](/recipes/distros/cuda/) — CUDA toolkit and libraries (via the CachyOS GPU base)
- [`/charly-infrastructure:dbus-layer`](/recipes/infrastructure/dbus-layer/) — session bus for desktop services
...
## Related Boxes
- [`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) — the labwc desktop metalayer this box is the GPU build of
...
- [`/charly-distros:cachyos`](/recipes/distros/cachyos/) — the CachyOS base image family (owns this box's submodule)

## Related Commands
- [`/charly-check:check`](/recipes/check/check/) — parent router for the out-of-process live-container check verbs (`wl:`/`cdp:`/`vnc:`/`mcp:`/`dbus:`)
- [`/charly-check:cdp`](/recipes/check/cdp/) — drive Chrome inside the streaming desktop
- [`/charly-check:wl`](/recipes/check/wl/) — interact with the labwc Wayland session
- [`/charly-check:dbus`](/recipes/check/dbus/) — the `dbus:` check verb (D-Bus notifications, served out-of-process by `candy/plugin-dbus`)
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — inherits chrome-devtools-mcp's 2 deploy-scope `mcp:` checks; use `charly check live selkies-labwc-nvidia --filter mcp` to verify the MCP server is alive and exposing the full tool catalog.
