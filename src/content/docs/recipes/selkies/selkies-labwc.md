---
title: "selkies-labwc"
description: "The labwc flavor of the Selkies streaming desktop (cpu/default GPU build) — a browser-accessible Wayland desktop streamed via pixelflux WebSocket on the CachyOS base."
---
...
Tunnel config is in `charly.yml`: `tunnel: {provider: tailscale, private: all}`. See [`/charly-core:deploy`](/recipes/core/deploy/).
...
All level 0/1 characters (ö, ä, ü, ß, =, ?) and AltGr characters (@, €, \\, ~) work for any layout. See [`/charly-selkies:labwc`](/recipes/selkies/labwc/) for details and [`/charly-selkies:selkies`](/recipes/selkies/selkies/) for the input pipeline.
...
**Recommended:** Use the `cdp: spa-*` steps for keyboard interaction — they bypass the local compositor and Chrome shortcut handlers (see [`/charly-check:check`](/recipes/check/check/) for the precise SPA-method modifier shape):
...
See [`/charly-core:charly-config`](/recipes/core/charly-config/) for `--update-all` propagation, [`/charly-selkies:chrome`](/recipes/selkies/chrome/) for `env_accept` (HTTP_PROXY/HTTPS_PROXY/NO_PROXY).
...
[`/charly-selkies:selkies`](/recipes/selkies/selkies/) (Patched pixelflux build pipeline) for the full pipeline and the
diagnostic recipe that found the leak.

## Related Boxes

- [`/charly-selkies:selkies-labwc-nvidia`](/recipes/selkies/selkies-labwc-nvidia/) — the GPU sibling of this CPU box: the same `selkies-desktop` metalayer on the CachyOS GPU base (`cachyos.nvidia`, `build: [pac, aur]`) with `builder.pixi: arch.cuda-arch-builder` for real NVENC, in the `opencharly/distro-cachyos` submodule. See [`/charly-distros:cachyos`](/recipes/distros/cachyos/).
- [`/charly-openclaw:openclaw-desktop`](/recipes/openclaw/openclaw-desktop/) — all-in-one CachyOS variant: this streaming desktop fused with the openclaw-full gateway + AI CLIs, a CPU ollama, and the full charly toolchain (build boxes, run nested pods, launch rootless libvirt VMs from inside the streaming desktop). Uses [`/charly-distros:container-nesting`](/recipes/distros/container-nesting/)'s `unmask=/proc/*` posture — no `--privileged`, still uid 1000.
- [`/charly-selkies:sway-browser-vnc`](/recipes/selkies/sway-browser-vnc/) — VNC-based alternative using Sway compositor instead of Selkies/labwc streaming
...
for `/etc/sudoers.d/charly-user` because it's root-only ([`/charly-check:check`](/recipes/check/check/) Gotcha #10).

## Related Skills

- [`/charly-selkies:selkies-desktop-layer`](/recipes/selkies/selkies-desktop-layer/) (metalayer), [`/charly-selkies:selkies`](/recipes/selkies/selkies/),
  [`/charly-selkies:chrome`](/recipes/selkies/chrome/), [`/charly-selkies:labwc`](/recipes/selkies/labwc/), [`/charly-coder:sshd`](/recipes/coder/sshd/),
  [`/charly-selkies:chrome-devtools-mcp`](/recipes/selkies/chrome-devtools-mcp/), [`/charly-selkies:pipewire`](/recipes/selkies/pipewire/)
- [`/charly-check:check`](/recipes/check/check/) — declarative testing framework + testing gotchas
- [`/charly-check:cdp`](/recipes/check/cdp/), [`/charly-check:wl`](/recipes/check/wl/) — desktop automation on this box
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — deploy setup (tunnel, port remapping, instances)
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — the box bundles `chrome-devtools-mcp` (transitively via the chrome metalayer), so 2 deploy-scope `mcp:` checks (`ping`, `list-tools`) run against its MCP server on port 9224. `charly check live selkies-labwc --filter mcp` runs them, enumerating the 29 chrome-devtools tools.
