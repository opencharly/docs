---
title: "plugin — cue-schema-contract"
...
cross-generator pipeline map + the generation-coverage current state live in [`/charly-internals:go`](/recipes/internals/go/)
...
full per-verb render pipeline with COPY staging + op coalescing). Authoring + IR mechanism: [`/charly-internals:install-plan`](/recipes/internals/install-plan/) (the
