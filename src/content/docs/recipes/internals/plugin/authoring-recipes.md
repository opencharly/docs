---
title: "plugin — authoring-recipes"
description: "Detail page of the plugin recipe card."
---

<!-- DO-NOT-EDIT — generated by `charly docs generate`; run `task docs:sync` to regenerate. -->

*Detail page of the [plugin](/recipes/internals/plugin/) recipe card.*
...
plugin-build over the envelope + the `render-seam` reverse legs); the host keeps ONLY the thin `buildengine-*` shards + `render-seam` (the loader + resolve + envelope projection + drive-model moved plugin-side — K3 build-engine migration, the host render-leg is DELETED). See [`/charly-build:build`](/recipes/build/build/) +
[`/charly-build:generate`](/recipes/build/generate/).
...
`candy/plugin-example-kind`; loader mechanism: [`/charly-internals:go`](/recipes/internals/go/) (`plugin_prescan.go`).
...
`op.Params`, attaches host-threaded members); the host fold is `runPluginKind` ([`/charly-internals:go`](/recipes/internals/go/)); the
...
`marker: INVALID`); dispatch is `runPluginKind` ([`/charly-internals:go`](/recipes/internals/go/)).
...
- **Plugin↔plugin + host-build (F10).** A plugin running WITH a reverse channel (deploy/step/check/build —
  ...
  Reference: `candy/plugin-example-dispatch`; mechanism: [`/charly-internals:install-plan`](/recipes/internals/install-plan/)
...
- **Deploy time.** An external deploy-target provider runs its full Add/Test/Update/Del lifecycle over the
  ...
  (out-of-process-only): `candy/plugin-example-lifecycle`; mechanism: [`/charly-internals:install-plan`](/recipes/internals/install-plan/)
  ...
  (`OpExecute`), placement-agnostic. Detail → [`/charly-internals:install-plan`](/recipes/internals/install-plan/) (the `pluginDeployTarget`
...
- **Build time.** `charly box build` / `charly box generate` connect the project's external plugin candies during
  ...
  plugins run as host code during its image builds. Detail → [`/charly-build:generate`](/recipes/build/generate/) + [`/charly-internals:generate-source`](/recipes/internals/generate-source/).
