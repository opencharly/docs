---
title: "git-workflow — evidence-and-freshness"
...
can merge. See [`/charly-internals:skills`](/recipes/internals/skills/) for the source→projection model and
[`/charly-build:docs`](/recipes/build/docs/) for the site generator.
...
- **The head moves.** Resolve a HEAD with `git ls-remote`, not the API — that read has
  ...
  [`references/multi-repo-coordination.md`](/recipes/internals/git-workflow/multi-repo-coordination/); it is repeated here because the failure it
...
[`references/branch-and-pr-loop.md`](/recipes/internals/git-workflow/branch-and-pr-loop/).** That check works only when you already suspect the
