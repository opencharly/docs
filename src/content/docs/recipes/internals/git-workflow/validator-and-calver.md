---
title: "git-workflow — validator-and-calver"
...
not "fix" a denial by editing the rule until you have confirmed the agent's project root. See [`/charly-internals:agents`](/recipes/internals/agents/) "Sub-agent operational invariants"
...
- **Working-tree cleanliness.** After the merge, `git status` is clean in every
  ...
  [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) "Cutover sizing — the batch law").
- **Report format.** The final message states: what was committed (commit subject +
  ...
  the project rulebook "AI Attribution", keyed to the change class ([`/charly-check:check`](/recipes/check/check/) "R10
...
1. Run [`/charly-internals:root-cause-analyzer`](/recipes/internals/root-cause-analyzer/) before attempting any fix — blind
   retry is forbidden.
