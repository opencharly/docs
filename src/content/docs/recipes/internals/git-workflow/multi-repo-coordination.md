---
title: "git-workflow — multi-repo-coordination"
...
- **No shared freeze barrier — per-worktree self-freeze instead.** Because each
  ...
  discipline: [`/charly-internals:agents`](/recipes/internals/agents/) "The charly binary in a multi-teammate /
...
3. **Repoint the consumer:** `charly box reconcile` rewrites B's `@github.../A:...`
   pins to `v<CalVer_A>` (see [`/charly-build:reconcile`](/recipes/build/reconcile/)).
...
lives in [`references/validator-and-calver.md`](/recipes/internals/git-workflow/validator-and-calver/), not in this file), never from a
