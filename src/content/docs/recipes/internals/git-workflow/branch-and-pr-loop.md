---
title: "git-workflow — branch-and-pr-loop"
...
- **Implementation** — one git worktree per cutover, per-worktree binaries for
  verification ([`/charly-internals:agents`](/recipes/internals/agents/) "Per-worktree binaries", proven), bed
...
when written, false when read ([`references/evidence-and-freshness.md`](/recipes/internals/git-workflow/evidence-and-freshness/), "Three
...
[`references/evidence-and-freshness.md`](/recipes/internals/git-workflow/evidence-and-freshness/), "Submodule pointers can be reverted by a
...
- **Sync-before-start.** `git fetch origin --prune --tags`; ff local `main` to
  ...
  fast-forward, STOP + run [`/charly-internals:root-cause-analyzer`](/recipes/internals/root-cause-analyzer/). (Local `main`
...
- **Worktree hygiene — the per-session lifecycle (create → work → land →
  ...
  worktree can hold (B7). A session's own lifecycle:
    ...
    3. **Remove after landing** — once `gh pr view <n> --json state` confirms
       ...
       orchestrator-managed teammate; [`/charly-internals:agents`](/recipes/internals/agents/) "Worktree
       lifecycle").
