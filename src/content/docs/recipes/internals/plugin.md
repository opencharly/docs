---
title: "plugin"
description: "Use when authoring or modifying a charly PLUGIN — a candy with a `plugin:` block that contributes Providers (verbs/kinds/deploy-targets/steps/builders/commands), its own CUE schema, builtin (compiled-in) or external (out-of-tree git repo)."
---
...
- [authoring-recipes](/recipes/internals/plugin/authoring-recipes/)
- [boundary-law](/recipes/internals/plugin/boundary-law/)
- [cue-schema-contract](/recipes/internals/plugin/cue-schema-contract/)
...
applies ([`/charly-image:layer`](/recipes/image/layer/)), including the mandatory `version:`/`description:`/`plan:`+`check:` (ADE).

## A plugin's declarations are a PUBLISHED surface

`charly docs generate` (see [`/charly-build:docs`](/recipes/build/docs/)) renders one **opencharly.ai** page per plugin
...
| Topic | File |
|---|---|
| The kernel/plugin boundary law (E/M/B/D/R, the decision procedure, the incomplete-seam self-test, the host-boundary-object trap, the resolve-to-envelope canonical shape) — sole owner of this doctrine — plus the v2 target architecture end-state | [`references/boundary-law.md`](/recipes/internals/plugin/boundary-law/) |
| The unified Provider model (transport-invisible dispatch, the `build` class, lifecycle phases, flat vs structural kind decode), placement (builtin/external × in-proc/out-of-process at build and deploy time), and the four authoring recipes (external, compiled-in, host-coupled kit, external command) | [`references/authoring-recipes.md`](/recipes/internals/plugin/authoring-recipes/) |
| The per-plugin CUE schema contract (dev-time Go params, runtime schema-over-Describe RPC), the load gate + validator, and why plugin schemas are self-contained | [`references/cue-schema-contract.md`](/recipes/internals/plugin/cue-schema-contract/) |
...
- [`/charly-internals:go`](/recipes/internals/go/) — the provider registry + the reserved-word spine (verbs/kinds/deploy/steps/builders/commands).
- [`/charly-image:layer`](/recipes/image/layer/) — the candy authoring surface the `plugin:` block extends.
- [`/charly-check:check`](/recipes/check/check/) — the plugin-verb check steps (`<word>: <input>` sugar) + ADE (a plugin's own acceptance plan).
- [`/charly-build:validate`](/recipes/build/validate/) — `charly box validate` rules.
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — the `pluginDeployTarget` deploy lifecycle (`OpExecute` reverse channel, ledger record) + the `OpEmit` build-time fragment; the deploy wire types CUE-sourced at `spec/schema/deploy.cue` / `spec/schema/buildwire.cue` / `spec/schema/seam.cue`.
- [`/charly-build:generate`](/recipes/build/generate/) + [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — the build-time plugin connect seam + the `emitTasks` placement-agnostic plugin-verb dispatch (`OpEmit` → fragment).
