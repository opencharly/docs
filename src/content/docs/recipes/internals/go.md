...
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — Understanding generated Containerfiles + deep dive on the task emission pipeline (`charly/tasks.go`).
- [`/charly-image:layer`](/recipes/image/layer/) — **Canonical author-facing reference** for the task verb catalog that `charly/tasks.go` implements.
