---
title: "egress"
description: "CUE EGRESS validation — validating (and where valuable, generating) the config files charly WRITES to a system before the bytes hit disk."
---
...
- **Ingress** (`charly/cue_schema.go`): validates the INPUT config a user authors
  ...
  `params` package is generated in ITS repo via the same pipeline), kept honest by the reproducibility + parity tests. Owned by [`/charly-build:validate`](/recipes/build/validate/); the schema-change codegen
