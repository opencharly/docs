---
title: "egress"
description: "CUE EGRESS validation — validating (and, where it adds value, generating) the config files charly WRITES to a system BEFORE the bytes hit disk."
---
...
- **Ingress** (`charly/cue_schema.go`): validates the INPUT config a user authors
  ...
  superproject task chains it), kept honest by the reproducibility + parity tests. Owned by [`/charly-build:validate`](/recipes/build/validate/); the schema-change codegen
...
CLI (the [`/charly-tools:cue`](/recipes/tools/cue/) candy):
...
- [`/charly-tools:cue`](/recipes/tools/cue/) — the `cue` CLI candy that drives the vendoring pipeline.
...
- [`/charly-internals:go`](/recipes/internals/go/) — the Go source map (`candy/plugin-fleet/egress.go`, `cue_schema.go`).
