---
title: "vm-spec"
description: "Go type reference for VmSpec and the discriminated-union source types (VmSource cloud_image | bootc | clone | imported | bootstrap)."
---
...
the exact id partition live in [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) "virtiofs
...
- the `#LibvirtDomain` subtree is modeled + closed in the same schema (enums/ranges/
  PCI-hex + `cpu.mode:custom ⇒ model`); see [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/).
...
`charly migrate` does **not** perform this mapping any more — the harvest lived in the migration chain removed at the `2026.186.2323` baseline reset, so a config still carrying the legacy fields predates the schema floor (`2026.174.1100`) and is unmigratable; re-author it by hand. See [`/charly-build:migrate`](/recipes/build/migrate/) for the floor/HEAD gate and [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) for the policy.

## Cross-References

- [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) — YAML-authoring companion (when to pick cloud_image vs bootc, adopt pattern, step-by-step recipes)
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — `LibvirtDomain` rendering + pure render functions
- [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) — `RenderCloudInit`, `composeUsers`, seed ISO
- [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) — the external vm deploy + the vm deploy plugin consuming the resolved VmSpec via the host prepare hook
- [`/charly-internals:ovmf`](/recipes/internals/ovmf/) — `ResolveOvmfForSpec` reads `spec.Firmware`
- [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) — why the legacy surface was deleted in one PR
- [`/charly-vm:vm`](/recipes/vm/vm/) — command-family skill; reads vm.yml through VmSpec
- [`/charly-build:migrate`](/recipes/build/migrate/) — `charly migrate` command
- [`/charly-internals:go`](/recipes/internals/go/) — Go CLI development overview
