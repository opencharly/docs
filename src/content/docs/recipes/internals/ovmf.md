---
title: "ovmf"
description: "UEFI firmware (OVMF_CODE + OVMF_VARS) path resolution for VMs — per-distro path table, per-VM NVRAM copies, secure-boot variants, the empty-strings=skip contract."
---
...
- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — `spec.Firmware` field
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — `RenderDomainXML` consumer; `<loader>`/`<nvram>` emission conditions
- [`/charly-vm:vm`](/recipes/vm/vm/) — command-family; BIOS vs UEFI decision matrix
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — live-test RCA showing why `firmware: bios` is the right default for Arch cloud image
