---
title: "agents"
description: "Multi-agent support in OpenCharly across agent harnesses (Claude Code, Codex, Kimi) — sub-agents, dynamic workflows, agent teams, fresh validator sessions, and how each drives the existing `charly check` disposable beds to test and verify."
---
...
- [agent-roster](/recipes/internals/agents/agent-roster/)
- [hooks-and-lifecycle](/recipes/internals/agents/hooks-and-lifecycle/)
- [orchestration-model](/recipes/internals/agents/orchestration-model/)
- [parallel-bed-testing](/recipes/internals/agents/parallel-bed-testing/)
- [program-discipline](/recipes/internals/agents/program-discipline/)
...
freely throughout to verify** (see [`references/parallel-bed-testing.md`](/recipes/internals/agents/parallel-bed-testing/)
...
| Topic | Reference |
|---|---|
| Primitives per harness (Claude Code / Codex / Kimi); when to use a sub-agent vs. dynamic workflow vs. agent team; the charly agent roster (executors/enforcers); the shipped workflows (`/verify-beds`, `/audit-deploy-configs`, `/triage-check-failure`, `/verify-status`); the agent-team primitive setup | [`references/agent-roster.md`](/recipes/internals/agents/agent-roster/) |
...
- [`/charly-check:check`](/recipes/check/check/) — the bed surface these agents/workflows drive
  (`charly check run`/`image`/`live`, the disposable check-bed inventory,
  exit codes).
- [`/charly-internals:disposable`](/recipes/internals/disposable/) — why `disposable: true` is the sole
  destroy authorization.
- [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) — the R10-gated landing the executors
  feed.
- [`/charly-internals:skills`](/recipes/internals/skills/) — agent/skill discovery and the signpost
  convention.
