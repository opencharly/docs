---
title: "libvirt-renderer"
description: "Translation facade from VmSpec + LibvirtDomain to libvirt domain XML (via the libvirtxml library) and QEMU argv."
---
...
| `qxl` | Legacy SPICE-specific (2010 Red Hat); X11-oriented; requires xf86-video-qxl for acceleration | More knobs to tune (ram_size + vram_size + vram64_size_mb + vgamem_mb quartet); simpledrm→qxldrmfb takeover race under UEFI (see [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) Finding B); X11-dominant | Legacy guests only |
...
`DomainDefineXML`, which remains the authoritative gate. Owned by [`/charly-internals:egress`](/recipes/internals/egress/).

## Cross-References

- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — VmSpec shape the renderer reads
- [`/charly-internals:ovmf`](/recipes/internals/ovmf/) — `ResolveOvmfForSpec` for UEFI path resolution
- [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) — paired renderer for seed ISO + user-data
- [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) — consumer that applies the rendered domain
- [`/charly-vm:vm`](/recipes/vm/vm/) — command-family skill; video-model decision table
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — BIOS decision RCA; virtio-gpu live-test bisect
- [`/charly-distros:qemu-guest-agent`](/recipes/distros/qemu-guest-agent/) — virtio-serial channel snippet that this renderer emits in `<devices>`
