---
title: "disposable"
description: "`disposable: true` is the ONE and ONLY authorization for autonomous destroy + rebuild via `charly update`."
---
...
`backend: libvirt`. See [`/charly-build:build`](/recipes/build/build/) `resource:` + [`/charly-vm:vm`](/recipes/vm/vm/) "GPU
passthrough".
...
verified healthy. Full RCA + the safe-switch model: [`/charly-vm:vm`](/recipes/vm/vm/) "GPU
driver-mode switch".
...
[`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) (`VmSpec.Libvirt`) and [`/charly-vm:vm`](/recipes/vm/vm/) (GPU passthrough).
...
- [`/charly-check:check`](/recipes/check/check/) — the 11 testing standards; disposable-only deployment
  is Standard 4, fresh-rebuild re-verification is Standard 11.
- [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) — kind:vm schema, including `disposable:` and
  `lifecycle:` fields.
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — canonical worked example.
- [`/charly-core:deploy`](/recipes/core/deploy/) — `--disposable` / `--lifecycle` flags on
  `charly bundle add`.
