...
| New feature added to charly CLI | Update `/charly-core:<cmd>` or `/charly-build:<cmd>` skill + [`/charly-internals:go`](/recipes/internals/go/) source map |
...
(see [`/charly-build:docs`](/recipes/build/docs/)). Three consequences bind every skill edit:
...
1. **Edit the OWNING candy repo's `skill:` entity, then regenerate.** Since the candy
   ...
   — see [`/charly-internals:marketplace`](/recipes/internals/marketplace/)). Regenerate the corpus with
...
|---|---|
| "R10 gate by change class" matrix (incl. the class → gate → tier cross-walk) + "Flag discipline" catalog | [`/charly-check:check`](/recipes/check/check/) |
| R1–R5, RDD (incl. "The spike"), ADE operationalization (forbidden patterns, risk table, worked examples) | [`/charly-internals:strict-policy`](/recipes/internals/strict-policy/) |
| SDD operationalization (the schema→generated-code pipeline map, the schema-change recipe, the generation-coverage current state) | [`/charly-internals:go`](/recipes/internals/go/) |
| Hard-cutover workflow, forbidden patterns, deliverables | [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) |
| `disposable:` / `preemptible:` flag semantics, "What counts as an R10 run" | [`/charly-internals:disposable`](/recipes/internals/disposable/) |
| Landing mechanics (branch loop, the two-step PR + `pr-validator` merge/tag, CalVer-generated-at-merge, branch protection, multi-repo order) | [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) |
| Agent/workflow/team primitives, hooks doctrine | [`/charly-internals:agents`](/recipes/internals/agents/) |
| Kernel/plugin doctrine (core = kernel; every capability a plugin candy), the two authoring shapes, placement, the three-lane transport doctrine, the seams catalog, **the kernel/plugin boundary law** (E/M/B/D/R) + the incomplete-seam mandate | [`/charly-internals:plugin`](/recipes/internals/plugin/) |
...
|---|---|---|
| [`/charly-automation:enc`](/recipes/automation/enc/) | `charly config --encrypt`, `charly config mount`, `charly config unmount`, `charly config passwd` | Encrypted-volume (gocryptfs) semantics, keyring resolution, `charly-enc-<image>-<volume>.scope` lifecycle |
| [`/charly-automation:openclaw-deploy`](/recipes/automation/openclaw-deploy/) | Composing `openclaw-*` layers | OpenClaw AI gateway deployment story |
| [`/charly-automation:sidecar`](/recipes/automation/sidecar/) | `charly config --sidecar tailscale` | Sidecar-container model, pod networking, env-var routing |
...
proof). Full story: [`/charly-internals:agents`](/recipes/internals/agents/). Dynamic workflows are NOT plugin
...
5. **Land docs-only**: skill edits are the Documentation-only change class →
   ...
   changelog file and its tag). See [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/).
...
[`/charly-internals:go-quality`](/recipes/internals/go-quality/).

## Cross-References

- [`/charly-internals:go`](/recipes/internals/go/) — Source code structure, adding new commands
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — Understanding generated Containerfiles
- [`/charly-internals:agents`](/recipes/internals/agents/) — Sub-agents, dynamic workflows, agent teams; how they drive the `charly check` beds; the hooks doctrine; the signpost convention
- [`/charly-build:validate`](/recipes/build/validate/) — Validation rules
