---
title: "vm-deploy-target"
description: "The external VM deploy substrate applies InstallPlan inside a guest via the reverse-channel SSH executor — plugin-deploy-vm, VM boot/readiness, Charly delivery, nested pods, lifecycle, VmDeployState, DeployExecutor, the pluginDeployTarget/UnifiedDeployTarget seams."
---
...
[`/charly-internals:install-plan`](/recipes/internals/install-plan/) for the shared IR + the full S3b architecture.
...
6. **`kit.EnsureCharlyInGuest`.** Runs the `VmCharlyInstall.Strategy` state machine (see [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/)).
...
guest as the deploy user, already correct. See [`/charly-internals:install-plan`](/recipes/internals/install-plan/)
...
a clean boot mid-deploy. See [`/charly-internals:install-plan`](/recipes/internals/install-plan/) RebootStep.
...
skipping; see [`/charly-check:check`](/recipes/check/check/) "parent.child reaches the actual leaf". `charly vm
...
When the VM's network uses libvirt user-mode + `<backend type='passt'/>` + `<portForward>` (see [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/)), the guest `SSHExecutor` connects to `127.0.0.1:<host-port>`. The portForward maps that through passt into the guest's `:22`. The indirection is invisible to `SSHExecutor` — it sees a normal TCP connect.

## Cross-References

- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — InstallPlan IR (the in-proc deploy-target implementers + step kinds; `pluginDeployTarget` consumes the IR for the external substrates)
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — the out-of-process plugin model + the executor reverse channel `candy/plugin-deploy-vm` rides
- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — VmSpec consumed by the vm deploy plugin's host prepare hook
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — renders domain XML; portForward + passt backend
- [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) — `kit.EnsureCharlyInGuest` (runs in the plugin's `OpPrepareVenue`)
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly fleet add vm:<name>` command + charly.yml schema
- [`/charly-local:local-deploy`](/recipes/local/local-deploy/) — the sibling external substrate (`deploy:local` via `candy/plugin-deploy-local`); same `kit.WalkPlans` + ReverseOps model
- [`/charly-vm:vm`](/recipes/vm/vm/) — VM lifecycle; creates the venue the vm deploy runs against
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — canonical worked example — VmDeployState persistence; ssh_key idempotency live-test
