...
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — InstallPlan IR (the in-proc DeployTarget implementers + step kinds; `pluginDeployTarget` consumes the IR for the external substrates)
...
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly bundle add vm:<name>` command + charly.yml schema
