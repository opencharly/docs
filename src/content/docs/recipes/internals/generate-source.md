...
description: "Containerfile generation: understanding charly box generate output, multi-stage builds, intermediate images, and the .build/ directory."
---
...
- [labels-and-caching](/recipes/internals/generate-source/labels-and-caching/)
- [multistage-and-intermediates](/recipes/internals/generate-source/multistage-and-intermediates/)
- [task-emission](/recipes/internals/generate-source/task-emission/)
...
| Topic | File |
|---|---|
| Task emission pipeline — per-verb emitters, `Task` struct, emitter helpers, shell-quoting, inline-content staging, user resolution, variable substitution, adjacent-coalescing, parent-dir auto-insertion, tag-section install emission | [`references/task-emission.md`](/recipes/internals/generate-source/task-emission/) |
| Multi-stage build stages (pixi/npm/AUR/OpResolve builders, scratch context), auto-intermediate grouping by `(Base, UID)`, intermediate images, container user resolution (adopt vs create) | [`references/multistage-and-intermediates.md`](/recipes/internals/generate-source/multistage-and-intermediates/) |
| LABEL placement + cache efficiency, OCI label reference table, runtime-only features, cache mounts | [`references/labels-and-caching.md`](/recipes/internals/generate-source/labels-and-caching/) |
...
- [`/charly-image:layer`](/recipes/image/layer/) — **Canonical author-facing reference** for the task verb catalog, `var:` substitution, YAML anchors, execution order. The emitter pipeline here implements what's documented there.
- [`/charly-build:generate`](/recipes/build/generate/) — User-facing `charly box generate` command.
- [`/charly-internals:go`](/recipes/internals/go/) — Source code map: `charly/tasks.go` (emitter pipeline, `emitTasks` shim → deploykit), `sdk/deploykit/candy_steps.go:WriteCandySteps` (orchestrator, relocated in #67), `charly/layers.go:Task` struct, `candy/plugin-box/validate_rules.go:validateCandyTasks`.
- [`/charly-build:validate`](/recipes/build/validate/) — User-facing validation rules (what `validateCandyTasks` enforces).
- [`/charly-build:build`](/recipes/build/build/) — Building from generated Containerfiles.
- [`/charly-internals:egress`](/recipes/internals/egress/) — the emitted Containerfile is egress-validated (`writeContainerfile` → `#RenderedText`, rejecting the `<no value>` template-failure marker) before it is written; the traefik-routes scratch-stage input is likewise validated (`#TraefikRoutes`).
