---
title: "cutover-policy"
description: "Authoritative reference for the \"Hard Cutover by Default\" policy governing schema changes, API renames, and deprecations."
---
...
- **Engineering-discipline gates (R1–R5).** See [`/charly-internals:strict-policy`](/recipes/internals/strict-policy/). Every failure during the cutover triggers [`/charly-internals:root-cause-analyzer`](/recipes/internals/root-cause-analyzer/) BEFORE any remediation (R1). Every issue surfaced is fixed in the cutover or escalated (R2). Duplication is refactored on first surface (R3). Workarounds are forbidden (R4). Stale references are swept (R5).
...
- [`/charly-build:migrate`](/recipes/build/migrate/) — the single `charly migrate` command, its CalVer schema versioning, and the CUE-anchored declarative migration table
- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — example output of a cutover (new types replacing deleted ones)
- [`/charly-internals:capabilities`](/recipes/internals/capabilities/) — example of coordinated label-map cleanup during a cutover
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — shared IR that survived the cutover unchanged (non-example — additive extension of the DeployTarget surface)
