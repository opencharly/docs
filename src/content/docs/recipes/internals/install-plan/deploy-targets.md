...
Used by: `charly fleet add vm:<name> <ref>` / `charly fleet del vm:<name>`. See [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) for the full flow, the plugin-implemented venue lifecycle, `VmDeployState` persistence, and SSH-key idempotency.
