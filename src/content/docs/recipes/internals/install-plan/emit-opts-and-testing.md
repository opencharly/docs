---
title: "install-plan — emit-opts-and-testing"
description: "Detail page of the install-plan recipe card."
---

<!-- DO-NOT-EDIT — generated by `charly docs generate`; run `task docs:sync` to regenerate. -->

*Detail page of the [install-plan](/recipes/internals/install-plan/) recipe card.*
...
[`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) "Guest-home resolution".

## `ReverseOp` catalogue

See [`/charly-local:local-deploy`](/recipes/local/local-deploy/) for the user-facing reverse-op table. The Go-level source of truth is `ReverseOpKind` in `spec/spec/deploy_consts.go` (the IR's step kinds; `spec/spec/install_plan.go` — the former `charly/install_plan.go` is DELETED, K-wave 2); each step's `Reverse()` method emits ops tagged with kind + targets + scope. Execution lives in `sdk/kit/reverse_ops.go` (moved from `charly/reverse_ops.go`) — one handler per kind, all routed through `runReverseOps(ops, executor)` in LIFO order.
