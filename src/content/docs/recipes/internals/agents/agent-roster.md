...
Companion reference for [`/charly-internals:agents`](/recipes/internals/agents/). Owns the primitives per
...
workflows are bed-scoped too" in [`references/parallel-bed-testing.md`](/recipes/internals/agents/parallel-bed-testing/)). The
...
[`references/parallel-bed-testing.md`](/recipes/internals/agents/parallel-bed-testing/)). "Prefer agents" governs bounded
work.
...
- **`check-bed-runner`** — runs `charly check run <bed>` one-shot (the full
  ...
  [`references/parallel-bed-testing.md`](/recipes/internals/agents/parallel-bed-testing/); an in-process teammate cannot, its
...
- **`layer-validator`** — pre-edit `charly.yml` sanity gate; defers the
  full schema to [`/charly-image:layer`](/recipes/image/layer/) + `charly box validate`.
- **`pr-validator`** — the fresh PR evaluator (the disposing half of the
  ...
  independent evaluation. See [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) (B1 step 2,
  B5).
...
[`references/hooks-and-lifecycle.md`](/recipes/internals/agents/hooks-and-lifecycle/) "Hooks doctrine" for the current hook
inventory).

## See also

- Entry: `../SKILL.md`
- [`references/orchestration-model.md`](/recipes/internals/agents/orchestration-model/) — the default execution model, model
  tiers, responsibility matrix.
- [`references/parallel-bed-testing.md`](/recipes/internals/agents/parallel-bed-testing/) — bed-scoped ownership, the
  implementation-workflow shape, the binding rule for running a bed.
- [`references/hooks-and-lifecycle.md`](/recipes/internals/agents/hooks-and-lifecycle/) — hooks doctrine, teammate lifecycle.
