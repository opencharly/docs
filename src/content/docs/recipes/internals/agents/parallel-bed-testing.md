---
title: "agents — parallel-bed-testing"
...
Companion reference for [`/charly-internals:agents`](/recipes/internals/agents/). Owns the bed-scoped
...
The shipped workflows in [`references/agent-roster.md`](/recipes/internals/agents/agent-roster/) verify. A dynamic
...
workflow expression of the B3 model ([`/charly-internals:git-workflow`](/recipes/internals/git-workflow/)), not
...
- **Multi-worktree concurrency corollary.** Because each worktree carries
  ...
  [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) B3) applies only to the shared-tree team
...
- **Declare an intended live bed run to the orchestrator and wait for a
  ...
  collision-free by construction; [`/charly-check:check`](/recipes/check/check/).)
...
- **Handling a long-running bed — by mechanism, not by who owns it.** A
  ...
  - **Reconnect via durable state, never a held process handle.**
    ...
    code is the discriminator (3 = skipped). See [`/charly-check:check`](/recipes/check/check/). On
...
not the defense. The full flag catalog and rule: [`/charly-check:check`](/recipes/check/check/)
"Flag discipline".

## See also

- Entry: `../SKILL.md`
- [`references/orchestration-model.md`](/recipes/internals/agents/orchestration-model/) — the model that schedules against
  this partition.
- [`references/agent-roster.md`](/recipes/internals/agents/agent-roster/) — `check-bed-runner`, the shipped workflows.
- [`references/hooks-and-lifecycle.md`](/recipes/internals/agents/hooks-and-lifecycle/) — sub-agent operational invariants,
