---
title: "go — schema-change-recipe"
description: "Detail page of the go recipe card."
---

<!-- DO-NOT-EDIT — generated by `charly docs generate`; run `task docs:sync` to regenerate. -->

*Detail page of the [go](/recipes/internals/go/) recipe card.*
...
4. **Bind behavior.** A new GENERIC install-VERB (a kernel primitive — rare) adds
   ...
   [`/charly-internals:plugin`](/recipes/internals/plugin/) "The kernel/plugin boundary law".
...
6. **Schema-version bump ONLY on an authored WIRE-key change.** Only if the
   ...
   the compiled-in `command:migrate` plugin) per [`/charly-build:migrate`](/recipes/build/migrate/). A pure
...
ingress validation recipe is owned by [`/charly-build:validate`](/recipes/build/validate/); the egress
analog is the "Adding a new egress schema" recipe in [`/charly-internals:egress`](/recipes/internals/egress/).
