---
title: "enc"
description: "Encrypted storage through `charly config` — gocryptfs, the `--encrypt` and encrypted-volume flags, config mount/unmount/status/passwd, or Charly encrypted-volume systemd units."
---
...
2. Try a collection matching the `keyring_collection_label` setting (if
   non-empty — see [`/charly-build:settings`](/recipes/build/settings/)). Useful for pinning a specific
...
override: `CHARLY_KEYRING_COLLECTION_LABEL`. See [`/charly-build:settings`](/recipes/build/settings/) for the full
...
charly's keyring backend reads them like any other collection. See [`/charly-build:secrets`](/recipes/build/secrets/)
...
This guards against a real data-loss shape: a quadlet missing the `ExecStartPre=charly config mount <image>` auto-mount hook (see "Boot Behavior: Which Backend, and Why" above and [`/charly-build:migrate`](/recipes/build/migrate/) "charly migrate") would silently bind an empty `plain/` over a populated cipher tree, the container's services would `initdb` / first-run-wizard against the empty dir, and plaintext data would accumulate on top of an encrypted vault. This error class fails the start IMMEDIATELY when `charly start` detects that exact pre-start state.
...
- [`/charly-core:deploy`](/recipes/core/deploy/) -- Quadlet integration, volume backing configuration, charly.yml
- [`/charly-core:charly-config`](/recipes/core/charly-config/) -- `encrypted_storage_path` and `volumes_path` settings, `charly config mount` short-circuit fast-path documented there too
- [`/charly-core:service`](/recipes/core/service/) -- Container lifecycle, `charly start` inline mount
- [`/charly-build:secrets`](/recipes/build/secrets/) -- Credential store hierarchy (env → keyring → config), `charly secrets set charly/enc <image>` to store a gocryptfs passphrase explicitly, `charly secrets list` to inspect indexed keys
- [`/charly-build:settings`](/recipes/build/settings/) -- `secret_backend`, `keyring_collection_label`, `encrypted_storage_path`, and other runtime config keys that control credential + volume resolution
- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) -- "Secret Service collections" health check, "Keyring index consistency" cross-check; invoke `charly doctor` when diagnosing broken-collection symptoms
...
**Workflow position:** Pre-deployment. Configure encrypted volumes during `charly config`. See also [`/charly-core:deploy`](/recipes/core/deploy/) (volume backing).
