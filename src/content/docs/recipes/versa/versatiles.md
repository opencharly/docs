---
title: "versatiles"
description: "VersaTiles CLI (versatiles-rs) — a single Rust binary that handles `convert` / `serve` / `probe` / `dev` for the `.versatiles`, `.pmtiles`, `.mbtiles`, and `.tar` tile-container formats."
---
...
in [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/): GitHub release API → asset URL
...
See [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) "CRITICAL: martin caches pmtiles file
mtime at startup" — same restart-race pattern applies.

## Cross-references

- [`/charly-versa:versa`](/recipes/versa/versa/) — image composing this layer
- [`/charly-versa:shortbread`](/recipes/versa/shortbread/) — produces the PMTiles files versatiles serves
- [`/charly-versa:versatiles-style`](/recipes/versa/versatiles-style/) — MapLibre style generator paired with
  versatiles serve as tile backend
- [`/charly-versa:versatiles-frontend`](/recipes/versa/versatiles-frontend/) — pre-built SPA on port 28002 for
  visually exploring tile content
- [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) — sibling martin tile server (port
  23000); the two run in parallel for the comparison surface
- [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — the versatiles convert round-trip demo
