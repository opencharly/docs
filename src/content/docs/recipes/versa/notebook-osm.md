---
title: "notebook-osm"
description: "Standalone marimo notebook (osm-monaco-viz.py) that self-authors TWO Airflow DAGs (osm + gtfs), triggers them via REST, runs polars + pyarrow analytics on both datasets, and renders TWO maps: streets via MapLibre GL JS + martin vector tiles + 3D terrain, transit via folium with 98 bus-stop CircleMarkers."
---
...
[`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) for why.
...
[`/charly-versa:versa`](/recipes/versa/versa/) "Required charly.yml env block".
...
- [`/charly-versa:marimo-layer`](/recipes/versa/marimo-layer/) — the runtime kernel
- [`/charly-versa:airflow-layer`](/recipes/versa/airflow-layer/) — DAG trigger REST API
- [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) — martin + tippecanoe + reload pattern
- [`/charly-versa:maputnik-layer`](/recipes/versa/maputnik-layer/) — companion vector-tile editor
- [`/charly-versa:marimo-mcp`](/recipes/versa/marimo-mcp/) — read-only inspection of this notebook's session
