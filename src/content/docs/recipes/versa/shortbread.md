---
title: "shortbread"
description: "Shortbread vector-tile schema generation via Tilemaker."
---
...
[`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/): shallow clone → `make -j$(nproc)` →
...
- [`/charly-versa:versatiles`](/recipes/versa/versatiles/) — the tile-server service that auto-discovers
  `/workspace/tiles/shortbread/*.pmtiles` files
- [`/charly-versa:versatiles-style`](/recipes/versa/versatiles-style/) — MapLibre style generator targeting
  the Shortbread schema (the notebook's shortbread cell renders the
  pipeline output using `colorful()`)
- [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — DAG cell 2 self-authors the
  `notebook_osm_shortbread_pipeline` that drives this layer
- [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) — sibling tippecanoe pipeline + martin
