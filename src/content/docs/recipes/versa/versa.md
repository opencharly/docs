---
title: "versa"
description: "versa — image bundling marimo reactive notebook environment with Apache Airflow + GPU-accelerated OSM/GTFS analytics + martin vector tiles + 3D terrain via MapLibre + Polars geospatial extensions (polars-st, geopolars) + GeoArrow + deck.gl rendering via lonboard."
---
...
5. `marimo` — [`/charly-versa:marimo-layer`](/recipes/versa/marimo-layer/) — pixi env (cudf-polars-cu13,
...
6. `airflow` — [`/charly-versa:airflow-layer`](/recipes/versa/airflow-layer/) — 4 supervisord services
...
7. `osm-tools` — [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) — tippecanoe (built from
...
8. `maputnik` — [`/charly-versa:maputnik-layer`](/recipes/versa/maputnik-layer/) — MapLibre style editor
   built with Vite `--base=/`
9. `pmtiles-viewer` — [`/charly-versa:pmtiles-viewer`](/recipes/versa/pmtiles-viewer/) — protomaps/PMTiles
   /app SPA, visual inspector for PMTiles archives (port 8001 → 28001)
10. `shortbread` — [`/charly-versa:shortbread`](/recipes/versa/shortbread/) — systemed/tilemaker (C++/Lua)
    + shortbread-tilemaker config — produces shortbread-schema vector
    tiles into `/workspace/tiles/shortbread/`
11. `versatiles-style` — [`/charly-versa:versatiles-style`](/recipes/versa/versatiles-style/) — @versatiles/style
    MapLibre style generator bundled to `/opt/versatiles-style/`
12. `versatiles-fonts` — [`/charly-versa:versatiles-fonts`](/recipes/versa/versatiles-fonts/) — SDF font glyphs
    (Noto Sans + 9 others) bundled to `/opt/versatiles-fonts/`
13. `maplibre-versatiles-styler` — [`/charly-versa:maplibre-versatiles-styler`](/recipes/versa/maplibre-versatiles-styler/) —
    interactive MapLibre style-switcher control bundled to
    `/opt/maplibre-versatiles-styler/`
14. `versatiles` — [`/charly-versa:versatiles`](/recipes/versa/versatiles/) — versatiles-rs CLI binary
    (convert/serve/probe) + `versatiles serve` supervisord service on
    port 8090 → 28090 (parallel tile server to martin)
15. `versatiles-frontend` — [`/charly-versa:versatiles-frontend`](/recipes/versa/versatiles-frontend/) —
    versatiles-org/versatiles-frontend SPA (port 8002 → 28002); also
    re-exports versatiles-style / fonts / styler at `/style/`,
    `/fonts/`, `/styler/`
16. `notebook-osm` — [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — data-only layer
...
18. `debug-tools` — [`/charly-versa:debug-tools-layer`](/recipes/versa/debug-tools-layer/) — 49 standard
...
deploy-level `port:` key IS auto-allocation ([`/charly-core:deploy`](/recipes/core/deploy/)
...
fallback default. See [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) cell #2.

### Multi-instance pattern (Pattern A from [/charly-core:deploy](/recipes/core/deploy/))
...
### Pinned-version pattern (Pattern B from [/charly-core:deploy](/recipes/core/deploy/))
...
|---|---|---:|---|
| marimo | `http://localhost:2718/mcp/server` | 10 (read-only inspection) | [`/charly-versa:marimo-mcp`](/recipes/versa/marimo-mcp/) |
...
- **Martin caches pmtiles mtime at startup.** Re-run a DAG that
  ...
  see [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/).
- **Notebook needs `image:` field too.** Already covered above —
  same point worth emphasizing.

## Cross-references

- [`/charly-versa:marimo-layer`](/recipes/versa/marimo-layer/) — marimo runtime layer (pixi env, service)
- [`/charly-versa:marimo-mcp`](/recipes/versa/marimo-mcp/) — marimo MCP server tool catalog
- [`/charly-versa:airflow-layer`](/recipes/versa/airflow-layer/) — Airflow 3.x compat findings
- [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — the dual-DAG OSM+GTFS notebook
- [`/charly-versa:maputnik-layer`](/recipes/versa/maputnik-layer/) — Vite `--base=/` build pattern
- [`/charly-versa:osm-tools-layer`](/recipes/versa/osm-tools-layer/) — martin reload pattern
- [`/charly-versa:debug-tools-layer`](/recipes/versa/debug-tools-layer/) — debug toolkit
- [`/charly-check:check`](/recipes/check/check/) — check verbs used by the live probes
- [`/charly-core:deploy`](/recipes/core/deploy/) — env block authoring + cross-pod topology
- [`/charly-internals:disposable`](/recipes/internals/disposable/) — `disposable: true` semantics
