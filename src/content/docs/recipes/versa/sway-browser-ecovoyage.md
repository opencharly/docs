...
MCP name-disambiguation rule — see [`/charly-core:deploy`](/recipes/core/deploy/)) so it
...
- [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — the marimo notebook whose 5 maps this skill exists to debug
- [`/charly-versa:versa`](/recipes/versa/versa/) — the image hosting the maps (versa/ecovoyage = Pattern A instance)
- [`/charly-selkies:sway-browser-vnc`](/recipes/selkies/sway-browser-vnc/) — the base image this deploy instantiates
- [`/charly-selkies:chrome-devtools-mcp`](/recipes/selkies/chrome-devtools-mcp/) — the layer providing the MCP server
- [`/charly-check:cdp`](/recipes/check/cdp/) — the verb catalog driving Chrome
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — verifying the MCP endpoint via the `mcp:` check verb (`charly check live <image> --filter mcp`)
- [`/charly-core:deploy`](/recipes/core/deploy/) — Pattern A multi-instance + MCP name disambiguation
- [`/charly-automation:sidecar`](/recipes/automation/sidecar/) — alternative tailscale sidecar pattern (not used here; this skill uses host-serve)
