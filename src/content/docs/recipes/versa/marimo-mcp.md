---
title: "marimo-mcp"
description: "marimo's built-in MCP server (10 read-only inspection tools — get_active_notebooks, get_cell_outputs, get_notebook_errors, etc.) at port 2718 path /mcp/server."
---
...
`--mcp` flag (see [`/charly-versa:marimo-layer`](/recipes/versa/marimo-layer/) service spec). It serves
...
state chaining; point an external MCP client ([`/charly-tools:mcporter`](/recipes/tools/mcporter/)) at the
...
- [`/charly-versa:marimo-layer`](/recipes/versa/marimo-layer/) — layer that runs the server
- [`/charly-versa:airflow-layer`](/recipes/versa/airflow-layer/) — Airflow service in the same pod; it has no MCP wrapper
- [`/charly-versa:notebook-osm`](/recipes/versa/notebook-osm/) — example notebook diagnosed via this MCP
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — MCP probe verb authoring + URL rewriter
- [`/charly-check:check`](/recipes/check/check/) — `mcp:` declarative check-verb reference
