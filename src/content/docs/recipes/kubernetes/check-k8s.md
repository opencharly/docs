---
title: "check-k8s"
description: "Kubernetes cluster-probe declarative check verb — the `kube:` check verb (nodes, pods, ingress, storage class, addon health, apply/delete, arbitrary resource GETs) served out-of-process by candy/plugin-kube (vendored client-go; no external kubectl)."
---
...
- [`/charly-check:check`](/recipes/check/check/) — the unified `charly check` surface (image / live /
  run), the plan-step vocabulary, and how the provider registry dispatches
  declarative verbs.
- [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/) — deploying images to a Kubernetes cluster
  (`kind: kubernetes` cluster templates, Kustomize generation, `charly fleet`).
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — the Provider model and the out-of-process
  plugin dispatch the `kube:` verb rides on.
- [`/charly-infrastructure:k3s`](/recipes/infrastructure/k3s/) — the k3s-server / k3s-agent candies whose
