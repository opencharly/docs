---
title: "helm"
description: "The helm words — the `step:helm-release` install step (a `helm upgrade --install` performed IN-VENUE, returning a `helm uninstall` ReverseOp) and the `verb:helm` declarative release-status assertion, both served out-of-process by candy/plugin-helm; plus the `helm_charts:` deploy field the k8sgen emitter renders."
---
...
spec-is-the-test discipline owned by [`/charly-check:check`](/recipes/check/check/): the candy that
...
- [`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/) — the `kube:` verb these beds pair with.
- [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/) — the `kubernetes:` deploy substrate and
  its Kustomize generation.
- [`/charly-check:check`](/recipes/check/check/) — the plan-step vocabulary and how the provider
  registry dispatches declarative verbs.
- [`/charly-internals:plugin`](/recipes/internals/plugin/) — the Provider model and out-of-process
  dispatch this rides on.
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — defines the vocabulary this page
...
- [`/charly-infrastructure:k3s`](/recipes/infrastructure/k3s/) — the `k3s-server` candy the beds deploy.
- [`/charly-coder:kubernetes-layer`](/recipes/coder/kubernetes-layer/) — the `kubernetes` candy that installs
