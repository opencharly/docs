---
title: "vms-catalog"
description: "Authoring reference for kind:vm entities in vm.yml — the VmSpec schema, source.kind discriminator (cloud_image vs bootc), base_user adopt pattern, and step-by-step recipes for both source kinds."
---
...
The VM surface parallels the `candy:` image surface: one YAML entry per entity, kind-keyed, discovered through includes. The Go types that back it live in [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/); the rendering paths in [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) and [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/).
...
<name>:
  vm:
    ...
    # Libvirt XML knobs (see [/charly-internals:libvirt-renderer](/recipes/internals/libvirt-renderer/)):
    libvirt:
      devices:
...
with the [`/charly-distros:workspace-mount`](/recipes/distros/workspace-mount/) layer (a systemd `.mount` unit for the
...
never auto-tunneled, by design. See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/)
...
Canonical example: [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — the most fully documented cloud_image VM, and the one to **read before authoring another one** (it is not the only one: `kind: cloud_image` sources also ship in `charly/charly.yml`, `distro-fedora` and `distro-arch`). It documents the non-obvious decisions learned the hard way:
...
- **virtio-gpu, not QXL.** See [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) "video model choice" — virtio-gpu is the modern default for Linux guests.
...
4. Run `charly vm build <vm-name>`. See [`/charly-vm:vm`](/recipes/vm/vm/) known-caveats section for bootc-specific gotchas (rootful storage split, nested-container `--transport containers-storage`, loopback device mount namespace).
...
Mirrors the container-side `base_user:` + `user_policy: adopt` pattern documented in [`/charly-image:image`](/recipes/image/image/) "user_policy". The key insight: **don't recreate accounts cloud-init already shipped** — just append the SSH pubkey and move on.
...
`charly_install.strategy: auto` in `cloud_init:` wires `charly`'s in-guest installer ([`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) → emitted `runcmd:` entries) so the provisioned VM comes up with `charly` already installed. Lets `charly fleet add vm:<name>` apply host-deploy-style layer recipes inside the VM over SSH without a bootstrap round-trip. See [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) for the emission + handshake.
...
Load-time errors raised by the closed `#Vm` CUE schema (`spec/schema/vm.cue`, see [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/)):
...
Projects predating this schema had three coupled fields on image entries (`candy:` nodes carrying `base:`/`from:`): `bootc: true`, `vm: {...}`, `libvirt: [...]`. **Two** were deleted in the hard cutover — `vm:` and `libvirt:`. `bootc: true` is LIVE and stays on the image entry (`spec/schema/box.cue` declares `bootc?: bool`); it marks the image bootable, and a separate `kind: vm` entity with `source.kind: bootc` references it via `source.box:`, exactly as [`/charly-image:image`](/recipes/image/image/) states. Those two fields are gone, and the step that harvested them into VM nodes lived in the migration chain that was **removed at the `2026.186.2323` migration-baseline reset**. So `charly migrate` no longer converts those legacy fields: a config still carrying them predates the supported schema floor (`2026.174.1100`) and is **unmigratable** — `charly migrate` refuses it (`predates the supported floor … re-author against the current schema`) and changes nothing on disk. Re-author such a VM by hand as a name-first `<name>: {vm: {…}}` node (the shape this skill documents).

For a config already within the migratable window (`[floor, HEAD)`), `charly migrate` is one idempotent command that brings it to the current schema. See [`/charly-build:migrate`](/recipes/build/migrate/) for the floor/HEAD gate and the full command reference, and [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) for why hard-cutover was the chosen policy.

## Cross-References

- [`/charly-vm:vm`](/recipes/vm/vm/) — the `charly vm build/create/start/stop/ssh/console` command family
- [`/charly-build:migrate`](/recipes/build/migrate/) — `charly migrate` conversion from legacy
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly fleet add vm:<name>` for in-guest layer application
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — canonical cloud_image VM
- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — Go type reference
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — libvirt XML emission
- [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) — NoCloud seed ISO + user-data emission
- [`/charly-internals:ovmf`](/recipes/internals/ovmf/) — UEFI firmware path resolution (when `firmware:` ≠ `bios`)
- [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) — the external vm deploy in the InstallPlan pipeline
- [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) — Hard Cutover by Default policy
- [`/charly-distros:cloud-init`](/recipes/distros/cloud-init/) — guest-side cloud-init layer (pairs with host-side `cloud_init:` emission)
- [`/charly-distros:qemu-guest-agent`](/recipes/distros/qemu-guest-agent/) — virtio-serial channel for host↔guest comms
