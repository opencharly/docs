...
Canonical example: [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/). Only existing cloud_image VM in the repo — **read it before authoring another one**. It documents the non-obvious decisions learned the hard way:
...
3. Start from [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) as a template. Change `url`, `base_user`, **`distro:`** (the template ships it commented out, which is legal only for the `arch` guest it happens to be — see the rule above), and the distro-specific cloud_init `package:` and `runcmd:`.
