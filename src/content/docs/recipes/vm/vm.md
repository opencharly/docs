...
|---|---|
| `bios` (default) | Cloud images that ship a GPT BIOS boot partition — **Arch, Fedora Cloud, CentOS Cloud are proven on beds** (`check-arch-repo`, `check-fedora-repo`, `box/fedora`). Avoids stale BOOTX64.EFI issues. No OVMF package dependency. No per-VM NVRAM. See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) Finding B for the RCA. |
