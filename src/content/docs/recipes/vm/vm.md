---
title: "vm"
description: "MUST be invoked before any work involving: virtual machines, charly vm commands, kind:vm entities in charly.yml, cloud_image vs bootc source types, libvirt/QEMU backends, BIOS vs UEFI firmware, virtio-gpu video, or VM lifecycle."
---
...
- **`charly update <vm-entity-name>`** does NOT gate on `disposable:` — an explicit invocation rebuilds ANY target (destroy→create the domain, reuse the qcow2 disk unless `--build`, then re-apply the deploy's layers via the shared `fleet add` path). For a non-disposable, non-ephemeral target it prints a one-line transparency note (`noteUpdateDisposability`) and proceeds. The `disposable: true` flag stays load-bearing as the authorization for UNATTENDED autonomous destroy + rebuild (the project rulebook R10 (`AGENTS.md` / `CLAUDE.md`)) and the check-runner's unattended fresh rebuild, NOT as an `charly update` capability check. See [`/charly-internals:disposable`](/recipes/internals/disposable/) and [`/charly-core:charly-update`](/recipes/core/charly-update/).
...
design; see [`/charly-internals:disposable`](/recipes/internals/disposable/) "Standing authorization").
...
2. **The hostdev is AUTO-ALLOCATED.** When a deploy of a VM declares
   ...
   [`/charly-internals:disposable`](/recipes/internals/disposable/) "resource-arbitration axis" + [`/charly-build:build`](/recipes/build/build/)
   `resource:`.
...
[`/charly-vm:cachyos-bootstrap-vm`](/recipes/vm/cachyos-bootstrap-vm/), [`/charly-check:check`](/recipes/check/check/)).
...
claims (see [`/charly-internals:disposable`](/recipes/internals/disposable/)); `charly vm gpu mode [vfio|nvidia]`
...
active lease (a leaked/stray disposable bed — detail: [`/charly-internals:disposable`](/recipes/internals/disposable/)
...
the [`/charly-distros:workspace-mount`](/recipes/distros/workspace-mount/) layer. See [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/).
  ...
  vm:
    source:
      ...
      # distro:                                    # `distro:` is inferred from `base_user` ONLY for the literals `arch` and
                                                   ...
                                                   # race described in [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/), or
...
Every field in `VmSpec` applies to both source kinds (parity guarantee). See [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) for the Go type reference and [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) for the full field-by-field authoring guide.
...
|---|---|
| `bios` (default) | Cloud images (Arch, Fedora Cloud, Ubuntu Cloud, Debian Cloud, CentOS Cloud) that ship a GPT BIOS boot partition. Avoids stale BOOTX64.EFI issues. No OVMF package dependency. No per-VM NVRAM. See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) Finding B for the RCA. |
...
See [`/charly-internals:ovmf`](/recipes/internals/ovmf/) for the per-distro OVMF_CODE/OVMF_VARS path table and the per-VM NVRAM mechanics. When `firmware: bios`, `ResolveOvmfForSpec` returns empty strings and the libvirt renderer skips `<loader>`/`<nvram>` entirely.
...
| `qxl` | Legacy X11 SPICE use | 2010-era Red Hat SPICE stack; simpledrm→qxldrmfb takeover race under UEFI (see [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) Finding B); more knobs (ram_size + vram_size + vram64_size_mb + vgamem_mb) |
| `cirrus` | BIOS fallback only | Low resolution, no acceleration |
| `none` | Headless VMs | No framebuffer at all |

SPICE is video-model-agnostic — it streams pixels from virtio-gpu as well as from QXL. See [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) "video-model choice" for the full decision rationale.
...
See [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) for the `VmSSH.KeyInjection` tri-state (`auto` | `enabled` | `disabled`) per channel, and [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) for the `composeUsers` adopt-merge pattern that delivers the key into the guest.
...
See [`/charly-core:deploy`](/recipes/core/deploy/) "vm: target" for the `charly fleet add vm:<name>` surface and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) for the executor model.
...
Non-obvious issues that surface only when VMs actually boot. [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) is the canonical cloud_image worked example; the bootc-specific caveats below apply to any `source.kind: bootc` VM.
...
libvirt ≥ 9.x `<portForward>` on `<interface type='user'>` only activates with `<backend type='passt'/>`. Emitted automatically by `renderDefaultInterface`; if you author a custom interface, include the backend line. Symptom when missing: ssh on port 2224 accepts TCP but lands in the wrong guest port. See [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/).

### Per-VM NVRAM required for UEFI

When `firmware: uefi-insecure` or `firmware: uefi-secure`, each VM gets its own NVRAM file under `~/.local/share/charly/vm/charly-<name>/nvram.fd`, copied from the OVMF_VARS template. Preserves UEFI variable state across reboots. See [`/charly-internals:ovmf`](/recipes/internals/ovmf/).

### Resource sizing over package pruning (cloud_image)

Cloud-init `packages: [spice-vdagent]` pulls GTK3 + X11 (~200 MB download, ~1 GB installed). Running at 2 GiB RAM stalls cloud-init. **Check host inventory (`free -h`, `nproc`) before sizing** — give the VM what it needs (8 GiB + 4 cpus for a desktop-class workload is reasonable on a 16-core / 61 GiB host). See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) Finding C for the live-test bisect.
...
[`/charly-check:check`](/recipes/check/check/)). A bare `charly fleet add
...
vm deploy stages what it needs. See [`/charly-core:deploy`](/recipes/core/deploy/) "Deploy-into nesting".)
...
Expected. The agent needs a `virtio-serial` channel that charly's QEMU backend doesn't expose under user-net. Switch to libvirt backend (`charly settings set vm.backend libvirt`) to activate it. See [`/charly-distros:qemu-guest-agent`](/recipes/distros/qemu-guest-agent/).

## Cross-References

- [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) — **VM entity authoring reference** (VmSpec schema, source.kind, adopt pattern)
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — canonical cloud_image VM (BIOS / virtio-gpu / resource sizing / stale BOOTX64.EFI RCA)
- [`/charly-internals:vm-spec`](/recipes/internals/vm-spec/) — Go type reference; validation rules; migration map
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — RenderDomainXML + device emission + passt backend + virtio-gpu
- [`/charly-internals:cloud-init-renderer`](/recipes/internals/cloud-init-renderer/) — RenderCloudInit + composeUsers + seed ISO + charly_install
- [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) — the external vm deploy + SSHExecutor + VmDeployState
- [`/charly-internals:ovmf`](/recipes/internals/ovmf/) — UEFI firmware path resolution; per-VM NVRAM; bios-skips-loader sentinel
- [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) — hard cutover policy
- [`/charly-build:migrate`](/recipes/build/migrate/) — `charly migrate` legacy conversion
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly fleet add vm:<name> <ref>` in-guest layer application
