...
See [`/charly-core:deploy`](/recipes/core/deploy/) "vm: target" for the `charly bundle add vm:<name>` surface and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) for the executor model.
...
`--dev-local-pkg` automatically — see [`/charly-check:check`](/recipes/check/check/)). A bare `charly bundle add
...
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly bundle add vm:<name> <ref>` in-guest layer application
