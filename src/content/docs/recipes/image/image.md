---
title: "image"
description: "The `charly box` command family, box definitions in charly.yml, box inheritance, defaults, platforms, builder configuration, the box dependency graph, or the build/deploy scope boundary."
---
...
neither `base:` nor `from:` is a **LAYER** fragment (see [`/charly-image:layer`](/recipes/image/layer/)).
...
|---|---|---|
| `charly box build` | Build container images from charly.yml | [`/charly-build:build`](/recipes/build/build/) |
| `charly box generate` | Write `.build/` Containerfiles | [`/charly-build:generate`](/recipes/build/generate/) |
| `charly box inspect` | Print resolved image config as JSON | [`/charly-build:inspect`](/recipes/build/inspect/) |
| `charly box list {boxes,candies,targets,services,routes,volumes,aliases}` | List components from charly.yml | [`/charly-build:list`](/recipes/build/list/) |
| `charly box merge` | Merge small layers in a built image | [`/charly-build:merge`](/recipes/build/merge/) |
| `charly box new candy <name>` | Scaffold a new candy directory | [`/charly-build:new`](/recipes/build/new/) |
| `charly box pull` | Fetch an image into local storage | [`/charly-build:pull`](/recipes/build/pull/) |
| `charly check box` | Run declarative tests against a disposable container from a built image (reads the baked plan from the `ai.opencharly.description` OCI label) | [`/charly-check:check`](/recipes/check/check/) |
| `charly box validate` | Check charly.yml + layers | [`/charly-build:validate`](/recipes/build/validate/) |
...
'charly box pull X' to fetch it first."* See [`/charly-build:pull`](/recipes/build/pull/) for the full sentinel
pattern.
...
The error messages are explicit when misconfigured: `cannot chdir to --dir "/missing": no such file or directory`. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) "Deployment: the `charly-mcp` layer" for the full bind-mount pattern and [`/charly-internals:go`](/recipes/internals/go/) "main.go" for the implementation note (guarded by `TestCharlyDir_FlagChdir` + `TestCharlyDir_Errors` in `main_dir_test.go`, and `TestNormalizeRepoSpec` + `TestCharlyRepo_*` in `main_repo_test.go`).
...
| Run build-time tests | `charly check box <image>` | Runs the baked `check:` steps in a disposable `podman run --rm` container (`context: [build]` steps only). For full-stack live check against a running deployment, use `charly check live <name>`. See [`/charly-check:check`](/recipes/check/check/). |
...
VM-related fields (`vm`, `libvirt`) are not valid on `candy:` image entries — the loader rejects them. VMs are declared as `kind: vm` entities in `vm.yml` — see [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) for authoring. `bootc: true` stays on a `candy:` image entry (marks the image as a bootable container); a separate `kind: vm` entity with `source.kind: bootc` references it. A config still carrying the removed on-image `vm:`/`libvirt:` fields predates the schema floor and must be re-authored (see [`/charly-build:migrate`](/recipes/build/migrate/)).
...
The mechanism: a **declarative** fact (what the base image ships, in the embedded `distro.<name>.base_user:` vocabulary — see [`/charly-build:build`](/recipes/build/build/)) + an **image-level policy** (how to reconcile with the image's `user:` field).
...
|---|---|---|---|
| [`/charly-distros:fedora`](/recipes/distros/fedora/) | no | create | `user` |
| [`/charly-distros:arch`](/recipes/distros/arch/) | no | create | `user` |
| [`/charly-distros:debian`](/recipes/distros/debian/) | no | create | `user` |
| [`/charly-distros:ubuntu`](/recipes/distros/ubuntu/) | **yes** (`ubuntu:1000:/home/ubuntu`) | adopt | `ubuntu` |
...
3. Apply `user_policy`:
   ...
   - `create` → no-op.
4. `WriteBootstrap` (`sdk/deploykit/bootstrap.go`, relocated from `charly/generate.go` in #67) keys on `UserAdopted`: adopt emits only a comment; create emits an idempotent `useradd` (see [`/charly-build:generate`](/recipes/build/generate/)).
...
These are the lowest priority in the env resolution chain. CLI flags (`-e`, `--env-file`) and workspace `.env` take precedence. See [`/charly-core:charly-config`](/recipes/core/charly-config/) and [`/charly-core:start`](/recipes/core/start/) for the full priority chain at config-time and run-time respectively.
...
See [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) for the full VmSpec schema, [`/charly-vm:vm`](/recipes/vm/vm/) for the `charly vm build/create/ssh` command family, and [`/charly-build:migrate`](/recipes/build/migrate/) for the schema floor/HEAD gate — the legacy `box.vm:` / `box.libvirt:` fields predate the floor and are no longer auto-converted, so re-author them as a `kind: vm` node.
...
**Host mappings are auto-allocated on `127.0.0.1` at deploy.** At `charly config`, every inherited container port without an explicit deploy pin gets a freshly-allocated free loopback host port (`ResolveDeployPorts`), persisted as `resolved_port:` and stable across `charly update`. A deploy/bed `port:` entry is a PIN (`host:container`) for specific container ports — the rest still auto-allocate. `charly status` shows the live mapping; check probes resolve it via `${HOST_PORT:N}`. See [`/charly-core:deploy`](/recipes/core/deploy/) and [`/charly-check:check`](/recipes/check/check/).

## OCI Labels

Every image `charly` builds carries a set of `ai.opencharly.*` OCI labels embedding the resolved image config so that `charly config` and `charly fleet` can work without the project source tree. The full list is assembled in `sdk/deploykit/write_labels.go` (emission) / read back via `ExtractMetadata` (`spec/container/box_metadata_coneb.go`) — label names in `spec/spec/label_consts.go`, the `spec.BoxMetadata` struct in `spec/schema/boxmetadata.cue` (see [`/charly-internals:capabilities`](/recipes/internals/capabilities/)):
...
- [`/charly-build:build`](/recipes/build/build/) -- `charly box build` (+ the `--no-cache` intermediate scratch-stage caveat)
- [`/charly-build:generate`](/recipes/build/generate/) -- `charly box generate` (Containerfile generation including OCI label emission)
- [`/charly-build:inspect`](/recipes/build/inspect/) -- `charly box inspect` (resolved OCI label set)
- [`/charly-build:list`](/recipes/build/list/) -- `charly box list {boxes,candies,targets,services,routes,volumes,aliases}`
- [`/charly-build:merge`](/recipes/build/merge/) -- `charly box merge` (post-build layer consolidation)
- [`/charly-build:new`](/recipes/build/new/) -- `charly box new candy <name>` (scaffold new candy directory)
- [`/charly-build:pull`](/recipes/build/pull/) -- `charly box pull` (fetch into local storage; `ErrImageNotLocal` recovery)
- [`/charly-build:validate`](/recipes/build/validate/) -- `charly box validate` (charly.yml + candies consistency check)

### Related skills

- [`/charly-image:layer`](/recipes/image/layer/) -- Candy definitions that compose into boxes (env_provide, env_require, env_accept, security resource caps)
- [`/charly-core:deploy`](/recipes/core/deploy/) -- Deploying built images (quadlet, bootc, tunnel lifecycle, instance tunnel inheritance)
- [`/charly-core:charly-config`](/recipes/core/charly-config/) -- `charly config` reads OCI labels + charly.yml; tunnel is charly.yml-only
- [`/charly-internals:go`](/recipes/internals/go/) -- `LoadConfig`, `ExtractMetadata`, `EnsureImage`, `ErrImageNotLocal` source locations
- [`/charly-check:check`](/recipes/check/check/) — Box-level plan steps (cross-candy invariants; deploy-default checks shipped with the image are `check:` steps carrying `context: [deploy]`). The plan steps are embedded in the `ai.opencharly.description` OCI label.
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — if the image transitively bundles an mcp-providing candy (e.g. `jupyter`, `chrome-devtools-mcp`), the bundled candy's `mcp:` tests run as part of `charly check live <image> --filter mcp`; see the skill for per-verb details and the port-publishing gotcha.
- [`/charly-vm:vm`](/recipes/vm/vm/) — `charly vm build/create/start/stop/ssh` command family; reads `vm.yml`, not `charly.yml`. Covers BIOS vs UEFI firmware, virtio-gpu video model, bootc caveats (rootful storage refresh, `-v /dev:/dev` loopback).
- [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) — authoring reference for the `kind: vm` entity schema.
- [`/charly-build:migrate`](/recipes/build/migrate/) — `charly migrate` brings a config up to the current schema (the legacy on-image `vm:`/`libvirt:` fields predate the floor and must be re-authored).
...
A repo reached via two import paths (e.g. `arch`, reached both directly as main → `arch` and transitively as main → `cachyos` → `arch`) — or an import cycle between two projects that import each other — is resolved to a single materialization at load time **by repo identity, not pinned version** — see [`/charly-internals:go`](/recipes/internals/go/) "import-namespace loader". The consequence for authors: **the importing project's namespace pins win**. When an imported namespace's release imports your repo back (`<root>: @…:<someOldPin>`), that back-reference resolves to YOUR local working tree (the root), NOT the old pinned snapshot — so a stale transitive pin in a published submodule release can never drag a divergent (or stale-schema) version of your own repo into the load.
...
**Workflow position:** Pre-build. Define images before building. See also [`/charly-image:layer`](/recipes/image/layer/) (candy authoring), [`/charly-build:build`](/recipes/build/build/) (building).
