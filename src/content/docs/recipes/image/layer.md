---
title: "layer"
description: "Candy authoring, charly.yml, plan steps, pixi.toml, package.json, Cargo.toml, or any file under candy/."
---
...
*Recipe card from the `charly-image` plugin (Kinds — schema authoring).*
...
**IMAGE** — author it per [`/charly-image:image`](/recipes/image/image/). The loader routes a `candy:`
...
model is [`/charly-build:docs`](/recipes/build/docs/); the landing order is [`/charly-internals:git-workflow`](/recipes/internals/git-workflow/) B6.
...
`command:` clone rather than looking for a path escape. Worked example: [`/charly-tools:docs-site`](/recipes/tools/docs-site/)
...
Every editor verb above auto-becomes an MCP tool via Kong reflection (`candy.set`, `candy.add-rpm`, `box.write`, …) so an agent driving `charly mcp serve` can author candies from scratch over RPC without touching the filesystem directly. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) "Authoring tools" for the worked end-to-end example, and [`/charly-build:new`](/recipes/build/new/) for the project / box / candy scaffolders that bootstrap the flow.
...
entries for makepkg `SRCDEST` + yay's clone cache) — see [`/charly-build:build`](/recipes/build/build/).
...
- `write: content:` — the file body is verbatim bytes (never substituted, never heredoc'd). If you need the resolved `${USER}` inside a file, use a `command:` step that writes via shell redirect after discovering the name via `getent passwd 1000 | cut -d: -f1`. Canonical worked example: [`/charly-coder:sshd`](/recipes/coder/sshd/)'s sudoers drop-in.
...
Canonical examples: [`/charly-tools:charly`](/recipes/tools/charly/) (tailscale `debian`→trixie / `ubuntu`→noble),
[`/charly-coder:docker-ce`](/recipes/coder/docker-ce/) (per-version repos) and `candy/gh` (compound shared repo).
...
never list it on any level that distro resolves. Worked example: [`/charly-coder:dev-tools`](/recipes/coder/dev-tools/)
...
the bare-distro tag section. See [`/charly-build:build`](/recipes/build/build/) and [`/charly-internals:install-plan`](/recipes/internals/install-plan/).
...
See [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) for the supervisord ServiceSchemaDef template, [`/charly-build:build`](/recipes/build/build/) for the three-phase template model, and [`/charly-local:local-deploy`](/recipes/local/local-deploy/) for how the host target consumes `service:` entries.
...
- [`/charly-infrastructure:postgresql`](/recipes/infrastructure/postgresql/) — canonical `use_packaged:` entry with drop-in overrides
- [`/charly-ollama:ollama`](/recipes/ollama/ollama/) — single custom entry (common shape)
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — custom entry with complex env and ordering
- [`/charly-coder:sshd`](/recipes/coder/sshd/) — mixed (packaged + custom) within one candy
- [`/charly-infrastructure:virtualization`](/recipes/infrastructure/virtualization/) — mixed entries + per-entry `distro:` filter: modular virtqemud/virtnetworkd on Fedora/Arch vs monolithic libvirtd on Debian/Ubuntu (canonical polymorphism example)
- [`/charly-infrastructure:traefik`](/recipes/infrastructure/traefik/) — multiple custom entries for a multi-service candy
...
Resource caps (memory / cpus) bound the blast radius of a Chrome crash loop on the chrome candy. See [`/charly-selkies:chrome`](/recipes/selkies/chrome/) (Resource Caps) and [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) for the (generic) eventlistener pattern.
...
See [`/charly-core:charly-config`](/recipes/core/charly-config/) (`--update-all` flag, provides filtering) and [`/charly-core:deploy`](/recipes/core/deploy/) (charly.yml `provides:` section) for the full lifecycle.
...
**Data candies** are candies with only `data:` + `volume:` — no packages, no services, no plan steps. Valid standalone. **Data images** (`data_image: true` in charly.yml) are scratch-based — consumed via `charly config --data-from <image>`. See [`/charly-jupyter:notebook-templates`](/recipes/jupyter/notebook-templates/) for a worked example.
...
by the single idempotent `charly migrate` — see [`/charly-build:migrate`](/recipes/build/migrate/).

---

## Cross-References

- [`/charly-image:image`](/recipes/image/image/) — Adding candies to box definitions; box composition; `data_image:` for data-only bundles; the full MCP-first authoring table including `box set`, `box add-candy`, `box rm-candy`, `box write`, `box cat`.
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — "Authoring tools" table exposing `candy.set`, `candy.add-rpm`, `candy.add-deb`, `candy.add-pac`, `candy.add-aur`, `candy.add-apk` as MCP tools; end-to-end build-from-scratch worked example.
- [`/charly-build:generate`](/recipes/build/generate/) — What `charly box generate` actually emits; the per-verb emitter pipeline; `.build/<image>/` layout.
- [`/charly-build:validate`](/recipes/build/validate/) — Validation rules (including per-verb step requirements).
- [`/charly-build:new`](/recipes/build/new/) — Scaffolding a new candy directory.
- [`/charly-build:build`](/recipes/build/build/) — Building images (`--no-cache` caveat; multi-stage scratch).
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — Cross-container `env_provide` / `mcp_provide` injection; `env_require` enforcement; `--update-all`; resource caps.
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly.yml` `provides:` section; tunnel is charly.yml-only.
...
- [`/charly-automation:sidecar`](/recipes/automation/sidecar/) — Sidecars as `env_provide` participants (tailscale `TS_*` filtering).
- [`/charly-build:secrets`](/recipes/build/secrets/) — Credential store chain for `secret_accept` / `secret_require`.
- [`/charly-selkies:chrome`](/recipes/selkies/chrome/) — Canonical consumer of `env_accept` (proxy vars), cgroup resource caps, and a heavy user-phase copy/mkdir step list.
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) — Event listener pattern triggered by resource caps.
- [`/charly-tools:charly`](/recipes/tools/charly/) — The charly-binary candy (composed by every charly-driving box). Paired with [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) which turns any box into an MCP server exposing the full charly CLI.
- [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) — Reference implementation of a meta-layer composition (`candy: [charly, supervisord]` — no install of its own, just wiring) with bind-mounted project directory and `CHARLY_PROJECT_DIR` env-var plumbing.
- [`/charly-jupyter:notebook-templates`](/recipes/jupyter/notebook-templates/) — Data-candy example.
- [`/charly-internals:generate-source`](/recipes/internals/generate-source/) — Internal architecture of the plan-step emission pipeline (Go side).

---

## Cross-kind name reuse

A candy is a top-level **name-first** node, so within a single document the top-level node names are **globally unique**. Cross-FILE name reuse across SEPARATE discovered files IS still permitted: the same identifier (e.g. `charly-cachyos`) MAY exist as a layer at `candy/charly-cachyos/charly.yml` AND an image at `box/charly-cachyos/charly.yml` simultaneously — both are `candy:` nodes (the image carries `base:`/`from:`; there is no `box:` KIND), routed to distinct internal maps (`uf.Candy` vs `uf.Box`). Verbs disambiguate by command context. When `charly fleet add <name>` resolves a ref where both an image AND a layer with that name exist, the image wins (image-first precedence); use `--add-candy <name>` to explicitly select the layer for an overlay. See the project rulebook "cross-FILE cross-kind reuse is fine, but a single document's top-level node names are GLOBALLY UNIQUE" and [`/charly-core:deploy`](/recipes/core/deploy/).
...
**Workflow position:** Pre-build. Author candies before adding them to boxes. See [`/charly-image:image`](/recipes/image/image/) (composition), [`/charly-build:build`](/recipes/build/build/) (building), [`/charly-build:generate`](/recipes/build/generate/) (emission internals).

## Related skills

- [`/charly-build:migrate`](/recipes/build/migrate/) — the single idempotent `charly migrate` brings any legacy config (including the former named data/step child-node shape) to the current schema
- [`/charly-internals:capabilities`](/recipes/internals/capabilities/) — how the `service:` list is baked into the `LabelService` OCI label
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — internal IR the loader feeds into build/deploy pipelines
