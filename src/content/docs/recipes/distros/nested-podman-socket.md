...
description: "Use when a pod's OWN podman store must be reachable as an API socket — the prerequisite for `charly box load`, and for any in-pod service that spawns containers into the candybox rather than the host store."
---
...
Composes `container-nesting` ([`/charly-distros:container-nesting`](/recipes/distros/container-nesting/)) and serves that
...
`charly box load` ([`/charly-build:load`](/recipes/build/load/)) delivering images in, or an in-pod
