---
title: "ubuntu"
description: "Base Ubuntu 24.04 noble image."
---
...
Base Ubuntu 24.04 (noble) image. Distinguished from [`/charly-distros:debian`](/recipes/distros/debian/) by **adopt mode**: the upstream `ubuntu:24.04` base image ships a pre-existing `ubuntu:ubuntu` account at uid 1000, and the embedded `distro.ubuntu` vocabulary declares `base_user:` so the `charly` generator honors that account rather than creating a new one.
...
See [`/charly-image:image`](/recipes/image/image/) "user_policy" and [`/charly-build:build`](/recipes/build/build/) "base_user" for the full table covering all four distros.
...
Adopt mode respects the base image's contract and scales declaratively. See [`/charly-coder:sshd`](/recipes/coder/sshd/) for the `getent passwd 1000` pattern that makes candy content (sudoers in particular) work uniformly across both create and adopt modes.
...
- [`/charly-distros:ubuntu-builder`](/recipes/distros/ubuntu-builder/) — pixi/npm/cargo multi-stage builder.
- [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) — kitchen-sink dev box.
- [`/charly-distros:ubuntu-debootstrap-builder`](/recipes/distros/ubuntu-debootstrap-builder/) — privileged debootstrap builder (`base: debian:13`).
- [`/charly-distros:ubuntu-debootstrap`](/recipes/distros/ubuntu-debootstrap/) — bootstrap-from-scratch rootfs.
- [`/charly-vm:ubuntu-debootstrap-vm`](/recipes/vm/ubuntu-debootstrap-vm/) — the `ubuntu-debootstrap` bootstrap VM + `check-ubuntu-debootstrap-vm` bed.
...
- [`/charly-distros:debian`](/recipes/distros/debian/) — sibling deb-family base without adopt mode (Debian 13 ships no pre-existing uid-1000 user).
- [`/charly-distros:ubuntu-builder`](/recipes/distros/ubuntu-builder/) — multi-stage builder.
- [`/charly-coder:ubuntu-coder`](/recipes/coder/ubuntu-coder/) — kitchen-sink dev box.
- [`/charly-distros:fedora`](/recipes/distros/fedora/) — RPM-family counterpart.
- [`/charly-distros:arch`](/recipes/distros/arch/) — pacman-family counterpart.

## Related commands

- [`/charly-build:build`](/recipes/build/build/) — `base_user:` declaration format, which lives in the embedded `distro.ubuntu` vocabulary.
- [`/charly-image:image`](/recipes/image/image/) — `user_policy:` field + reconciliation.
...
- [`/charly-coder:sshd`](/recipes/coder/sshd/) — `getent passwd 1000`-based sudoers works for both `user` (create) and `ubuntu` (adopt).
- [`/charly-coder:language-runtimes`](/recipes/coder/language-runtimes/) — Microsoft `dotnet-install.sh` (Ubuntu noble doesn't ship dotnet-sdk-9.0 in main; Microsoft's noble apt repo only has 10.0; the dotnet-install.sh `--channel 9.0` is the cross-distro solution).
