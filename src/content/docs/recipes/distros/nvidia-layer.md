---
title: "nvidia-layer"
description: "NVIDIA GPU runtime support: driver libs, nvidia-container-toolkit (CDI), and VA-API."
---
...
See [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) (Hardware Detection) for the detection probe and [`/charly-distros:rocm`](/recipes/distros/rocm/) for the AMD-side counterpart using the same mechanism.
...
- [`/charly-distros:nvidia`](/recipes/distros/nvidia/) — Fedora NVIDIA GPU base image (nvidia + cuda candies)
- [`/charly-distros:cachyos`](/recipes/distros/cachyos/) — `cachyos.nvidia`, the CachyOS GPU base (cachyos + agent-forwarding + nvidia + cuda); the nvidia/cuda candies being multi-distro (rpm + pac) is what lets this Arch/CachyOS GPU base reuse them unchanged
- [`/charly-coder:charly-arch`](/recipes/coder/charly-arch/) — Arch Linux charly toolchain (shared candies + nvidia)
- [`/charly-distros:charly-fedora`](/recipes/distros/charly-fedora/) — Fedora charly toolchain (shared candies + nvidia)

## Related Candies

- [`/charly-distros:cuda`](/recipes/distros/cuda/) — CUDA development toolkit (depends on nvidia)
- [`/charly-distros:rocm`](/recipes/distros/rocm/) — AMD GPU counterpart (ROCm runtime + OpenCL), uses the same `appendAutoDetectedEnv()` DRINODE injection
- [`/charly-selkies:selkies`](/recipes/selkies/selkies/) — Primary consumer of the DRINODE env for VAAPI H.264 encode
- [`/charly-languages:python-ml`](/recipes/languages/python-ml/), [`/charly-jupyter:llama-cpp`](/recipes/jupyter/llama-cpp/), [`/charly-jupyter:jupyter-ml`](/recipes/jupyter/jupyter-ml/) — CUDA ML stacks that depend on this candy

## Related Commands

- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) — Host NVIDIA detection (GPU probe, CDI spec status, driver version)
- [`/charly-core:shell`](/recipes/core/shell/) — DRINODE auto-injection applies to interactive shells too
- [`/charly-automation:udev`](/recipes/automation/udev/) — Device permission management for `/dev/dri/*` and `/dev/nvidia*`
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — Runtime GPU device injection at deployment time (same `appendAutoDetectedEnv()` path)
- [`/charly-core:start`](/recipes/core/start/) — Runtime GPU device injection at service start time
