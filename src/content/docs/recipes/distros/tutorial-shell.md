...
description: "The teaching box quoted end to end by opencharly.ai — a minimal, real dev shell composing one tool candy, one service candy, and the container init."
---
...
| Candy | Teaches |
|---|---|
| [`/charly-tools:ripgrep`](/recipes/tools/ripgrep/) | a **tool** candy — packages plus deterministic probes, no service |
| [`/charly-coder:sshd`](/recipes/coder/sshd/) | a **service** candy, and the canonical init-polymorphism example: ONE `service:` list carrying both a `use_packaged:` systemd form and a custom `exec:` supervisord form |
| [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) | the container **init** that actually runs the service |
...
- [`/charly-distros:fedora`](/recipes/distros/fedora/) — the parent base image
- [`/charly-distros:charly-fedora`](/recipes/distros/charly-fedora/) — the next box up in size: the same shape plus the charly toolchain
- [`/charly-coder:fedora-coder`](/recipes/coder/fedora-coder/) — the kitchen-sink end of the same spectrum (~32 candies)

## Related Commands

- [`/charly-build:build`](/recipes/build/build/) — build the box
- [`/charly-check:check`](/recipes/check/check/) — the bed model and `charly check run`
...
- [`/charly-image:image`](/recipes/image/image/) — image family umbrella (`candy:` entries carrying `base:`/`from:`)
- [`/charly-build:docs`](/recipes/build/docs/) — the generator that publishes this card and the box's reference page
