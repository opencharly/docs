...
description: "Mounts a virtiofs share tagged `workspace` at /workspace inside a VM guest via a systemd .mount unit."
---
...
[`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/)), so the entity declares only the filesystem.
...
- [`/charly-internals:libvirt-renderer`](/recipes/internals/libvirt-renderer/) — `mapFilesystem` + `ensureVirtiofsSharedMemory`
- [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) — `filesystems:` authoring on the kind:vm entity
- [`/charly-vm:cachyos-bootstrap-vm`](/recipes/vm/cachyos-bootstrap-vm/) — the CachyOS VM family that consumes it
