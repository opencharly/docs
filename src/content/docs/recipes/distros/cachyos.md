---
title: "cachyos"
description: "CachyOS base image (docker.io/cachyos/cachyos-v3) — x86_64_v3-optimized Arch derivative."
---
...
[`/charly-internals:go`](/recipes/internals/go/) "import-namespace loader"). The image DAG
...
- Candies author AUR packages under `distro.arch.aur.package` (see
  [`/charly-image:layer`](/recipes/image/layer/) "AUR"); the `arch` distro tag is what cachyos boxes match
...
- [`/charly-distros:cachyos-pacstrap-builder`](/recipes/distros/cachyos-pacstrap-builder/) — privileged pacstrap builder (`base: arch.arch`)
- [`/charly-distros:cachyos-pacstrap`](/recipes/distros/cachyos-pacstrap/) — bootstrap-from-scratch rootfs (builds end-to-end)
- [`/charly-vm:cachyos-bootstrap-vm`](/recipes/vm/cachyos-bootstrap-vm/) — bootstrap VM (`cachyos-vm`) + `check-cachyos-vm` check bed
- [`/charly-local:charly-cachyos`](/recipes/local/charly-cachyos/) — the operator CachyOS workstation profile
- [`/charly-versa:versa`](/recipes/versa/versa/) — CachyOS-rooted notebook/OSM image in this submodule (`base: cachyos`)
...
variant ([`/charly-distros:cachyos-pacstrap`](/recipes/distros/cachyos-pacstrap/)) is retained for offline/air-gapped
...
- [`/charly-distros:arch`](/recipes/distros/arch/) — the Arch base (`cachyos-pacstrap-builder` is `base: arch.arch`, via the `arch` import namespace)
- [`/charly-image:image`](/recipes/image/image/) — box family umbrella (composition, build/validate/inspect)
- [`/charly-internals:cutover-policy`](/recipes/internals/cutover-policy/) — the hard-cutover policy governing submodule splits
