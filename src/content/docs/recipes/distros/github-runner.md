---
title: "github-runner"
description: "GitHub Actions self-hosted runner as a supervised container service — rootless (uid 1000), Arch/CachyOS packages, credential-backed registration token."
---
...
- charly-host `depends=` completion: `slirp4netns`, `libisoburn`, `cdrtools`, `swtpm`
  ...
  shells out to `sudo pacman`). See [`/charly-distros:githubrunner`](/recipes/distros/githubrunner/) "CI: builds the
...
- [`/charly-distros:githubrunner`](/recipes/distros/githubrunner/)
...
- [`/charly-image:layer`](/recipes/image/layer/) — candy authoring reference (tasks, vars, secret_accept, check block syntax)
- [`/charly-check:check`](/recipes/check/check/) — declarative testing framework for the `check:` block + the `check-githubrunner-pod` bed
