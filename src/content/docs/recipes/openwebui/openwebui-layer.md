...
*Recipe card from the `charly-openwebui` plugin (Images — the deployable catalog).*
...
- [`/charly-languages:python`](/recipes/languages/python/) -- Python runtime dependency
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) -- process manager dependency
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) -- alternative AI agent with similar MCP/LLM auto-config pattern

## Related Commands

- [`/charly-core:charly-config`](/recipes/core/charly-config/) -- `charly config openwebui --update-all` for service discovery
- [`/charly-build:secrets`](/recipes/build/secrets/) -- `charly secrets` for Secret Service / GPG credential management
- [`/charly-core:service`](/recipes/core/service/) -- `charly service status openwebui` for runtime management
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) -- probe the MCP servers openwebui consumes (auto-configured into `TOOL_SERVER_CONNECTIONS` from `CHARLY_MCP_SERVERS`): `charly check live <provider-image> --filter mcp` shows what tools openwebui will see and verifies liveness before debugging openwebui itself.

## Related Boxes

- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) -- the deployed box
- [`/charly-jupyter:jupyter`](/recipes/jupyter/jupyter/) -- deploy alongside for MCP notebook access and code execution
- [`/charly-ollama:ollama`](/recipes/ollama/ollama/) -- deploy alongside for local LLM inference
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) -- alternative AI frontend (CLI-based agent vs web UI)
