---
title: "openwebui"
description: "Open WebUI box with auto-configured LLM providers, MCP servers, and Jupyter on port 8080."
---
...
- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) — Auto-config entrypoint, secrets, env_accept, TOOL_SERVER_CONNECTIONS format
- [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) — SSH/GPG forwarding

## Related Boxes

- [`/charly-jupyter:jupyter`](/recipes/jupyter/jupyter/) — deploy alongside for MCP notebooks and code execution
- [`/charly-ollama:ollama`](/recipes/ollama/ollama/) — deploy alongside for local LLM inference
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — alternative AI frontend (CLI agent vs web UI)
- [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) — deploy alongside for shared Chrome browser
...
email env var injected. See [`/charly-check:check`](/recipes/check/check/) for the framework.

## Related Skills

- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) — candy authoring
- [`/charly-check:check`](/recipes/check/check/) — declarative testing framework
- [`/charly-build:secrets`](/recipes/build/secrets/) — WEBUI_ADMIN_PASSWORD + provider API keys
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — `-e WEBUI_ADMIN_EMAIL=...` deploy-time env setup
