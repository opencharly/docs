...
- There is **no** field for pointing a candy at another directory. (An older revision of
  [`/charly-image:layer`](/recipes/image/layer/) documented a `directory:` field for exactly this; that field does not
...
- [`/charly-build:docs`](/recipes/build/docs/) — the `charly docs generate` verb that produces the content this builds.
- [`/charly-coder:nodejs`](/recipes/coder/nodejs/) — the node runtime this candy requires.
- [`/charly-check:check`](/recipes/check/check/) — the bed model and the check-verb catalog.
