---
title: "charly"
description: "OpenCharly CLI (charly) binary installed into container/VM images for in-container use."
---
...
[`/charly-internals:install-plan`](/recipes/internals/install-plan/) "Check-vs-production charly toolchain". This is
...
- [`/charly-infrastructure:virtualization`](/recipes/infrastructure/virtualization/), [`/charly-infrastructure:gocryptfs`](/recipes/infrastructure/gocryptfs/), [`/charly-infrastructure:socat`](/recipes/infrastructure/socat/) -- the candies the `charly` candy composes alongside the binary to form the full toolchain
- [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) -- candies: [charly, supervisord] meta-composition that deploys `charly mcp serve` (~192-tool MCP gateway) with a `/workspace` bind mount (volume NAME `project`) for build-mode tools + auto-fallback to opencharly/charly when nothing is bound
