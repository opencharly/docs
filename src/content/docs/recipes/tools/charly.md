...
- [`/charly-coder:charly-mcp`](/recipes/coder/charly-mcp/) -- candies: [charly, supervisord] meta-composition that deploys `charly mcp serve` (the MCP gateway) with a `/workspace` bind mount (volume NAME `project`) for build-mode tools + auto-fallback to opencharly/charly when nothing is bound
