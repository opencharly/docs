---
title: "cue"
description: "The CUE data-validation / configuration CLI (cue), pinned to v0.16.1."
---
...
- running the offline schema-vendoring pipeline (`cue import jsonschema:`,
  ...
  [`/charly-internals:egress`](/recipes/internals/egress/);
...
- [`/charly-internals:egress`](/recipes/internals/egress/) — the egress-validation subsystem the cue CLI feeds (the vendoring pipeline, `candy/plugin-egress/egress-schemas/vendor/`).
- [`/charly-build:validate`](/recipes/build/validate/) — `charly box validate` (the ingress side; uses the embedded library, not this CLI).
- [`/charly-image:layer`](/recipes/image/layer/) — the `download:` verb the candy uses.
