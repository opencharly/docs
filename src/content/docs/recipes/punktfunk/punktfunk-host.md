...
description: "The punktfunk streaming host — the `punktfunk/1` QUIC host daemon, its browser console and its plugin runner, installed from unom's signed pacman repo on Arch/CachyOS."
---
...
Sending the whole line as a bearer yields a clean HTTP 401. [`/charly-check:punktfunk`](/recipes/check/punktfunk/)
parses it.
...
- [`/charly-check:punktfunk`](/recipes/check/punktfunk/) — the `punktfunk:` verb that probes and manages a running host
- [`/charly-selkies:sway`](/recipes/selkies/sway/) — the compositor the container venue depends on
- [`/charly-check:check`](/recipes/check/check/) — beds, `disposable: true`, the R10 sequence
- [`/charly-image:layer`](/recipes/image/layer/) — candy authoring (the `distro:` cascade, `service:` forms)
