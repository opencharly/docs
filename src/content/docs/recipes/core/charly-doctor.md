---
title: "charly-doctor"
description: "Host dependency checker and hardware detector for the `charly doctor` CLI verb."
---
...
- [`/charly-core:shell`](/recipes/core/shell/) — auto-detected env vars (DRINODE, DRI_NODE, HSA_OVERRIDE_GFX_VERSION) injected via the same `appendAutoDetectedEnv()` path
- [`/charly-core:start`](/recipes/core/start/) — same auto-injection path at service-start time
- [`/charly-distros:nvidia`](/recipes/distros/nvidia/) — NVIDIA GPU runtime support + DRINODE Auto-Injection section
- [`/charly-distros:rocm`](/recipes/distros/rocm/) — AMD ROCm runtime support + DRINODE/HSA_OVERRIDE_GFX_VERSION auto-detect table
- [`/charly-selkies:selkies`](/recipes/selkies/selkies/) — Primary consumer of DRINODE for VAAPI H.264 encode
