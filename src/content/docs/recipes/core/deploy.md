...
1. **Execution verbs** — `charly fleet add <name>` / `charly fleet del <name>`. Apply or reverse a deployment. The target is dispatched by which substrate node the deploy carries:
   ...
   - `k8s: {from: <name>}` → Kustomize base/overlays tree. See [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/).
...
dotted-path `charly fleet add pod.device`). See [`/charly-check:android`](/recipes/check/android/). K8s-specific choices (storage class, ingress class, cert issuer, secret backend) live in a **cluster profile** file (`~/.config/charly/clusters/<name>.yaml` or in-repo `clusters/<name>.yaml`), *not* in the deployment. This means one deployment spec targets dev/staging/prod clusters with zero schema changes — only the cluster profile differs.
...
For service lifecycle commands (start/stop/status/logs/update/remove), see [`/charly-core:service`](/recipes/core/service/). For VM lifecycle (build/create/start/stop/ssh), see [`/charly-vm:vm`](/recipes/vm/vm/); for in-VM layer deploys via `charly fleet add vm:<name>`, see the "VM target" section below and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/). For encrypted storage, see [`/charly-automation:enc`](/recipes/automation/enc/). For host-target semantics, see [`/charly-local:local-deploy`](/recipes/local/local-deploy/). For Kubernetes targets, see [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/). For the Go IR that drives all four targets, see [`/charly-internals:install-plan`](/recipes/internals/install-plan/).
...
- **`k8s: {from: <name>}`** — emit a Kustomize base/overlays tree. See [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/).
...
The loader raises a hard load-time error on obsolete deploy names (the retired `qc` / `cachyos-dx` keys) and on any obsolete root shape (a `kind: deployment` / `deployment:` / `kind: deploy` / `deploy:` / `bundle:` document — the kind is now the name-first substrate-node (`pod:`/`vm:`/`k8s:`/`local:`/`android:`/`group:`) discriminator); every such error points at `charly migrate`. See [`/charly-build:migrate`](/recipes/build/migrate/).
