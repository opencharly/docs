...
The loader raises a hard load-time error on obsolete deploy names (the retired `qc` / `cachyos-dx` keys) and on any obsolete root shape (a `kind: deployment` / `deployment:` / `kind: deploy` / `deploy:` / `fleet:` document — the kind is now the name-first substrate-node (`pod:`/`vm:`/`k8s:`/`local:`/`android:`/`group:`) discriminator); every such error points at `charly migrate`. See [`/charly-build:migrate`](/recipes/build/migrate/).
