---
title: "deploy"
description: "MUST be invoked before any work involving: `charly fleet add`/`charly fleet del` commands, quadlet generation, volume backing, tunnels (Tailscale/Cloudflare), `add_candy:` overlay, or per-machine deploy overlays."
---
...
1. **Execution verbs** — `charly fleet add <name>` / `charly fleet del <name>`. Apply or reverse a deployment. The target is dispatched by which substrate node the deploy carries:
   - `local: {from: <template>}` → the external `deploy:local` plugin (`candy/plugin-deploy-local`) applies to the local filesystem over the executor reverse channel (or, placed under another resource node via tree position, via NestedExecutor into the enclosing deployment). See [`/charly-local:local-deploy`](/recipes/local/local-deploy/).
   - `vm: {from: <entity>}` → the external `deploy:vm` plugin (`candy/plugin-deploy-vm`) applies INSIDE a running VM over SSH via the reverse channel; the plugin auto-boots the venue in its `OpPrepareVenue` (via `HostBuild("cli")`), using the host prepare hook's resolved data. See "VM target" section below and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/).
   ...
   - `kubernetes: {from: <name>}` → Kustomize base/overlays tree. See [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/).
...
A deploy's install timeline can include a `run:` step carrying a plugin verb (the `<word>: <input>` sugar — see [`/charly-image:layer`](/recipes/image/layer/) "Plugin verb steps"). When the verb is served by an EXTERNAL (out-of-process) plugin, that step EXECUTES at deploy on a `local:`/`vm:` target — the plugin runs its effect on the live venue over the executor reverse channel (`Invoke(OpExecute)`) and returns teardown `ReverseOp`s the target records to the ledger and replays at `charly fleet del`. A builtin `ProvisionActor` verb keeps its in-proc shell path; the two are placement-agnostic (same authored step, the runtime picks the path). Detail → [`/charly-internals:install-plan`](/recipes/internals/install-plan/) (the `ExternalPluginStep` IR kind) + [`/charly-internals:plugin`](/recipes/internals/plugin/).
...
dotted-path `charly fleet add pod.device`). See [`/charly-check:android`](/recipes/check/android/). Kubernetes-specific choices (storage class, ingress class, cert issuer, secret backend) live in a **cluster profile** file (`~/.config/charly/clusters/<name>.yaml` or in-repo `clusters/<name>.yaml`), *not* in the deployment. This means one deployment spec targets dev/staging/prod clusters with zero schema changes — only the cluster profile differs.
...
| Apply local deploy | `charly fleet add <name> <ref>` (entry with a `local:` substrate node) | Apply layers directly to local filesystem (see [`/charly-local:local-deploy`](/recipes/local/local-deploy/)) |
...
For service lifecycle commands (start/stop/status/logs/update/remove), see [`/charly-core:service`](/recipes/core/service/). For VM lifecycle (build/create/start/stop/ssh), see [`/charly-vm:vm`](/recipes/vm/vm/); for in-VM layer deploys via `charly fleet add vm:<name>`, see the "VM target" section below and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/). For encrypted storage, see [`/charly-automation:enc`](/recipes/automation/enc/). For host-target semantics, see [`/charly-local:local-deploy`](/recipes/local/local-deploy/). For Kubernetes targets, see [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/). For the Go IR that drives all five targets, see [`/charly-internals:install-plan`](/recipes/internals/install-plan/).
...
- **`kubernetes: {from: <name>}`** — emit a Kustomize base/overlays tree. See [`/charly-kubernetes:kubernetes`](/recipes/kubernetes/kubernetes/).
...
Reverses a deployment. Gated host-side reversal respects `--keep-repo-changes` and `--keep-services`. Container teardown: `podman stop` + `rm` + overlay image removal (unless `--keep-image`) + ledger cleanup. VM teardown: SSH-executed ReverseOps in the guest, preserving the VM itself (use `charly vm destroy` separately). See [`/charly-local:local-deploy`](/recipes/local/local-deploy/) for the full 15-kind ReverseOp table.

## VM target: `charly fleet add vm:<vm-name> <ref>`

Applies layer recipes **inside a running VM** over SSH. Same `InstallPlan` IR as host and container targets; the difference is that bash bodies run via `ssh guest 'sudo bash -s'` through an `SSHExecutor` (see [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/)).
...
This is the same merge semantics as a `local:` deploy — just with SSH-wrapped execution. See [`/charly-internals:install-plan`](/recipes/internals/install-plan/) for the compiler and [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) for the execution model.
...
See [`/charly-vm:vms-catalog`](/recipes/vm/vms-catalog/) for vm-entity authoring, [`/charly-vm:vm`](/recipes/vm/vm/) for the lifecycle commands, [`/charly-internals:vm-deploy-target`](/recipes/internals/vm-deploy-target/) for the Emit flow.
...
declare no `port:` — see [`/charly-image:image`](/recipes/image/image/) "Ports"). At `charly config`,
...
Service name: `charly-<image>.service`. Container name: `charly-<image>`. Entrypoint: determined by the embedded `init:` vocabulary for the configured init system. Encrypted volumes are mounted via `ExecStartPre=charly config mount` in the quadlet, which creates transient `charly-enc-<image>-<volume>.scope` units for each encrypted volume. These scope units are independent of the container service — they survive stop/restart (see [`/charly-automation:enc`](/recipes/automation/enc/)). The boot gate is the **secret backend**: with a keyring-class backend (`keyring`, `auto`, or unset) the deploy starts at boot, and `ExecStartPre` blocks in the mount until the keyring unlocks (`TimeoutStartSec=0`, so it waits indefinitely rather than failing); with `secret_backend: config` there is no `WantedBy=default.target` and the deploy needs an explicit `charly start` — a security decision recorded at sdk#121, because a passphrase stored in cleartext must not be auto-mounted, unattended, at every boot.
...
See [`/charly-image:layer`](/recipes/image/layer/) for port protocol syntax in `charly.yml`.
...
Port protocols are stored in the `ai.opencharly.port_proto` image label so deploy-mode commands work without access to the original layer definitions. Remote refs require `charly box pull` first — see [`/charly-build:pull`](/recipes/build/pull/).
...
`charly status` surfaces this as a non-fatal warning (graceful degradation falls back to image-label-driven display); the strictly-charly.yml-driven verbs (`charly fleet show`, `charly config status`, `charly start`) hard-fail. Run `charly migrate` to convert in place — it backs the original up to `<file>.bak.<unix-ts>` and rewrites to the latest schema. See [`/charly-build:migrate`](/recipes/build/migrate/) "charly migrate".
...
- `memory_max` → `MemoryMax=6G` (lowercase `g` is auto-normalized to `G` because systemd parses lowercase as `infinity` — see [`/charly-selkies:chrome`](/recipes/selkies/chrome/) gotcha)
...
**Canonical consumer:** the chrome candy's cgroup caps. See [`/charly-selkies:chrome`](/recipes/selkies/chrome/) (Resource Caps) — the caps bound a Chrome crash loop's blast radius; a wedged loop (orphan memfd shmem) is cleared by restarting the container, which tears down the cgroup. See [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) (Event Listeners) for the eventlistener pattern in general and [`/charly-image:layer`](/recipes/image/layer/) (Security Declaration) for the authoring side.
...
See [`/charly-image:layer`](/recipes/image/layer/) for `env_provide`/`mcp_provide` field declarations and [`/charly-core:charly-config`](/recipes/core/charly-config/) for `--update-all` propagation.
...
**Local-storage requirement.** Because deploy-mode commands read OCI labels directly from local container storage (via `ExtractMetadata` → `podman inspect`), the image must be pulled first. If it isn't, the command fails with `ErrImageNotLocal` and the CLI suggests `charly box pull`. See [`/charly-build:pull`](/recipes/build/pull/) for the sentinel pattern and remote-ref (`@github.com/...`) handling.
...
- **Data provisioning**: `charly config` automatically provisions data from data candies into bind-backed volumes (via `--seed`, default true). `charly update` merges new data non-destructively. See [`/charly-core:charly-config`](/recipes/core/charly-config/) and [`/charly-core:charly-update`](/recipes/core/charly-update/)
...
- **`charly remove --purge`**: removes named volumes (also encrypted volumes + the `<name>-overlay` images — see [`/charly-core:remove`](/recipes/core/remove/))
...
See [`/charly-check:vnc`](/recipes/check/vnc/) for full VNC authentication documentation.
...
Requires the `socat` layer as a dependency. The relay runs as a `relay-<port>` service in the configured init system. See [`/charly-openclaw:openclaw`](/recipes/openclaw/openclaw/) for an example.

**Chrome CDP exception:** Chrome DevTools no longer uses `port_relay`. Chrome 146+ rejects connections with non-localhost Host headers, so a simple socat relay is insufficient. Instead, Chrome uses a `cdp-proxy` Python supervisord service that listens on `0.0.0.0:9222`, forwards to Chrome on `127.0.0.1:9223` with Host header rewriting, and rewrites response URLs (e.g., `webSocketDebuggerUrl`) with Content-Length correction. See [`/charly-selkies:chrome`](/recipes/selkies/chrome/) and [`/charly-check:cdp`](/recipes/check/cdp/) for details.
...
See [`/charly-core:charly-config`](/recipes/core/charly-config/) for setup workflow and [`/charly-image:layer`](/recipes/image/layer/) for declaration format.

## Sidecar Pod Deployment

When sidecars are attached via `charly config --sidecar <name>`, deployment generates a Podman **pod** instead of a standalone container. See [`/charly-automation:sidecar`](/recipes/automation/sidecar/) for full sidecar documentation.
...
- **`requires_shared:` — the REFCOUNTED claimant** (mutually exclusive with
  ...
  **vfio→nvidia+CDI** at the first claimant's deploy and back **nvidia→vfio** only
  ...
  claim is still preempted first. Full GPU-mode-switch model: [`/charly-vm:vm`](/recipes/vm/vm/) +
  [`/charly-internals:disposable`](/recipes/internals/disposable/); the bed surface: [`/charly-check:check`](/recipes/check/check/).
- **Standing authorization — preempt autonomously.** You may trigger preemption
  ...
  non-preemptible resource. See [`/charly-internals:disposable`](/recipes/internals/disposable/) "Standing
  authorization".
...
- **GPU auto-allocation** — when a token ALSO carries an embedded `resource:`
  ...
  double-inject). See [`/charly-internals:disposable`](/recipes/internals/disposable/) "resource-arbitration axis"
  + [`/charly-build:build`](/recipes/build/build/) `resource:`.
...
- **Orthogonal to disposable/ephemeral** — no derivation either way; a deploy may
  be both preemptible and disposable. Full reference: [`/charly-internals:disposable`](/recipes/internals/disposable/)
...
addresses the subject via `${HOST:<subject>}` (see [`/charly-check:check`](/recipes/check/check/)
...
- **Disposability is inherited, never invented.** `foldMembers` promotes a member to
  ...
  [`/charly-internals:disposable`](/recipes/internals/disposable/)).
...
- **Addressing the subject from a driven probe** uses the unified `${HOST:...}` variable —
  ...
  `:<port>` → host-reachable endpoint) — see [`/charly-check:check`](/recipes/check/check/) "Cross-deployment
  probing".

## Cross-References

**Deploy surface:**
- [`/charly-local:local-deploy`](/recipes/local/local-deploy/) — Local-target execution model: the external `deploy:local` plugin, ledger, gates, ReverseOp kinds, sudo batching
- [`/charly-internals:install-plan`](/recipes/internals/install-plan/) — The InstallPlan IR shared by the deploy targets: ALL FIVE substrates (local/vm/pod/kubernetes/android) are external out-of-process deploys via plugins over the executor reverse channel — local/vm consume the IR via `kit.WalkPlans` (vm's walk runs inside the guest); pod's plugin walks nothing — its candy `plugin-deploy-pod` lifecycle builds the overlay host-side: the core `overlay` host-builder runs the prep+resolve M-seam, the candy renders the overlay in its own code via `deploykit.OCITarget` + `deploykit.NewRenderGeneratorFromProject` (`add_candy:` synthesis, P11c); `charly box build` itself emits via the separate WriteCandySteps → EmitTasks generator in `sdk/deploykit` (relocated in #67), not the IR
- [`/charly-internals:local-infra`](/recipes/internals/local-infra/) — Supporting Go files for local deploys: hostdistro, ledger, builder_run, shell_profile, reverse_ops, `sdk/deploykit/compile_service_steps.go` (service_render, relocated), `candy/plugin-fleet/deploy_ref.go` (deploy_ref, relocated)
...
- [`/charly-automation:sidecar`](/recipes/automation/sidecar/) — Sidecar containers, pod networking, Tailscale exit nodes, Environment Contract (provides filtering)
- [`/charly-core:service`](/recipes/core/service/) — Service lifecycle (start/stop/update/remove)
- [`/charly-core:start`](/recipes/core/start/) — Ergonomic alias for `charly fleet add <image> <image>` (container target)
- [`/charly-core:stop`](/recipes/core/stop/) — Ergonomic alias for `charly fleet del <name>`
- [`/charly-core:charly-update`](/recipes/core/charly-update/) — Per-instance update pattern; equivalent to `charly fleet add <name> --pull`
- [`/charly-core:charly-config`](/recipes/core/charly-config/) — Resource cap flags (`--memory-max/high/swap/cpus`), provides filtering, env_require enforcement, NO_PROXY auto-enrichment, `--sidecar`, `-i` instance support, MCP name disambiguation
- [`/charly-automation:enc`](/recipes/automation/enc/) — Encrypted storage commands (charly config mount/unmount)
- [`/charly-check:vnc`](/recipes/check/vnc/) — VNC password setup for desktop containers
- [`/charly-vm:vm`](/recipes/vm/vm/) — Virtual machine deployment (charly vm)
- [`/charly-build:build`](/recipes/build/build/) — Building images before deployment (+ the `--no-cache` intermediate scratch-stage caveat)
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — verify the MCP endpoints declared by `provides.mcp:` entries are actually reachable (`charly check live <image> --filter mcp`); note the **port-publishing gotcha** when a `port:` override in charly.yml predates a newly-added mcp-providing layer
- [`/charly-image:image`](/recipes/image/image/) — Image configuration, OCI label emission, `labels.go:238` tunnel read-skip
- [`/charly-image:layer`](/recipes/image/layer/) — Unified `service:` schema (use_packaged + structured custom), `env_provide`/`env_require`/`env_accept` field declarations, security resource caps
- [`/charly-check:check`](/recipes/check/check/) — Local `check:` in charly.yml overlays image-baked deploy defaults: entries with matching `id:` replace, otherwise append. `id: X, skip: true` disables a baked check without a replacement.

**Canonical layer worked examples:**
- [`/charly-selkies:chrome`](/recipes/selkies/chrome/) — cgroup resource-caps consumer (caps bound a Chrome crash loop)
- [`/charly-infrastructure:supervisord`](/recipes/infrastructure/supervisord/) — Event listener pattern triggered by the caps; ServiceSchemaDef that renders `service:` entries to supervisord INI
- [`/charly-infrastructure:postgresql`](/recipes/infrastructure/postgresql/) — Canonical `use_packaged:` entry (packaged unit reuse)
- [`/charly-ollama:ollama`](/recipes/ollama/ollama/), [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — Custom `service:` entries
- [`/charly-selkies:selkies-labwc`](/recipes/selkies/selkies-labwc/) — Multi-instance proxy deployment, tunnel inheritance workaround
...
The loader raises a hard load-time error on obsolete deploy names (the retired `qc` / `cachyos-dx` keys) and on any obsolete root shape (a `kind: deployment` / `deployment:` / `kind: deploy` / `deploy:` / `fleet:` document — the kind is now the name-first substrate-node (`pod:`/`vm:`/`kubernetes:`/`local:`/`android:`/`group:`) discriminator); every such error points at `charly migrate`. See [`/charly-build:migrate`](/recipes/build/migrate/).
...
**Workflow position:** After [`/charly-build:build`](/recipes/build/build/), before [`/charly-core:service`](/recipes/core/service/).
Previous step: [`/charly-build:build`](/recipes/build/build/) (build the image). Next step: [`/charly-core:service`](/recipes/core/service/) (start, status, logs).
