---
title: "ssh"
description: "Generic SSH support for Charly — host-alias remote execution or `charly ssh tunnel` access to remote SPICE and VNC endpoints."
---
...
fd-passing, with zero charly involvement. See [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/).
...
- [`/charly-vm:arch-cloud-vm`](/recipes/vm/arch-cloud-vm/) — "Connecting from a remote workstation" —
  the canonical worked example across all three paths.
- [`/charly-build:settings`](/recipes/build/settings/) — `hosts.<alias>` key schema.
- [`/charly-check:spice`](/recipes/check/spice/) — the declarative `spice:` check verb; the host resolves the VM's SPICE endpoint (honoring `CHARLY_LIBVIRT_URI` for a remote hypervisor).
- [`/charly-check:libvirt`](/recipes/check/libvirt/) — the declarative `libvirt:` check verb served out-of-process by candy/plugin-vm; honors `CHARLY_LIBVIRT_URI` for a remote hypervisor.
- [`/charly-check:vnc`](/recipes/check/vnc/) — the declarative `vnc:` check verb (served out-of-process by candy/plugin-vnc); the host pre-resolves the RFB endpoint for pod AND vm targets (a VM's libvirt VNC via bridge/tunnel).
