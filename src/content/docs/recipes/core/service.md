---
title: "service"
description: "MUST be invoked before any work involving: charly start/stop/status/logs/update/remove commands, charly config (deployment), init system service management, or container lifecycle."
---
...
- [`/charly-core:shell`](/recipes/core/shell/) -- Interactive shells and exec into running containers
- [`/charly-core:deploy`](/recipes/core/deploy/) -- Quadlet generation details, tunnels, volume backing
- [`/charly-automation:enc`](/recipes/automation/enc/) -- Encrypted storage (mounted inline by charly start)
- [`/charly-core:charly-config`](/recipes/core/charly-config/) -- `run_mode`, `auto_enable`, `engine.run` settings
- [`/charly-check:cdp`](/recipes/check/cdp/) -- the `cdp:` check verb (`cdp: status`, served out-of-process by candy/plugin-cdp)
- [`/charly-check:vnc`](/recipes/check/vnc/) -- the `vnc:` check verb (`vnc: status`, served out-of-process by candy/plugin-vnc)
- [`/charly-check:wl`](/recipes/check/wl/) -- Desktop automation + the `wl:` verb's sway-* methods (e.g. a `wl: sway-tree` step)
- [`/charly-check:wl`](/recipes/check/wl/) -- the `wl: status` method (tool availability check)
...
**Workflow position:** After [`/charly-build:build`](/recipes/build/build/) and [`/charly-core:deploy`](/recipes/core/deploy/). This skill covers the runtime lifecycle.
Previous step: [`/charly-core:deploy`](/recipes/core/deploy/) (quadlet generation, tunnels). Next step: per-pod plugin (`/charly-jupyter:<name>`, `/charly-coder:<name>`, etc.) or `/charly-distros:<name> / /charly-languages:<name> / /charly-infrastructure:<name> / /charly-tools:<name>` for verification.
