---
title: "shell"
description: "`charly shell`, interactive shells, command execution in containers, workspace mounts, TTY allocation, or port relay."
---
...
then `charly shell <image-name>` works via labels. See [`/charly-build:pull`](/recipes/build/pull/).
...
```
See [`/charly-build:pull`](/recipes/build/pull/) for the full sentinel pattern.
...
then invoke `charly shell <short-name>`. See [`/charly-build:pull`](/recipes/build/pull/) for the full remote-ref
workflow.
...
- [`/charly-build:pull`](/recipes/build/pull/) -- **Required** before `charly shell` can work on a fresh host. Fetches the image into local storage so `ExtractMetadata` can read its OCI labels. Handles remote refs (`@github.com/...`) that `charly shell` itself rejects.

### Deploy-mode neighbors

- [`/charly-core:cmd`](/recipes/core/cmd/) -- Single command execution with D-Bus notification (running containers only)
...
- [`/charly-core:charly-doctor`](/recipes/core/charly-doctor/) -- Host hardware probe that feeds `appendAutoDetectedEnv()` (DRINODE, HSA_OVERRIDE_GFX_VERSION)
- [`/charly-build:settings`](/recipes/build/settings/) -- Engine and bind address settings

### Build-mode references

- [`/charly-image:image`](/recipes/image/image/) -- Box definitions (ports, volumes, env) in `charly.yml`; authoritative source before a pull
- [`/charly-build:build`](/recipes/build/build/) -- Build the image you intend to shell into

### Layer skills

- [`/charly-distros:nvidia`](/recipes/distros/nvidia/), [`/charly-distros:rocm`](/recipes/distros/rocm/) -- GPU layers consuming the auto-injected env
