---
title: "clean"
description: "Prune reusable build artifacts to defaults: retention (images, check runs) and sweep one-time makepkg leftovers."
---
...
- [`/charly-build:build`](/recipes/build/build/) — `charly box build` + the `keep_images` auto-prune.
- [`/charly-check:check`](/recipes/check/check/) — `charly check run` + the `keep_check_runs` auto-prune.
- [`/charly-vm:vm`](/recipes/vm/vm/) — `charly vm destroy --disk` for VM disk removal.
- [`/charly-image:image`](/recipes/image/image/) — the `defaults:` block where retention keys live.
