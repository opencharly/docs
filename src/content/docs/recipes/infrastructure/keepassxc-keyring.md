...
description: "Configure KeePassXC as the freedesktop.org Secret Service provider on a target:local host: enable FdoSecrets, autostart KeePassXC, disable competing daemons (gnome-keyring + kwallet) at the per-user XDG-autostart and systemd-user-unit layers, install pinentry/libsecret, and install generic direnv shell hooks for bash/zsh/fish."
---
...
| Skill | Relationship |
|-------|--------------|
| [`/charly-infrastructure:keepassxc`](/recipes/infrastructure/keepassxc/) | The package-only candy. `keepassxc-keyring` `require:` on it; never composes it. |
| [`/charly-infrastructure:gnupg`](/recipes/infrastructure/gnupg/) | Same — keepassxc-keyring `require:` on gnupg, never composes. |
| [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) | Distinct concern. agent-forwarding is for FORWARDING the host's GPG/SSH agents INTO containers via socket bind-mounts. keepassxc-keyring is about turning the host itself into a Secret Service server. Both can be active simultaneously. |
| [`/charly-build:secrets`](/recipes/build/secrets/) | The CLI surface that talks to KeePassXC after this layer is in place. `charly secrets gpg setup` and `charly secrets gpg doctor` find pinentry-qt + libsecret on PATH because this layer installed them. |
| [`/charly-coder:direnv`](/recipes/coder/direnv/) | Installs the direnv binary. This layer adds the missing piece (the shell hook) for `.envrc` to actually trigger on `cd`. |
| [`/charly-image:layer`](/recipes/image/layer/) | Candy authoring reference. |
| [`/charly-check:check`](/recipes/check/check/) | `check:` block format reference. |
...
- [`/charly-infrastructure:keepassxc`](/recipes/infrastructure/keepassxc/) — package install (parent dependency)
- [`/charly-infrastructure:gnupg`](/recipes/infrastructure/gnupg/) — gpg binary (parent dependency)
- [`/charly-distros:agent-forwarding`](/recipes/distros/agent-forwarding/) — sibling: container-side agent forwarding metalayer
- [`/charly-build:secrets`](/recipes/build/secrets/) — `charly secrets` and `charly secrets gpg` CLI surface
- [`/charly-coder:direnv`](/recipes/coder/direnv/) — direnv binary candy (the shell hook this candy adds completes the chain)
- [`/charly-image:layer`](/recipes/image/layer/) — candy authoring reference
- [`/charly-check:check`](/recipes/check/check/) — declarative testing reference
- [`/charly-local:local-spec`](/recipes/local/local-spec/) — `kind: local` template reference (where this candy is composed)
- [`/charly-local:local-deploy`](/recipes/local/local-deploy/) — `target: local` deploy semantics
