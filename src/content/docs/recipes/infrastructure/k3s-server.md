---
title: "k3s-server"
description: "k3s control-plane (server) node with ServiceLB, Traefik v2, and local-path-provisioner enabled by default."
---
...
| Depends on | [`/charly-infrastructure:k3s`](/recipes/infrastructure/k3s/) |
...
Deploy-scope (using the declarative `kube:` check verb — see [`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/)).
...
## Related Candies
- [`/charly-infrastructure:k3s`](/recipes/infrastructure/k3s/) — Base candy installing the k3s binary (required dep)
- [`/charly-infrastructure:k3s-agent`](/recipes/infrastructure/k3s-agent/) — Worker nodes joining this server
- [`/charly-coder:kubernetes-layer`](/recipes/coder/kubernetes-layer/) — `kubectl`/`helm` on the operator side (not needed in the cluster)
- [`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/) — The test verb used by this candy's deploy-scope checks
