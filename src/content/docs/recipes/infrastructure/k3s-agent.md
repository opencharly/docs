---
title: "k3s-agent"
description: "k3s worker (agent) node — joins an existing k3s-server via pre-shared token."
---
...
Deploy-scope (uses [`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/)). The cluster-probe verb is
...
- [`/charly-infrastructure:k3s-server`](/recipes/infrastructure/k3s-server/) — Control-plane node this agent joins
- [`/charly-kubernetes:check-k8s`](/recipes/kubernetes/check-k8s/) — Test verb used by the agent-joined check
