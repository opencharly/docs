---
title: "tailscale-up"
description: "Tailscale runtime wiring for target:local hosts."
---
...
1. **Starts `tailscaled.service`** via `systemctl start ... || true`.
   The [`/charly-infrastructure:tailscale`](/recipes/infrastructure/tailscale/) candy is `systemctl enable`-only
...
- Any `target: pod` deployment — pod tunnels go through the
  ...
  tailscale, not in-pod). See [`/charly-core:deploy`](/recipes/core/deploy/) "Tailscale Serve".
- Bootc images — they need fresh-boot semantics that don't match the
  deploy-runtime contract here. Use [`/charly-infrastructure:tailscale`](/recipes/infrastructure/tailscale/) alone.
...
[`/charly-core:deploy`](/recipes/core/deploy/) documents a `tunnel: tailscale` field that emits
...
See [`/charly-core:deploy`](/recipes/core/deploy/) "Tailscale Serve" for the per-pod tunnel
matrix and [`/charly-automation:sidecar`](/recipes/automation/sidecar/) for the tailscale-sidecar pattern
(an alternative when a pod needs its own tailnet identity, not just
host-level serve).

## Related Skills

- [`/charly-infrastructure:tailscale`](/recipes/infrastructure/tailscale/) — the install-side companion (daemon
  install + systemctl enable). `tailscale-up` `require:` it.
- [`/charly-core:deploy`](/recipes/core/deploy/) — `charly.yml` `tunnel: tailscale` mechanism that
  consumes the `--operator` permission.
- [`/charly-local:local-deploy`](/recipes/local/local-deploy/) — the `target: local` execution model
  this candy is designed for.
- [`/charly-local:local-spec`](/recipes/local/local-spec/) — `local.yml` template authoring; the
...
- [`/charly-check:check`](/recipes/check/check/) — declarative testing reference (three-state
  pattern used here).
