---
title: "redis"
description: "Redis in-memory data store on port 6379 with periodic persistence."
---
...
- **RPM (Fedora):** `redis` (dnf request) → installed as
  **`valkey-compat-redis`** on Fedora 43. The old `redis` package was replaced;
  ...
  [`/charly-check:check`](/recipes/check/check/) Authoring Gotcha #8.
...
(see [`/charly-check:check`](/recipes/check/check/) for the full schema — this candy is the
...
- [`/charly-immich:immich`](/recipes/immich/immich/) -- primary consumer (depends on redis)
- [`/charly-infrastructure:postgresql`](/recipes/infrastructure/postgresql/) -- often paired with redis in service stacks
- [`/charly-check:check`](/recipes/check/check/) -- declarative testing framework (this candy is the gold-standard pattern)
- [`/charly-image:layer`](/recipes/image/layer/) -- candy authoring + `env_provide` field docs
- [`/charly-infrastructure:valkey`](/recipes/infrastructure/valkey/) -- Remi-repo Valkey 9 package (separate candy; different version than Fedora's default valkey-compat-redis)
