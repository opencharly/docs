---
title: "jupyter-ml-layer"
description: "Full CUDA ML stack + JupyterLab with real-time collaboration and CRDT MCP server on port 8888."
---
...
- [`/charly-jupyter:llama-cpp`](/recipes/jupyter/llama-cpp/) — Sub-candy: llama.cpp binaries
- [`/charly-jupyter:unsloth`](/recipes/jupyter/unsloth/) — Sub-candy: vLLM wheel + fine-tuning + vLLM patch
- [`/charly-jupyter:jupyter-mcp`](/recipes/jupyter/jupyter-mcp/) — Sub-candy: CRDT MCP extension
...
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — MCP consumer (auto-discovers via `CHARLY_MCP_SERVERS`; uses jupyter tools to read/edit/execute cells)
- [`/charly-openwebui:openwebui`](/recipes/openwebui/openwebui/) — MCP consumer (sets `CODE_EXECUTION_ENGINE=jupyter` when this server is discovered, routing Open WebUI code blocks to the Jupyter kernel)
