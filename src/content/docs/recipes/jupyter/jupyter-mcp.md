---
title: "jupyter-mcp"
description: "JupyterLab CRDT MCP server extension with 11 tools (notebook_*/cell_* + room_list + notebook_list_users) for programmatic notebook access."
---
...
- [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) — client-side verb for probing this server's tool catalog (ping, list-tools, call); use `charly check live jupyter --filter mcp` to see all 11 tools this candy registers
...
- [`/charly-hermes:hermes`](/recipes/hermes/hermes/) — downstream MCP consumer (auto-discovers `jupyter` via `CHARLY_MCP_SERVERS`; uses the 11 tools to read/edit/execute notebook cells programmatically)
