...
description: "Full CUDA ML JupyterLab box with finetuning, Ollama, and LLM course notebooks, CRDT MCP server, and real-time collaboration."
---
...
- [`/charly-jupyter:jupyter-ml`](/recipes/jupyter/jupyter-ml/) — Same stack without finetuning notebooks
- [`/charly-jupyter:jupyter`](/recipes/jupyter/jupyter/) — Lightweight variant (no CUDA, multi-arch)
- [`/charly-jupyter:unsloth-studio`](/recipes/jupyter/unsloth-studio/) — Unsloth Studio UI (different pixi env, same finetuning notebooks)

**MCP testing:** same 3 deploy-scope `mcp:` checks as `jupyter-ml` are inherited here. See [`/charly-build:charly-mcp-cmd`](/recipes/build/charly-mcp-cmd/) for the verb reference.
