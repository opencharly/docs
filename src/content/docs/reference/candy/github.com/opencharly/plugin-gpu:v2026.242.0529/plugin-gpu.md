---
title: "plugin-gpu"
description: "GPU/VFIO HOST-DETECTION plugin (cutover C11) — the sysfs/exec probing charly core formerly held in charly/devices.go: NVIDIA-usable-via-CDI detection (nvidia-smi + CDI-spec/nvidia-ctk reachability), AMD-GPU + GFX-version detection (amdgpu DRM + KFD topology), VFIO passthrough readiness (IOMMU groups, per-function PCI scan, display-class GPU + IOMMU-group members), host-device auto-detection (device_patterns glob + real-GPU render-node pick), user-scope CDI-spec generation, and the RLIMIT_MEMLOCK + /dev/vfio group-access passthrough-readiness probes."
---
...
| **Repo** | `box/github.com/opencharly/plugin-gpu:v2026.242.0529` |
| **Plugin** | yes — see the [plugin reference](/reference/plugin/github.com/opencharly/plugin-gpu:v2026.242.0529/plugin-gpu/) |

GPU/VFIO HOST-DETECTION plugin (cutover C11) — the sysfs/exec probing charly core
formerly held in charly/devices.go: NVIDIA-usable-via-CDI detection (nvidia-smi +
CDI-spec/nvidia-ctk reachability), AMD-GPU + GFX-version detection (amdgpu DRM +
KFD topology), VFIO passthrough readiness (IOMMU groups, per-function PCI scan,
display-class GPU + IOMMU-group members), host-device auto-detection (device_patterns
...
| Intent | Step |
|---|---|
| `check` | the gpu-detection plugin ships a buildable Go module the host compiles in |
