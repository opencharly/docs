---
title: "status"
description: "The status command word, served by 2 plugin candies: plugin-status and plugin-status."
---
...
## About `plugin-status`, which serves it
...
`charly --help` prints the command tree, including where each `status` is invoked and under which parent.
