---
title: "shell"
description: "The shell command word, served by 2 plugin candies: plugin-pod and plugin-pod."
---
...
`shell` is a command word served by 2 plugin candies — `plugin-pod` and `plugin-pod` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `shell` is invoked and under which parent.
