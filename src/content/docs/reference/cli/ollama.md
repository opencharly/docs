---
title: "ollama"
description: "The ollama command word, served by 2 plugin candies: plugin-ollama and plugin-ollama."
---
...
`ollama` is a command word served by 2 plugin candies — `plugin-ollama` and `plugin-ollama` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.

## About `plugin-ollama`, which serves it
...
`charly --help` prints the command tree, including where each `ollama` is invoked and under which parent.
