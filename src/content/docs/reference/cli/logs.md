...
`logs` is a command word served by the `plugin-pod` plugin candy. This plugin is listed in `charly/charly.yml`'s `compiled_plugins:`, so its providers are compiled into the `charly` binary and register in-process.
...
`charly --help` prints the command tree, including where `logs` is invoked and under which parent.
