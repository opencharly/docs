...
`reconcile` is a command word served by the `plugin-box` plugin candy. This plugin is listed in `charly/charly.yml`'s `compiled_plugins:`, so its providers are compiled into the `charly` binary and register in-process.
...
`charly --help` prints the command tree, including where `reconcile` is invoked and under which parent.
