---
title: "stop"
description: "The stop command word, served by the plugin-pod plugin candy."
---
...
`stop` is a command word served by the `plugin-pod` plugin candy. This plugin is listed in `charly/charly.yml`'s `compiled_plugins:`, so its providers are compiled into the `charly` binary and register in-process.
...
`stop` may be a top-level word or nested under a parent command. Run `charly --help` for the top-level words, and `charly <parent> --help` for a parent's nested grammar.
