---
title: "new"
description: "The new command word, served by 2 plugin candies: plugin-box and plugin-box."
---
...
`new` is a command word served by 2 plugin candies — `plugin-box` and `plugin-box` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `new` is invoked and under which parent.
