---
title: "new"
description: "The new command word, served by the plugin-box plugin candy."
---
...
`new` is a command word served by the `plugin-box` plugin candy. This plugin is listed in `charly/charly.yml`'s `compiled_plugins:`, so its providers are compiled into the `charly` binary and register in-process.
...
`new` may be a top-level word or nested under a parent command. Run `charly --help` for the top-level words, and `charly <parent> --help` for a parent's nested grammar.
