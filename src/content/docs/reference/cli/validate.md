---
title: "validate"
description: "The validate command word, served by 2 plugin candies: plugin-box and plugin-box."
---
...
`validate` is a command word served by 2 plugin candies — `plugin-box` and `plugin-box` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `validate` is invoked and under which parent.
