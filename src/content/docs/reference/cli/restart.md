---
title: "charly restart"
description: "The restart command word, served by the plugin-pod plugin candy."
---
...
`restart` is a command word served by the `plugin-pod` plugin candy. This plugin is listed in `charly/charly.yml`'s `compiled_plugins:`, so its providers are compiled into the `charly` binary and register in-process.
...
Run `charly restart --help` for the live flag grammar.
