---
title: "restart"
description: "The restart command word, served by 2 plugin candies: plugin-pod and plugin-pod."
---
...
| **plugin-pod — Served by** | [plugin-pod](/reference/plugin/github-com-opencharly-plugin-pod-v2026-266-1109/plugin-pod/) |
| **plugin-pod — Placement** | compiled-in (in-process) |
| **plugin-pod — Version** | `2026.201.0000` |

`restart` is a command word served by 2 plugin candies — `plugin-pod` and `plugin-pod` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.

## About `plugin-pod`, which serves it
...
`charly --help` prints the command tree, including where each `restart` is invoked and under which parent.
