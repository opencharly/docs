---
title: "cp"
description: "The cp command word, served by 2 plugin candies: plugin-pod and plugin-pod."
---
...
`cp` is a command word served by 2 plugin candies — `plugin-pod` and `plugin-pod` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `cp` is invoked and under which parent.
