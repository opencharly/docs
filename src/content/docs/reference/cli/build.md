---
title: "build"
description: "The build command word, served by 2 plugin candies: plugin-box and plugin-box."
---
...
`build` is a command word served by 2 plugin candies — `plugin-box` and `plugin-box` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `build` is invoked and under which parent.
