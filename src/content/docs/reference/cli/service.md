---
title: "service"
description: "The service command word, served by 2 plugin candies: plugin-pod and plugin-pod."
---
...
`service` is a command word served by 2 plugin candies — `plugin-pod` and `plugin-pod` — at different points in the command tree. Both are real invocations; `charly --help` prints which is which.
...
`charly --help` prints the command tree, including where each `service` is invoked and under which parent.
